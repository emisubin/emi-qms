...
public sealed class ProjectStore(DatabaseConnectionStringProvider connectionStringProvider)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new ProjectDetailResponse
        {
            ...
            StatusReason = reader.IsDBNull(15) ? null : reader.GetString(15)
        };
    }
        ...
        CancellationToken cancellationToken)
    {
        await using var dataSource = CreateDataSource();
        await using var connection = await dataSource.OpenConnectionAsync(cancellationToken);
        var existing = await ReadProjectEditSnapshotAsync(connection, null, projectId, cancellationToken);
        if (existing is null)
        {
        ...
        if (!string.Equals(existing.ProjectTitleNormalized, input.ProjectTitleNormalized, StringComparison.Ordinal)
            && await ProjectTitleExistsAsync(connection, null, input.ProjectTitleNormalized, projectId, cancellationToken))
        {
            return ProjectMutationResult<ProjectDetailResponse>.Conflict("동일한 PJT Title이 이미 존재합니다.");
        }
    ...
    private static ProjectListItemResponse ReadProjectListItem(NpgsqlDataReader reader, bool includeSalesAmount)
    {
        return new ProjectListItemResponse
        {
            ...
            DeliveryLocation = reader.IsDBNull(9) ? null : reader.GetString(9),
            Status = reader.GetString(10),
            CreatedAt = reader.GetFieldValue<DateTimeOffset>(11),
            UpdatedAt = reader.GetFieldValue<DateTimeOffset>(12),
            SalesAmount = includeSalesAmount && !reader.IsDBNull(13) ? reader.GetDecimal(13) : null,
            CurrencyCode = includeSalesAmount && !reader.IsDBNull(14) ? reader.GetString(14) : null
        };
    }
        ...
        CancellationToken cancellationToken)
    {
        await using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = "select status from projects where id = @project_id for update;";
        command.Parameters.AddWithValue("project_id", projectId);
