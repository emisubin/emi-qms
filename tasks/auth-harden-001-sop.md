...
사용자 비활성화, System Administrator role 제거, 삭제 예약·bulk delete와 malformed lifecycle purge 방어에 적용한다. 직접 SQL 변경 절차가 아니다.
...
정상 삭제 예정 row는 inactive이며 canonical set 밖이어야 한다. Lifecycle marker와 active EntraId + System Administrator role이 함께 존재하면 purge 전용 guard가 canonical role row를 잠그고 다른 canonical administrator를 재계산한다.

- 다른 canonical administrator 0: 기존 last-admin domain error와 HTTP 400, row 변경 0
- 다른 canonical administrator 1 이상: guard 통과 후 기존 reference scan 적용
- Role assignment 등 reference 존재: 기존 정책에 따라 `PurgeBlocked`

Malformed lifecycle state는 정상 application flow가 아니므로 data-integrity Finding으로도 기록한다. Guard 거부 row를 직접 SQL로 수정하거나 강제 purge하지 않는다.

## 12. Background purge

Due purge는 stable target ordering과 기존 batch transaction을 사용한다. Purge 전용 guard는 lifecycle marker를 제외한 active EntraId + canonical role target만 role-row boundary로 보낸다. 다른 canonical administrator가 0이면 exception으로 batch 전체를 rollback한다. 앞선 row의 delete·log도 commit되지 않아야 한다. Guard rejection이나 DB 오류가 있으면 row를 임의 삭제하지 않는다.
...
- Malformed lifecycle fixture는 isolated synthetic DB에서만 구성
- Purge guard와 generic reference scan 결과를 구분
...
- [x] Malformed lifecycle immediate purge의 last-admin rejection 확인
- [x] Due purge rejection의 batch 전체 rollback 확인
...
| 2026-07-13 | Change 001 REDESIGN 승인과 purge 전용 predicate·batch rollback 절차 반영 |
| 2026-07-13 | Change 001 targeted 5/5·backend 361/361·표준 전체 검증 완료, 사용자 검수 대기 전환 |
| 2026-07-13 | Change 001 사용자 검수 완료와 merge 승인 반영 |
