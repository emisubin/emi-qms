...
| 2 | TASK-DESIGN-LOGIN-001 후속 change — 공통 PC·모바일 로그인 | 지정 Figma와 기존 배포 디자인의 차이만 보정. 로그인·승인·재인증 동작 보존 |
| 3 | [TASK-OSAN-PROGRESS-001](https://github.com/emisubin/emi-qms/blob/c3a3c79374babc840dca054bd1a05237a2c49685/tasks/osan-progress-001.md) — 모바일 진행 상세·완료 | 대상 개별·전체 선택, 7단계 이동·설명·안내 사진과 완료 처리. 최신 승인 계약의 개별/일괄 처리·포장 선행조건·전체 대상 포장 후 프로젝트 자동 완료 보존 |
| 4 | [TASK-OSAN-PHOTO-001](../tasks/osan-photo-001.md) — 완료 증빙 사진 | 촬영·첨부·미리보기·영구 저장·완료 후 재조회, 진행 상세·완료 처리와 통합. 저장 구조·제한·일괄 대상 조회 표현은 설계에서 확정 |
...
이번 요청은 로드맵·관련 Task 문서 정합성 확인과 로컬 커밋 후 UX-001 구현 기준 확정 착수까지다. 제품 구현·원격 반영·배포는 포함하지 않는다. 사용자가 작업을 지정하면 실질 선행조건을 확인해 그 요청을 실행하고, 단순 큐 순서 차이로 재승인을 만들지 않는다. 하네스 재구축의 기록·원격 반영 상태는 [TASK-GOV-CODEX-002](../tasks/gov-codex-002.md)에서 별도로 추적한다.
