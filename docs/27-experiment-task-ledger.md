...
TASK-PRODUCTION-CONTROL-001은 migration `0058`의 Item별 제조·생산계획·연결 version과 프로젝트 snapshot, migration `0059`의 LQC 제조 단계 불변 identity를 추가했다. 유효한 두 양식이 사용 중인 뒤 생성되는 프로젝트만 `LinkedV1`이 되고 기존 프로젝트와 양식 없는 Item은 `Legacy` UI·데이터를 유지한다. 연결형 프로젝트는 생산관리 탭의 6열 계획 대비 실적표, 항목별 자동 근거와 계획/실적 가로 막대 일정으로 구매·자재·제조·품질·물류 원본 사실을 조회 시 집계하며 FAT 비필수 프로젝트는 FAT를 분모에서 제외한다. Backend Release build·생산계획 API/통합 `21/21`, migration 전체 `34/34`, Frontend lint error 0·build·전체 `138/138`, 고정 검수 runtime desktop·390px 증빙을 통과했고 사용자 최종 일괄 검수만 남는다. Backend 전체 회귀는 현 로컬 runner에서 5분간 완료 출력이 없어 중단했으며 대표 승격 전 clean runner에서 다시 수행한다.
