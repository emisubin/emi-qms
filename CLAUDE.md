...
- 최신 interview, 승인된 planning과 review, Roadmap, 직접 관련된 구현과 tests를 다시 읽는다.
...
- `draft`는 atomic exclusive create로 기존 target이나 symlink를 덮어쓰지 않고, `revise`는 승인된 target과 review가 모두 있을 때만 사용한다.
