...
| 3.3M | TASK-SITE-ACCESS-001 — 유지 세션 포함 사이트 접속 이력 | APPROVED_FEATURE_IMPLEMENTATION / P2_REMEDIATION | Latest-main Integration / Automated, Full-Stack & Independent Validation Complete / User Validation Pending / Publication Approved | 사용자·브라우저 client·30분 활동 창별 접속 원장, 19개 고정 메뉴, 서버/DB 권위 시각, 명시적 로그아웃 종료, 별도 coverage·시간 해석 안내와 전체 감사 이력·선택 Excel 통합을 구현했다. 공개 G2의 migration `0084`·추적 97을 보존하고 사이트 접속을 `0085`·추적 98로 교정했다. 독립 검증에서 제품·통합 PASS 뒤 상태 문서 P2 `SITE-ACCESS-FINAL-F01`까지 `0274756ab300827d62ee385a83d66773d346b6ca` read-only 재확인으로 닫아 Open P0/P1/P2 `0/0/0`·local GO다. | TASK-AUDIT-001, 기존 Entra/MSAL identity, `Audit.Read.All`, migration `0083`, 공개 G2 `0084` | 사용자 화면 검수 대기. Persistent UAT·Azure 배포 미승인. Git 게시·원격 main 병합은 승인·미실행 | Yes | Push·Ready PR·CI Gate·원격 main merge |
...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 026 Main Merged / Azure Released / User Validation Pending | PR #108과 main CI를 통과한 exact main SHA `51aba7e97a2d1fee0f9ee4b82a3f89d514171acf`를 Azure release `32197298425`로 배포했다. Backend·Frontend만 교체하고 migration을 생략했으며 public security smoke를 통과했다. 기존 Web Push·Teams·메일·Key Vault 참조와 업무 데이터는 보존했다. | TASK-AZURE-PILOT-001, TASK-CI-COST-001, TASK-WORKFLOW-CONTINUITY-001 Change 018 | 기존 운영 인증·알림 설정·업무 데이터 보존 | Yes | 공개 전체 흐름 상태 전용 표시 사용자 검수 → 운영 관찰 |
...
| 6.5 | TASK-G2-OPERATIONS-002 — 납품 목표·불량·홈 임시 시뮬레이션 | NEW_FEATURE / BUGFIX | Change 003 Automated & Independent Validation Complete / Publication & Azure Deployment Approved | 사용자 승인 Codex 직접 기획·구현. Additive migration `0084`로 납품 목표와 불량을 정식 데이터에 추가하고 홈 저장 없는 임시 시뮬레이션을 제공한다. Change 003은 2026-08-28부터 `오늘 재고 = 전일 재고 + 전일 생산 - 전일 납품 - 전일 불량`으로 바꿔 해당 날짜에 출하 가능한 시작 재고를 표시하고 홈 임시 입력도 다음 날짜 재고부터 반영한다. Backend `573/573`, Frontend `249/249`, 실제 PostgreSQL 경계·부분 조회와 isolated Full-Stack `1/1`을 통과했다. | TASK-G2-OPERATIONS-001 완료 계약, 최초 PR #115·exact main `220d1201c9dbb881fb3e5c5061871fb943c7961b`, Change 003 기준 main `a8bb000dbbbe7d307bf1de96259917b750460497` | Persistent UAT 미적용. 운영 G2 원본 데이터·schema 무변경 | Yes | Ready PR 필수 CI → exact main Azure Change 028 → 공개 8월 28일 재고 `6대` 확인 |
...
| 97 | G2 납품 목표·불량·홈 임시 시뮬레이션 | 최초 공개 완료 / Change 003 게시·Azure 배포 승인 | 사용자/제조/영업/물류/개발 | TASK-G2-OPERATIONS-002 | 최초 exact main `220d1201c9dbb881fb3e5c5061871fb943c7961b`에 납품 목표·불량·홈 임시 예상값과 migration `0084`가 반영됐다. Change 003은 2026-08-28부터 전일 실적 기반 출하 가능 재고로 교정하며 schema·원본 데이터·Persistent UAT는 변경하지 않는다. |
| 98 | 유지 세션 포함 사이트 접속 감사 | Latest-main automated/full-stack/independent validation complete / user validation pending / publication·remote main merge approved | 사용자/개발/운영 | TASK-SITE-ACCESS-001 | 새로고침·다른 화면 진입을 사용자+브라우저 client+30분 창으로 한 행에 묶고 명시적 로그아웃·고정 메뉴·접속 환경을 전체 감사 이력과 선택 Excel에서 확인한다. 사용자 검수 완료와 코드 병합 승인은 별도 Gate다. 공개 G2의 추적 97·migration 0084를 보존하고 사이트 접속은 추적 98·migration 0085로 교정했으며 Open P0/P1/P2 `0/0/0`·local GO다. Persistent UAT·Azure 배포는 미승인이다. |
