...
| 3.3M | TASK-SITE-ACCESS-001 — 유지 세션 포함 사이트 접속 이력 | APPROVED_FEATURE_IMPLEMENTATION / P2_REMEDIATION | Latest-main Integration / Automated & Full-Stack Validation Complete / Independent Product Validation PASS / Documentation P2 Final Recheck Pending / User Validation Pending / Publication Approved | 사용자·브라우저 client·30분 활동 창별 접속 원장, 19개 고정 메뉴, 서버/DB 권위 시각, 명시적 로그아웃 종료, 별도 coverage·시간 해석 안내와 전체 감사 이력·선택 Excel 통합을 구현했다. 공개 G2의 migration `0084`·추적 97을 보존하고 사이트 접속을 `0085`·추적 98로 교정했다. 독립 검증은 제품·통합을 PASS했고 상태 문서 불일치 P2 `SITE-ACCESS-FINAL-F01`의 1차 보정 재확인에서 남은 stale Gate 문장까지 교정해 current HEAD 최종 확인을 기다린다. | TASK-AUDIT-001, 기존 Entra/MSAL identity, `Audit.Read.All`, migration `0083`, 공개 G2 `0084` | 사용자 화면 검수 대기. Persistent UAT·Azure 배포 미승인. Git 게시·원격 main 병합은 승인됐지만 P2 최종 재확인 전 실행 금지 | Yes | current HEAD `SITE-ACCESS-FINAL-F01` 재확인 → Push·Ready PR·CI Gate·원격 main merge |
...
| 6.5 | TASK-G2-OPERATIONS-002 — 납품 목표·불량·홈 임시 시뮬레이션 | NEW_FEATURE | Main Merged / Azure Released / User Validation Complete | 사용자 승인 Codex 직접 기획·구현. Additive migration `0084`로 납품 목표와 불량을 정식 데이터에 추가하고, 불량 차감 재고·기존 forecast expiry·권한/CAS를 유지했다. 홈 첫 그래프 아래 생산 현황표에서 생산·납품·불량을 저장 없이 바꾸면 그래프·KPI·재고가 즉시 갱신되고 재조회 시 폐기된다. 납품 목표 주황 점선, 표 행 순서, 예상 파랑·휴일 빨강 우선, 390px 날짜 입력 축소와 Change 001 직접 입력 중앙 정렬·증감 버튼 제거를 자동·격리 browser와 사용자 검수로 확인했다. | TASK-G2-OPERATIONS-001 완료 계약, PR #115, exact main `220d1201c9dbb881fb3e5c5061871fb943c7961b` | Persistent UAT 미적용. Azure 공개 release 완료 | Yes | 운영 관찰 / 후속 변경은 별도 Task·승인 |
...
| 97 | G2 납품 목표·불량·홈 임시 시뮬레이션 | PR #115 main 병합·Azure 공개배포 완료 | 사용자/제조/영업/물류/개발 | TASK-G2-OPERATIONS-002 | exact main `220d1201c9dbb881fb3e5c5061871fb943c7961b`에 납품 목표·불량 정식 저장, 불량 차감 재고, 저장 없는 홈 임시 예상값과 migration `0084`가 반영됐다. Persistent UAT는 미적용 |
| 98 | 유지 세션 포함 사이트 접속 감사 | Latest-main automated/full-stack validation complete / independent product PASS·documentation P2 final recheck pending / user validation pending / publication·remote main merge approved | 사용자/개발/운영 | TASK-SITE-ACCESS-001 | 새로고침·다른 화면 진입을 사용자+브라우저 client+30분 창으로 한 행에 묶고 명시적 로그아웃·고정 메뉴·접속 환경을 전체 감사 이력과 선택 Excel에서 확인한다. 사용자 검수 완료와 코드 병합 승인은 별도 Gate다. 공개 G2의 추적 97·migration 0084를 보존하고 사이트 접속은 추적 98·migration 0085로 교정했으며, `SITE-ACCESS-FINAL-F01` 최종 재확인 전에는 게시하지 않는다. |
