...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 009 Main Merged / Teams 1.0.4 Approval Submitted / Change 010 Image Publish Approved / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `0068 Exact`·PITR restore와 세 workload readiness 완료. Change 009 PR #70을 원격 main에 병합하고 공개 Activity type 10개·Backend 자동 delivery·migration `0069`·Teams `1.0.4` package 검증과 Teams 승인 요청을 완료했다. DNS TXT는 권한 DNS `3/3`·공용 resolver `4/4`에서 일치하고 Azure CNAME 진단도 성공했다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~009 기준선, migration `0068` handover, 사용자 문서 동기화·최신 main image 게시 승인 | Change 010 문서 merge·최종 main image 게시, migration `0069`·Backend revision, Front Door validation/TLS, Teams 관리자 승인·조직 catalog·actual provider 미완료. 에스컬레이션 세부 정책은 후속 기획 | Yes | Change 010 문서 merge·최종 main image → migration `0069`·Backend revision → DNS/TLS → Teams 승인·설치·event별 provider 검수 |
...
- 상태/다음 순서: Change 009 PR #70 원격 main 병합·CI 완료, Foundation·secret-scope RBAC·image 게시·DB role bootstrap·migration `0068 Exact`·PITR restore rehearsal 완료 / 세 Container Apps `3/3 Running`, Backend·ClamAV internal ingress·Frontend external ingress / DNS validation TXT 권한 DNS `3/3`·공용 resolver `4/4` 일치, Azure CNAME 진단 성공, Front Door domain `Pending`·TLS/route `NotStarted`, public traffic·external notification 비활성 / Teams `1.0.4` 관리자 승인 요청 제출 / Change 010 문서 merge 뒤 최종 main Backend·Frontend image 게시가 다음 Gate
...
- 예상 migration: Azure는 `0068 Exact`, 최신 main은 additive constraint migration `0069` 포함. 최신 migration image의 one-shot job으로 `0069 Exact`를 확인하기 전 새 application revision을 활성화하지 않음
