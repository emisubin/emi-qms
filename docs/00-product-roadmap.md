...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 023 Publication Approved | 기존 release guard를 유지한다. Change 023은 사용자 검수를 마친 프로젝트 기본정보·부서 Pending, 관리자 홈, 설계 도번·열반을 `main-pr-only` Ruleset의 단일 PR·필수 `CI Gate` 뒤 exact main SHA로 공개 배포한다. migration은 `0076`→`0077`, 앱 교체는 Backend→Frontend 순서다. | TASK-AZURE-PILOT-001, Change 018 승인형 workflow, TASK-CI-COST-001, TASK-PROJECT-PENDING-001, TASK-ADMIN-001 Change 001, TASK-PANEL-DESIGN-001 Change 001 | 기존 운영 인증·PWA Web Push·Teams·메일 설정과 데이터 보존. 기존 P3 runner 경고·20일 운영 관찰 유지 | Yes | 통합 PR CI → migration `0076`·`0077` → Backend·Frontend release → public security smoke |
