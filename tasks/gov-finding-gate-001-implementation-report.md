...
Support closure 확인 중 raw page snapshot 1건과 publication push 결과의 원격 account path 원문 1건이 fixed-projection 경계를 재차 벗어났지만 tracked leak·secret은 0이었다. Support는 completion/follow-up/closed `1/1/1`·cached reference `REMOVED`, GitHub는 PR number·state·check count만 반환하도록 보정해 절차 P2를 Resolved로 유지했다.
...
- GitHub app connector 재조회: 설치 범위 제한으로 unavailable. origin/main과 인증된 CLI fixed projection으로 보완
...
- 문서 PR #50 squash merge: 사용자 승인 / 실행 중
