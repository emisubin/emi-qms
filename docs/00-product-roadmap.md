...
| 3.3M | TASK-SITE-ACCESS-001 — 유지 세션 포함 사이트 접속 이력 | APPROVED_FEATURE_IMPLEMENTATION | Local Implementation / Automated, Full-Stack & Independent Validation Complete / User Validation Pending / Publication Not Approved | 사용자·브라우저 client·30분 활동 창별 접속 원장, 19개 고정 메뉴, 서버/DB 권위 시각, 명시적 로그아웃 종료, 별도 coverage·시간 해석 안내와 전체 감사 이력·선택 Excel 통합을 구현했다. 기존 Login/Logout·변경·권한 감사 의미는 보존하며 독립 검증은 Open P0/P1/P2 `0/0/0`·local GO다. | TASK-AUDIT-001, 기존 Entra/MSAL identity, `Audit.Read.All`, migration `0083` | 과거 접속 소급, heartbeat·클릭·키 입력·모든 HTTP, URL·query·업무 식별자, 실제 근무시간 추론, Git 게시·Persistent UAT·Azure 배포 제외 | Yes | 사용자 local 화면 검수 → 별도 게시 승인 |
