# 통합정보시스템 Product Roadmap

## 1. 문서 목적

이 문서는 EMI 통합정보시스템의 전체 개발 방향, 업무 프로세스, 확정사항, 미확정 추적 대상, 후속 TASK 우선순위를 한 곳에 정리하는 기준 문서다.
...
본 시스템은 단순 품질관리시스템이 아니라, 프로젝트 생성부터 납품 완료와 영업 정산까지 부서별 업무 흐름을 연결하는 통합정보시스템이다.
...
| 제품/패널 정보 | 제품/패널 | 패널명, 제품명, 사이즈, QR 가능 여부 |
| 생산계획 | 프로젝트 | Item 기준 생산단계, 예정일, 담당자 지정 |
| 구매정보 | 구매품목 | 발주품목, 기술 담당자, 발주일, 입고일, 이슈, 입고 완료 |
| 자재 도착 | 구매품목 | 구매품목별 도착 등록 |
| 입고 확정 | 구매품목 | 수입검사 적합 후 사용 가능 자재로 확정 |
| 키팅 완료 | 제품/패널 | 제조 투입 가능 상태 |
| 제조 작업 | 제품/패널 및 제조 단계 | 작업 시작, 작업 종료, 제조 중단 |
| 검사 | 검사 단위 및 제품/패널 | IQC, LQC, OQC, 전진검수, FAT |
| Pending List | 이슈 단위 | 부적합, PUNCH, 제조 중단, 기타 |
| 포장 | 포장 단위 및 제품/패널 | 포장번호, 포함 제품, 사진 |
| 납품 | 제품/패널 및 프로젝트 | 출발, 납품 완료, 거래명세서 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | 영업 | 프로젝트 생성 | 프로젝트 | 고객사, Item, PJT Code, PJT Title, 면수, 납기일, 영업담당자, 포장방식, FAT 필요 여부 | 필수 프로젝트 정보 생성 | 생산관리: 생산계획·담당자 입력 | 관련 부서 프로젝트 생성 참조 | FAT 필요 여부는 후속 구현 필드 |
| 2 | 생산관리 | 생산계획·담당자 | 프로젝트 | 생산계획 단계, 예정일, 구매/생산관리/제조/품질/물류 담당자 | 필수 생산계획 예정일 및 required 담당자 기준 충족 | 설계: 제품명·사이즈 입력, 구매: 구매정보 입력 가능 | 영업, 구매, 제조 참조 | 생산계획 skeleton은 새 프로젝트 생성 시 자동 생성 |
| 3 | 설계 | 제품명·사이즈 | 제품/패널 | 패널명, 제품명, 사이즈 | 활성 패널의 필수 제품/패널 정보 입력 | 구매: 구매정보 입력 | 생산관리 참조 | 목포장 프로젝트는 사이즈 필수 |
...
| 6 | 품질 | 수입검사 | 구매품목 또는 제품/패널 | IQC 체크, 값 입력, 외함 사진, 적합/부적합 | IQC 적합 또는 Pending 등록 | 자재: 입고 확정 | 구매, 생산관리 참조 | 부적합 시 Pending List |
| 7 | 자재 | 입고 확정 | 구매품목 | 입고 확정, 사용 가능 처리 | IQC 적합품 입고 확정 | 자재: 키팅 완료 | 생산관리 참조 | 구매품목 단위 |
| 8 | 자재 | 키팅 완료 | 제품/패널 | 키팅 완료 여부, 부분/일괄 처리 | 제조 투입 가능 상태 | 제조: 제조 작업 입력 | 생산관리, 제조 참조 | 별도 생산 불출 단계 없음 |
| 9 | 제조 | 제조 작업 | 제품/패널 및 제조 단계 | 작업 시작, 작업 체크, 작업 종료, 중단 사유 | 제조 단계 작업 완료 | 품질: LQC 입력 | 생산관리 참조 | 제조 중단은 Pending List |
| 10 | 품질 | LQC | 제품/패널 또는 검사 단위 | LQC 체크리스트, 값, 사진, 적합/부적합 | LQC 적합 또는 Pending 등록 | 제조: 제조 완료 입력 | 제조, 생산관리 참조 | 상세 양식 회신 대기 |
| 11 | 제조 | 제조 완료 | 제품/패널 | 제조 완료 체크, 완료일 | 제조 완료 처리 | 품질: 자체검수 입력 | 생산관리 참조 | 완료 후 OQC로 연결 |
| 12 | 품질 | 자체검수 | 제품/패널 또는 검사 단위 | OQC 체크리스트, 값, 사진, 적합/부적합 | 자체검수 적합 또는 Pending 등록 | 품질: 전진검수 입력 | 제조, 생산관리 참조 | 상세 양식 회신 대기 |
| 13 | 품질 | 전진검수 | 프로젝트 및 제품/패널 | 전진검수 결과, PUNCH LIST | 전진검수 완료 또는 Pending 등록 | 품질: FAT 또는 물류: 포장 완료 | 영업, 생산관리 참조 | PUNCH는 제품/패널 단위 가능 |
| 14 | 품질 | FAT 선택 | 프로젝트 및 제품/패널 | FAT 결과, 고객 확인, PUNCH LIST | FAT 필요 시 완료 또는 Pending 등록 | 물류: 포장 완료 | 영업, 생산관리 참조 | 선택 단계. FAT 불필요 프로젝트는 제외 |
| 15 | 물류 | 포장 완료 | 포장 단위 및 제품/패널 | 포장번호, 포함 제품, 포장사진 | 포장 사진 포함 완료 | 물류: 출발 처리 | 영업, 생산관리 참조 | Packing Unit 필요 |
| 16 | 물류 | 출발 처리 | 제품/패널 | 상차 여부, 상차 사진, 출발일 | 출발 처리 완료 | 물류: 납품 완료 | 영업 참조 | 출발 사진 필수 |
| 17 | 물류 | 납품 완료 | 제품/패널 및 프로젝트 | 납품 완료, 거래명세서 서명본 | 납품 완료 증빙 등록 | 영업: 세금계산서·완료 처리 | 영업, 생산관리 참조 | “출하완료” 대신 “납품완료” |
| 18 | 영업 | 세금계산서·완료 | 프로젝트 | 세금계산서 발행, 완료 체크 | 모든 제품 납품 및 세금계산서 발행 | 없음 | 관련 부서 완료 알림 | 최종 프로젝트 완료 |
...
품질 담당자는 중복 지정 가능하다. 예를 들어 LQC 담당자와 전진검수/FAT 담당자가 같은 사용자여도 허용한다.
...
| DesignPrimary | 설계 정담당자 | 제품명·사이즈 입력 담당 |
...
| QualityIQC | IQC 담당자 | 수입검사 담당 |
| QualityLQC | LQC 담당자 | LQC 담당 |
| QualityOQC | OQC 담당자 | 자체검수 담당 |
| QualityCustomerInspection | 전진검수/FAT 담당자 | 전진검수 및 FAT 담당 |

담당자 fallback 규칙은 다음 순서를 따른다.

1. 해당 단계의 Primary 또는 품질 단계 담당자
2. 없거나 비활성이면 해당 단계 Secondary
...
- 패널명, 제품명, 사이즈는 설계가 입력한다.
- 패널명 입력 시 시스템상 QR 생성 가능 상태가 된다.
- 목포장 프로젝트는 사이즈 입력이 필수다.
- 패널정보 완료 여부와 QR 가능 여부는 별도 표시한다.
- 실제 작업 진행은 제품/패널별로 관리한다.
- 프로젝트 상태는 workflow 기준 요약 상태로 표시한다.

프로젝트 상태는 관리 상태와 workflow 상태를 함께 고려한다.
...
프로젝트 진행률은 기본적으로 완료된 필수 workflow 단계 수 / 전체 필수 workflow 단계 수로 계산한다. FAT는 선택 단계이며, FAT 필요 프로젝트만 진행률 분모에 포함한다.
...
- 담당자 지정도 workflow 완료 판정에 포함될 수 있으나, 구체 required 담당자 기준은 TASK별로 명시한다.
...
| --- | --- | --- | --- | --- |
| IQC | 구매품목 또는 제품/패널 | 수입검사서 체크, 값 입력, 적합/부적합 | 외함 사진 필수 | 상세 항목 후속 |
| LQC | 제품/패널 또는 검사 단위 | LQC 성적서 입력 | 필수 위치 회신 대기 | 입력 방식 회신 대기 |
| OQC | 제품/패널 또는 검사 단위 | 자체검수 성적서 입력 | 필수 위치 회신 대기 | 입력 방식 회신 대기 |
| 전진검수 | 프로젝트 및 제품/패널 | 검수 결과, PUNCH LIST | 필요 시 첨부 | 필수 단계 |
| FAT | 프로젝트 및 제품/패널 | 고객 입회 검사, PUNCH LIST | 필요 시 첨부 | 선택 단계 |
...
| 영역 | 현재 구현됨 | 후속 수정 필요 |
| --- | --- | --- |
| 프로젝트 | 생성, 목록, 상세, 수정, 상태 변경, 삭제/복구/보관함 | FAT 필요 여부 추가, workflow 기준 상태 고도화 |
| 패널정보 | placeholder, 패널정보 입력, Excel preview/apply, 목포장 사이즈 검증 | 설계 단계 완료 판정 정교화 |
| 포장방식 | StretchWrap/WoodenCrate 등 기본 포장방식 | 포장방식 관리자 기준정보화 |
| 구매정보 | 직접 입력, Excel preview/apply, 입고 완료, 완료일시 표시, grouped history | 업체 헤더, 필수 항목 설정 UI 운영화 |
| 자재 | 자재 입고 입력 기반 | 자재 도착/IQC 요청/입고 확정/키팅 분리 |
| 생산관리 | 메뉴, 목록, 프로젝트 펼침, 생산계획 조회/수정, 담당자 지정 | 담당자 구조 18단계 기준 완전 확장 |
...
| 내 업무 | 목록, KPI, 프로젝트별 그룹, 시작/완료 | 시작/완료 이력 관리자 화면 |
| 알림 | 전체/읽음/읽지 않음, 프로젝트별 그룹, 읽음 처리 | 긴급/차단 알림과 Pending List 연결 |
| workflow | 18단계 stage, 프로젝트 workflow 요약, 일부 기존 페이지 hook | 모든 단계와 실제 화면 연결 |
...
## 20. 현재 기능에서 수정해야 할 방향

현재 개발된 기능에서 앞으로 수정해야 할 주요 방향은 다음과 같다.

- 담당자 구조를 18단계 책임 유형으로 완전히 확장한다.
- 품질 담당자를 IQC, LQC, OQC, 전진검수/FAT 책임 유형으로 분리한다.
- 프로젝트 생성 시 FAT 필요 여부를 입력하게 한다.
- 구매정보에 업체 헤더와 업체별 구매품목 구조를 도입한다.
...
| 15 | FAT 필요 여부 기본값 | 미확정 | 영업/품질 회신 | TASK-006B/012A | 프로젝트 생성 필드 필요 |
| 16 | 구매 업체 입력 방식 | 미확정 | 구매 회신 | TASK-006B 또는 후속 구매 TASK | 업체 header 구조 필요 |
...
| 제품/패널 | 실제 진행, 검사, 포장, 납품 추적 단위 | 패널명 입력 시 QR 가능 |
| Item | 제품 기준값 | UL67, UL891, UL508A, IEC, LLP, RPP |
| QR | 제품/패널 추적용 식별 수단 | 시스템 생성 기준과 현장 부착 기준 구분 |
