...
export function MaintenanceAnnouncement({status,userKey,scope,unavailable,onOpenNotice}:{status?:MaintenanceStatus;userKey?:string;scope:string;unavailable:boolean;onOpenNotice:(id:string)=>void}) {
 ...
 useEffect(()=>{if(!status?.releaseId||!status.popupPending||identity===claimed)return;let active=true;
  ...
  return()=>{active=false;};
 },[status?.releaseId,status?.popupVersion,status?.popupPending,userKey,scope,identity,claimed]);
 ...
 {open&&<section className="update-popup" role="region" aria-label="업데이트 안내"><header><h2>{status.title||'PMS 업데이트 안내'}</h2><button aria-label="업데이트 안내 닫기" onClick={()=>setOpen(false)}>×</button></header><div className="update-popup-body"><p>{caption}</p><dl><dt>저장 제한 예정 시간</dt><dd>{time(status.startsAtUtc)} ~ {time(status.expectedEndsAtUtc)}</dd></dl><p className="update-subtitle">업데이트 내용</p><div className="maintenance-body">{status.body}</div><p className="update-note">입력 중인 내용은 유지됩니다. 완료 안내 후 내용을 확인하고 다시 저장해 주세요. 새로고침할 필요는 없습니다.</p></div><footer><small>계정마다 한 번 표시되는 안내입니다.</small>{status.noticeId&&<button onClick={()=>{onOpenNotice(status.noticeId!);setOpen(false);}}>공지사항 보기</button>}<button onClick={()=>setOpen(false)}>확인</button></footer></section>}</>;
}
