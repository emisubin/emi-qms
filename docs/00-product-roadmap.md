...
| 1.4C | TASK-NOTICE-EDITOR-001 공지 굵게·수정·첨부 | APPROVED_FEATURE_IMPLEMENTATION / UAT_RUNTIME | Change 002 User Validation Complete / Publication Approved | 제한형 굵게, 작성자 CAS 수정·revision, 작성·편집 첨부와 인증 사용자 다운로드, migration `0073`을 구현·검증하고 사용자가 통합 PR·Azure 공개 배포를 승인 | TASK-NOTICE-BOARD-001·Upload Security 계약·TASK-CI-COST-001 | 실제 malware scanner/provider와 새 알림 채널 제외 | Yes | latest main 통합 PR `CI Gate` → Change 021 Azure release |
...
| 3.3E | TASK-PRIVACY-NOTICE-001 개인정보·이용 안내 | APPROVED_FEATURE_IMPLEMENTATION / POLICY_DECISION | Change 006 User Validation Complete / Publication Approved | 로그인 후 정적 안내, 공통 footer 진입점, 부드러운 목차 이동, logo 홈 이동, 프로필 사진 선택 안내와 PWA 설치·알림 안내를 구현하고 회사 보유 문구·현재 운영 범위를 승인 | TASK-TEAMS-PWA-001·현재 Microsoft Teams/메일 운영 계약 | 새 provider·Web Push·동의 DB·법률 자문 제외. 변경 시 P3 재검토 | Yes | latest main 통합 PR `CI Gate` → Change 021 Azure release |
