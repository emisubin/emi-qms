...
관리자 대시보드는 숫자만 표시하지 않고 즉시 상세 추적 화면으로 이동할 수 있어야 한다.

- 발송 실패 카드는 `/admin/system/notification-deliveries?status=Failed`로 이동한다.
- 발송 대기 카드는 `/admin/system/notification-deliveries?status=Pending`으로 이동한다.
- 진행 중 에스컬레이션 카드는 `/admin/system/work-item-escalations?status=Active`로 이동한다.
- 진행 중 에스컬레이션은 L0 예정일 임박, L1 예정일 초과, L2 초과 +2영업일, L3 초과 +3영업일로 breakdown을 표시한다.
- 알림 발송 상태 화면은 실패/대기 필터와 관리자 조치 안내를 표시한다.
- 에스컬레이션 상태 화면은 active escalation의 의미와 단계별 조치 안내를 표시한다.
- 발송 실패 수동 재처리 UI와 에스컬레이션 수동 해제 UI는 이번 범위에서 제외한다.

## 8-7. 관리자 표 정렬 원칙

- 관리자 표는 header/body column count와 width가 일치해야 한다.
- 텍스트 컬럼은 좌측 정렬한다.
- 상태 badge와 날짜/시각은 중앙 정렬한다.
- 숫자는 우측 또는 중앙 정렬 중 화면별 일관성을 유지한다.
- checkbox 컬럼은 중앙 정렬한다.
- 작업 버튼은 우측 정렬한다.
- 모바일에서는 page-level horizontal overflow가 생기지 않도록 table 내부 scroll 또는 안전한 responsive layout을 사용한다.
...
- [ ] 실패 상세 목록에서 어떤 알림이 실패했는지와 오류 코드/조치 안내를 확인할 수 있음
- [ ] 발송 대기 카드를 누르면 대기 상세 목록으로 이동함
- [ ] 대기 상세 목록에서 어떤 알림이 대기 중인지와 다음 시도 시각/대기 사유를 확인할 수 있음
- [ ] 진행 중 에스컬레이션 카드에서 L0/L1/L2/L3 breakdown을 확인할 수 있음
- [ ] 진행 중 에스컬레이션 카드를 누르면 상세 목록으로 이동함
- [ ] 에스컬레이션 상세에서 프로젝트/업무/담당자/due_date/current_level을 확인할 수 있음
- [ ] 에스컬레이션 안내 문구가 표시됨
- [ ] 제거 대상 메뉴가 보이지 않음
  - Item 관리
  - 포장방식 관리
