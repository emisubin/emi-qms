...
export function OsanNoticeBoard({userKey, noticeId, compose, admin, enabled, onList, onOpen, onCompose}:{userKey?:string;noticeId?:string;compose?:boolean;admin:boolean;enabled:boolean;onList:()=>void;onOpen:(id:string)=>void;onCompose:()=>void}) {
 ...
 </form>:noticeId?<>{!detail?<p>공지를 불러오는 중…</p>:<><div className="board-toolbar"><OsanButton onClick={onList}>목록</OsanButton><div>{detail.canEdit&&<OsanButton disabled={!enabled||busy} onClick={()=>setEdit(true)}>수정</OsanButton>}{detail.canDelete&&<OsanButton tone="danger" disabled={!enabled||busy} onClick={()=>{if(window.confirm('공지를 삭제할까요? 내용과 첨부 이력은 보존됩니다.'))void run(async()=>{await deleteNotice(userKey,detail.noticeId);onList();});}}>삭제</OsanButton>}</div></div>
 ...
 {admin&&settings&&<details className="settings"><summary>관리자 설정</summary><div className="settings-inner"><label><input type="checkbox" checked={settings.pinned} disabled={!enabled||busy} onChange={e=>void changeSettings({pinned:e.target.checked})}/>상단 고정</label><label><input type="checkbox" checked={settings.popupEnabled} disabled={!enabled||busy} onChange={e=>void changeSettings({popupEnabled:e.target.checked})}/>팝업 표시</label><OsanButton disabled={!enabled||busy||!settings.popupEnabled} onClick={()=>{if(window.confirm('이미 확인한 사용자에게도 한 번 더 표시할까요?'))void changeSettings({},true);}}>다시 공지</OsanButton><p>팝업은 계정마다 한 번 표시됩니다. 일반 수정으로 다시 표시되지 않습니다.</p></div></details>}
 </>}</>:<><div className="board-toolbar"><span>전체 {list?.totalCount??0}건</span></div>
 <table className="board-table"><thead><tr><th className="number">번호</th><th>제목</th><th className="author">작성자</th><th className="date">등록일</th><th className="file">첨부</th></tr></thead><tbody>{items.map((item,i)=><tr key={item.noticeId} className={item.pinned?'pinned':''} onClick={()=>onOpen(item.noticeId)}><td className="number">{item.pinned?<span className="pin">공지</span>:(list?.totalCount??0)-(page-1)*20-i}</td><td className="title"><button className={item.isRead ? 'notice-title-read' : 'notice-title-unread'} onClick={event=>{event.stopPropagation();onOpen(item.noticeId);}}>{item.title}</button></td><td className="author" title={item.authorDisplayName}>{item.authorDisplayName}</td><td className="date">{new Date(item.createdAtUtc).toLocaleDateString('ko-KR')}</td><td className="file">{item.attachmentCount||'—'}</td></tr>)}</tbody></table>
 {!list?<p>목록을 불러오는 중…</p>:items.length===0?<p>표시할 공지가 없습니다.</p>:null}
 <form className="board-search" onSubmit={e=>{e.preventDefault();setPage(1);setSearch(query.trim());}}><input aria-label="공지 검색" placeholder="제목·작성자 검색" value={query} onChange={e=>setQuery(e.target.value)}/><OsanButton type="submit">검색</OsanButton></form><div className="board-pages"><OsanButton disabled={page===1} onClick={()=>setPage(p=>p-1)}>이전</OsanButton><span>{page} / {Math.max(1,Math.ceil((list?.totalCount??0)/20))}</span><OsanButton disabled={!list||page*20>=list.totalCount} onClick={()=>setPage(p=>p+1)}>다음</OsanButton></div>
 </>}
 </section>;
}
