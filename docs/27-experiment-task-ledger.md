...
- 대표 repository·GitHub `main`: 최초 실험 승격과 2026-08-04 `TASK-EXPERIMENT-PROMOTION-001 Change 002`의 5174 제품·Azure 기준선 통합을 Ready PR·CI로 반영한 canonical 기준선
...
| `DESIGN-000` | reference 기반 CSS semantic token·공통 React primitive, Change 001 black & white wireframe, Change 002 전 부서 입력 흐름 통일, Change 003 다중 업무의 `업무 선택→KPI·프로젝트→단일 프로젝트 입력` 분리·제조/품질 세로 패널 탐색·Pending KPI/프로젝트 목록과 상세 격리, Change 004 PC 평가 기반 UX 보정, Change 005 물류·정산 좁은 입력 header와 검은 표면 글자 대비 보정. Change 006은 독립 Graphite 실험의 공통 시각·구성, 장식용 왼쪽 rail 제거, table density, 클릭형 desktop/mobile 부서 disclosure와 업무 선택 전용 page 삭제를 최신 main에 선택 이식 | [본체](../tasks/design-000-implementation-report.md), [Change 001](../tasks/design-000-change-001-implementation-report.md), [Change 002](../tasks/design-000-change-002-implementation-report.md), [Change 003](../tasks/design-000-change-003-implementation-report.md), [Change 004](../tasks/design-000-change-004-implementation-report.md), [Change 005](../tasks/design-000-change-005-implementation-report.md), [Change 006 계약](../tasks/design-000-change-006.md), [Change 006 구현 보고](../tasks/design-000-change-006-implementation-report.md) | `EXPERIMENT_COMPLETE / USER_VALIDATION_COMPLETE`; Change 006 `MAIN_MERGED / USER_VALIDATION_COMPLETE` |
...
| `TASK-UL891-SET-001` | UL891 세트 사양 version·주문 instance·개별 physical panel, 부분출하·발주 회수·달력월 발행요청. Change 002~008의 기존 흐름에 이어 Change 009에서 사용자 version·code 입력을 제거한 단일 현재 설계, 위치 identity 기반 중복 사양, 활성 위치·패널 projection과 기존 42면 보존을 확정했다 | [최종 기획](41-ul891-panel-set-plan.md), [Change 005](../tasks/ul891-set-001-change-005-implementation-report.md), [Change 006](../tasks/ul891-set-001-change-006-implementation-report.md), [Change 007](../tasks/ul891-set-001-change-007-implementation-report.md), [Change 008](../tasks/ul891-set-001-change-008-implementation-report.md), [Change 009](../tasks/ul891-set-001-change-009-implementation-report.md), [구현 보고서](../tasks/ul891-set-001-implementation-report.md), [검수 체크리스트](../tasks/ul891-set-001-change-009-user-validation-checklist.md) | `EXPERIMENT_COMPLETE / USER_VALIDATION_COMPLETE`; 통합 후보 자동 검증 완료·사용자 재검수 대기 |
...
| --- | --- | --- | --- |
| `TASK-UL891-PRODUCTION-PLAN-001` | Ul891Set+LinkedV1의 실제 실물 세트별 생산계획에 Change 002~008의 전체 기본계획·빈 세트 보호·명시적 덮어쓰기·후속 세트 상속, 실적 연결 한 행 편집, 일정표 색·날짜선·테두리와 담당자 요약을 추가 | [최종 기획](44-ul891-set-production-plan.md), [Change 008 구현 보고](../tasks/ul891-production-plan-001-change-008-implementation-report.md), [통합 구현 보고](../tasks/experiment-promotion-001-implementation-report.md), [통합 검수 체크리스트](../tasks/experiment-promotion-001-change-003-user-validation-checklist.md) | `EXPERIMENT_COMPLETE / USER_VALIDATION_COMPLETE`; 통합 후보 Backend `482/482`, Frontend `175/175`, Full-Stack `56/56` 완료·사용자 재검수 대기 |
...
DESIGN-000 Change 006은 독립 Graphite 실험의 Frontend source·test만 최신 main 기반 승격 branch에 이식하고 실험 문서·screenshot·Git history를 제외했다. 장식용 왼쪽 강조 rail을 제거하고 표 밀도를 통일했으며, 왼쪽 메뉴는 desktop·390px 모두 처음 접힌 부서 행을 눌러 한 부서만 펼치고 child로 실제 업무에 직접 이동한다. `DepartmentWorkHub.tsx`와 업무 선택 전용 page는 삭제했고 legacy 부서 root만 첫 실제 업무 replace redirect로 보존했다. Frontend lint error/warning 0, typecheck, unit `170/170`, production build와 격리 mock E2E `4/4`를 통과했다. 사용자는 2026-08-01 검수를 완료했고 승격 commit `f68169d`를 local `main` merge `f1f94ed`로 반영했다. Backend·DB·migration·배포·원격 게시에는 변경이 없다.
...
대표 repo·`main` 최초 승격은 `TASK-EXPERIMENT-PROMOTION-001`에서 experiment commit 계보, migration `0001`~`0064`, 전체 자동 회귀, fresh 공식 UAT handover와 rollback을 검증해 완료했다. Change 002는 2026-08-04 5174 local 제품 기준선과 Azure Change 003~005 원격 기준선을 별도 merge commit으로 통합하고 Frontend·Backend·Full-Stack·Azure artifact·실제 5174 smoke와 Ready PR CI를 다시 확인했다. direct push와 squash 없이 두 계보를 보존하며, 5175 UL891 변경과 실제 통합 image 재배포는 포함하지 않는다.

Change 003은 5175에서 사용자 검수한 UL891 Change들만 통합 `origin/main` 기준선에 선택 이식했다. migration `0068`, Backend `482/482`, Frontend `175/175`, Mock `4/4`, isolated Full-Stack `56/56`을 통과했으며 Graphite·Azure·Pending 복구 계보를 유지한다. Change 004는 42개 활성 패널의 현재 화면 순번을 `1..42`로 분리하면서 영구 code `P52`와 이력을 보존했고, Frontend `175/175`, 집중 Full-Stack `1/1`, 실제 5191 제조·품질·물류 각 42행을 확인했다. 사용자는 구현·Git 게시·Ready PR CI 뒤 main 병합을 승인했으며 Persistent UAT·Azure image 재배포는 다음 Gate로 남는다.
