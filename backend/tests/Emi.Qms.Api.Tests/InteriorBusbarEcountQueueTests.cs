...
public sealed class InteriorBusbarEcountQueueTests
{
    ...
    public async Task LegacyCreatorRecoveryRequiresUniqueCreationEvidence()
    {
        ...
        var sql=await File.ReadAllTextAsync(Path.Combine(root,"database/migrations/0112_interior_busbar_project_creator.sql"),TestContext.Current.CancellationToken);
    ...
    public async Task MigrationPreservesLegacySalesAndDoesNotBackfillShipments()
    {
        ...
        var root = AppContext.BaseDirectory;
        while (!Directory.Exists(Path.Combine(root,"database","migrations"))) root = Directory.GetParent(root)!.FullName;
        await InteriorBusbarStore.Exec(c, await File.ReadAllTextAsync(Path.Combine(root,"database/migrations/0113_interior_busbar_shipment_sales.sql"), TestContext.Current.CancellationToken));
