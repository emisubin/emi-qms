...
| 5.1 | DESIGN-000 Design foundation | P2_REMEDIATION | Change 006 User Validation Complete / Local Main Merge Approved / Remote Unpublished | Change 001~005 완료 foundation을 재구현하지 않고 독립 Graphite 실험의 공통 시각·구성, 장식용 왼쪽 rail 제거, table density, 클릭형 부서 disclosure와 업무 선택 전용 page 삭제를 최신 main fixed allowlist로 승격. Frontend lint·typecheck·unit `170/170`·build·격리 mock E2E `4/4` 완료 | 사용자 Graphite 구현·검수·local merge 승인, 최신 main 기반 clean promotion branch, 명시적 Azure Gate 재정렬 승인 | Backend·DB·migration·배포·실제 provider 제외. Figma publish와 App route split은 범위 밖. Remote push·PR 미승인 | Yes | local main merge 완료 기록 → TASK-AZURE-DEPLOY-001 비용 Gate·`PRE_TRAFFIC_GATE` 복귀 |
...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Main Merge Approved / Cost Gate Pending | Front Door Standard·Container Apps Consumption·PostgreSQL B2s·ACR Basic·Azure Files·Key Vault·Log Analytics provider-specific 코드, Teams manifest와 자동 검증 완료 | TASK-AZURE-PILOT-001 main merge, 사용자 20일 구성·Change 002 merge 승인 | 비용 관련 Azure resource 생성·image push·restore·DNS·traffic은 사용자 직접 실행 | Yes | main 병합 → 사용자 Azure 실행 → pre-traffic 복구·보안·provider 검수 |
...
- 상태/다음 순서: 비용 없는 local implementation·자동 검증·commit 완료, main merge 사용자 승인 / Azure resource 생성 비용 Gate 대기
...
| 2026-08-01 | DESIGN-000 Change 006 사용자 검수를 완료하고 local `main` 병합을 승인하되 원격 push·PR·배포는 보류 | 업무 선택 전용 page 삭제, 부서 행 click disclosure, 장식용 왼쪽 강조 rail 제거와 Graphite 표 밀도를 제품 기준선에 반영하고 다음 실행 순서를 TASK-AZURE-DEPLOY-001 비용 Gate로 복귀시키기 위함 | 23장~25장, 27-experiment-task-ledger, DESIGN-000 Change 006 |
