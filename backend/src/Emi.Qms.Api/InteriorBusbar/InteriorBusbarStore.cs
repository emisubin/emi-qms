...
;
        foreach (var (key, sql) in new (string, string)[]{
            ...
            ("ecountEmployees","select u.id user_id,u.display_name,m.employee_code from qms_users u left join busbar_ecount_employees m on m.user_id=u.id order by u.display_name,u.id"),("projects",ProjectQuery),("plans","select p.*,(select count(*) from busbar_products x where x.plan_id=p.id and x.status='Complete') actual_quantity from busbar_plans p order by plan_date desc"),
...
;
 ...
 : await One(c, table, id);
    ...
    public Task<object> CommercialPreview(Guid projectId) => ReadSnapshot<object>(async c =>
    {
        ...
        var employeeCode = await EcountEmployeeCode(c, project);
        if (string.IsNullOrWhiteSpace(employeeCode)) missing.Add("프로젝트 최초 등록자의 이카운트 담당자 연결");
        var supply = price * quantity;
        var vat = supply is null ? (decimal?)null : supply.Value * 0.1m;
        return new { projectId, registeredByName = project["registeredByName"], employeeCode, workOrderNumber = project["customerJobNumber"], purchaseOrderNumber = "",
    ...
    private static async Task<Guid> SaveProject(NpgsqlConnection c, BusbarProjectRequest r, Guid actor)
    {
       ...
       ;
        ...
        var creatorName = (await One(c, "qms_users", actor))["displayName"] as string;
