...
| 3.3A | TASK-PRODUCTION-CONTROL-001 Item별 생산계획·자동 실적·가로 막대 일정 | NEW_FEATURE / BUGFIX | Change 010 Local Verified / Publish In Progress | 본체와 Change 002~009의 원격 main 반영·사용자 검수 완료. Change 010은 같은 현재 양식의 후행 재선택이 빠른 편집 진입을 취소하던 P2를 최소 보정하고 집중 `3/3`·연속 `10/10`·Frontend 전체 `177/177`·lint·typecheck·build를 통과했으며 표준 CI 대기 | TASK-ADMIN-002·생산계획·구매·자재·제조·품질·물류 원본 데이터 | Change 010 원격 main·운영 Azure image 미반영. 기존 프로젝트는 Legacy/snapshot 유지 | Yes | Change 010 PR·main CI → 문서 상태 동기화. 운영 재배포는 별도 승인 |
