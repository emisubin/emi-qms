...
| 3.3D | TASK-ADMIN-003 사용자 부서·역할·부서장 연결 보정 | P2_REMEDIATION | User Validation Complete / Publication and Deployment Approved | 운영에 누락된 표준 부서를 10개 한글명으로 보강하고 부서 기본 역할 자동 지정, 복수 부서장과 기존 양식관리 binding·audit 동기화를 구현. Backend `493/493`, Frontend `190/190`, fresh/existing migration과 격리 browser `1/1`, 사용자 직접 검수 통과 | TASK-ADMIN-001 사용자·부서 관리, TASK-ADMIN-002 양식관리 binding, migration `0071` | Git 게시·CI·main 병합·운영 `0072` 적용 실행 대기 | Yes | TASK-TEAMS-PWA-001 Change 011과 통합 게시 → CI → main 병합 → `0072`·Backend·Frontend 운영 교체 |
...
| 3.3C | TASK-TEAMS-PWA-001 — Teams 실행 화면·PWA 설치·브랜드 통일 | BUGFIX / P2_REMEDIATION | Change 011 User Validation Complete / Publication and Deployment Approved | Change 001~003·007·009·010 운영 rollout 완료. Change 011은 새 웹 제품 logo, 로그인 보안 안내와 오산·청주 회사 footer를 local 구현하고 Frontend `190/190`, auth-shell `12/12`, 사용자 직접 검수를 통과했으며 PWA·Teams icon을 보존 | Azure Easy Auth·Teams Activity 10종·기존 웹 MSAL·PWA icon·TASK-DESIGN-LOGIN-001 Change 010·TASK-ADMIN-003 통합 branch | Git 게시·CI·main 병합·운영 배포 실행 대기. Web Push는 별도 NEW_FEATURE | Yes | TASK-ADMIN-003과 통합 게시 → CI → main 병합 → 운영 공개배포 |
...
- 다음 Gate: Change 020 최신 main 운영 image 동기화를 완료했다. 사용자 실제 PC·iPhone·Android 지정 logo 육안 검수와 20일 운영 관찰을 유지한다.
