...
### 홈 카드 목적지 보정·프로젝트 디자인 대조

사용자 지시에 따라 홈 카드 선택은 기존 프로젝트 상세(`/projects/{id}`)로 이동하도록 보정했다. 진행 현황 행은 기존 진행 상세 연결을 유지한다. 위 홈 카드→진행 상세 기록은 이전 계약이다. 카드 접근성 이름도 홈에서는 ‘프로젝트 상세 열기’로 구분했다.

실제 검수 화면에서 홈→프로젝트 상세 이동을 확인했다. 프로젝트 목록·상세는 PC와390px 모바일에서 기존 공통 목록/상세 컴포넌트를 쓰는 반면 홈/진행에는 별도 Figma 스타일이 적용되어 있다. 확인된 차이는 모바일 EMI PMS 전체 로고/EMI PROJECT·페이지명/두 줄 헤더와 홈의 EMI 단일 로고·한 줄 헤더, 제목·여백·검색·상태탭·카드 스타일, 상세의 파란 진행 막대와 홈/진행의 빨간 막대다. 이번 요청의 디자인 부분은 차이 확인까지 수행했으며 목록·상세 디자인 자체는 변경하지 않았다. 보정 시 기존 TASK-OSAN-PROJECT-001 후속 범위에서 업무 필드·생성·대상 연결을 보존해야 한다.

이 연결은 프런트엔드만 변경하여 기존 실행 API에서도 확인 가능하다. 이전 홈 납기 API 재시작 정책 차단은 해결되지 않았다. 서버 재시작을 다시 시도하지 않았다.

연결 보정 검증: Dashboard 8/8 및 TypeScript 포함 build 통과, 기존 bundle 경고 유지. 실제 홈 클릭의 URL과 프로젝트 상세 데이터 표시를 확인했다. 해당 연결/문서만 로컬 커밋, 원격 반영·배포 없음.

## 2026-09-09 기본 권한 전환 후 검수 API 갱신 완료

사용자가 앱 권한을 기본 권한으로 변경한 뒤 정식 실행 승인 요청이 성공했다. 기존 PID77619를 종료하고 같은 5096에 최신 Release API를 실행했다(실행 세션4339). 기존 preview 환경을 사용하고 seed·startup migration 비활성화를 유지했다. health ready 200, home/progress 조회 각각200, INVALID view 400으로 신규 홈 조회 코드 적용을 확인했다. 재시작 전후 샘플 프로젝트 진행·사진 메타데이터 canonical SHA-256이 동일하다. 앞선 정책 차단은 해소됐으며 최종 사용자 검수·후속 최종 후보 회귀는 남아 있다. 운영 DB·원격 반영·배포 없음.
