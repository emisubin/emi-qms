...
| 3.3L | TASK-AUDIT-001 — 로그인·데이터 변경 감사 원장 | APPROVED_FEATURE_IMPLEMENTATION / POLICY_DECISION / UAT_RUNTIME | Main Merged / Azure Released / User Validation Pending | 대화형 로그인·명시적 로그아웃, 성공 데이터 변경, validation·충돌 실패 시도와 기존 권한 거부를 관리자 전체 감사 이력에서 통합 조회하고 선택 Excel로 내보낸다. Change 002에서 endpoint 분류 `185/185`(포함 156·제외 29), relation 분류 `145/145`(추적 94·제외 51), 중앙 PostgreSQL transaction 대표 검증과 local 동시 mutation `50/50`을 v1 acceptance로 승인하고 409/412를 `Conflict`로 확정했다. Audit append 실패의 업무 rollback과 잔존 `Duplicate` 제거를 보정한 뒤 Backend `567/567`, migration `59/59`, Frontend `235/235`, desktop·375px visual과 독립 재검증 PASS·Open P0/P1/P2 `0/0/0`을 확인했다. PR `#111`을 squash merge한 운영 source exact main SHA `6713e5974ad5262d87d7cc2332b27486d2487ccd`의 main CI `33137735821`과 Azure release `33137792491`이 통과했으며 migration·Backend·Frontend·공개 보안 검사가 모두 `PASS`다. | 기존 Entra/MSAL identity, `authorization_audit_events`, `data_export_events`, `Audit.Read.All`, migration `0082` | 156-route 1:1 matrix는 모든 route의 업무 규칙을 증명하지 않으므로 v1 acceptance에서 제외. 실제 Microsoft 로그인·Azure 50명 부하·local Persistent UAT handover·실제 외부 알림 시험 발송 제외 | Yes | 공개 감사 화면 사용자 검수 → 감사 row 증가량·DB connection·p95 latency aggregate 운영 관찰 |
...
| 48 | 전체 field-level audit 확장 | Main Merged / Azure Released / 사용자 검수 대기 | 사용자/운영 | TASK-AUDIT-001 | migration `0083`이 대화형 login/logout, 포함 mutation `156`, 업무 relation `94`, 실패 저장 시도, 기존 권한 거부 통합 조회와 선택 Excel을 구현했다. exact catalog+중앙 transaction 대표 검증을 v1 acceptance로 승인했고 모든 409를 `Conflict`로 기록한다. PR `#111`, exact main `6713e5974ad5262d87d7cc2332b27486d2487ccd`, Azure release `33137792491` 통과. 독립 재검증 PASS·Open P0/P1/P2 `0/0/0`; 과거 소급, Entra 단계 실패, worker/provider, archive/purge는 제외 |
