...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 005 Main Merged / Change 006 Local Validated / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `67 Exact`·PITR restore와 세 workload readiness 완료. 원격 main `aac5f27`에 Graphite·UL891 `0068`·프로젝트 진행률 보정 통합. Change 006에서 제공 EMI 원본 기반 Teams `1.0.2` package·PWA manifest/icon 검증 완료 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~005 main merge, 원격 main `aac5f27`, 사용자 Teams·PWA asset 승인 | Front Door domain validation·TLS·인증·actual provider, Change 006 게시, 최신 main image·migration `0068` handover 미완료 | Yes | Change 006 게시 → 최신 main image·migration `0068` 재배포 → Edge·TLS·provider 검수 |
...
- 상태/다음 순서: Change 003~005와 최신 제품 통합 원격 main `aac5f27`, Foundation·secret-scope RBAC·image 게시·DB role bootstrap·migration `67 Exact`·PITR restore rehearsal 완료 / 세 Container Apps provisioning `Succeeded`·latest revision healthy, Backend·ClamAV internal ingress·Frontend external ingress·origin health `200`·업무 route `403` / DNS TXT·CNAME 확인, Front Door domain validation `Pending`·deployment `NotStarted`, public traffic·external notification 비활성 / Change 006 Teams·PWA 브랜드 자산 로컬 검증 완료, 게시 뒤 최신 image·migration `0068` handover가 다음 Gate
