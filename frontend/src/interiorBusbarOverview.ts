...
export function busbarOverview(data: BusbarWorkspace, date: string) {
  ...
  const projects = data.projects.filter(p => !p.isDeleted && p.requestedQuantity > p.shippedQuantity && p.dueDate.slice(0, 10) <= through)
  ...
  const families = data.productFamilies.filter(f => !f.isDeleted).map(f => {
    const plans = data.plans.filter(p => !p.isDeleted && p.productFamilyId === f.id);
    ...
    if (remainingProduction > 0) {
      const newestBom = data.boms.filter(b => b.productFamilyId === f.id).sort((a,b) => b.version-a.version)[0];
      const bom = newestBom?.isDeleted ? undefined : newestBom;
    ...
    const deliveries = projects.filter(p => !p.isDeleted && p.productFamilyId === f.id).reduce((n,p) => n+p.requestedQuantity-p.shippedQuantity,0);
  ...
  const materials = data.materials.filter(m => !m.isDeleted).map(m => {
