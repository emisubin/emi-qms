...
, f.Actor));
 ...
 = Guid.NewGuid();
        public static async Task<Fixture> Create(InteriorBusbarPublicationOptions? publicationOptions = null, bool applyCommercialMigration = true, bool applyShipmentMigration = true, bool applyTraceMigration = true, bool applyDeletionMigration = true, bool applyPhotoMigration = true)
        {
...
;
            await using (var c = new NpgsqlConnection(f.Connection))
            {
                ...
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0106_interior_busbar.sql")), c).ExecuteNonQueryAsync();
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0107_interior_busbar_planned_products.sql")), c).ExecuteNonQueryAsync();
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0108_interior_busbar_product_qr.sql")), c).ExecuteNonQueryAsync();
                if (applyCommercialMigration) await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0109_interior_busbar_commercial_data.sql")), c).ExecuteNonQueryAsync();
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0110_interior_busbar_ecount_queue.sql")), c).ExecuteNonQueryAsync();
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0111_interior_busbar_ecount_runtime.sql")), c).ExecuteNonQueryAsync();
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0112_interior_busbar_project_creator.sql")), c).ExecuteNonQueryAsync();
                if (applyShipmentMigration) {
                    await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0113_interior_busbar_shipment_sales.sql")), c).ExecuteNonQueryAsync();
                    if (applyTraceMigration) await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0114_interior_busbar_panel_trace.sql")), c).ExecuteNonQueryAsync();
                }
                await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0115_interior_busbar_master_access.sql")), c).ExecuteNonQueryAsync();
                if (applyDeletionMigration)
                {
                    await new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0117_interior_busbar_soft_delete.sql")), c).ExecuteNonQueryAsync();
