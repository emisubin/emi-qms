...
| 3.3D | TASK-ADMIN-003 사용자 부서·역할·부서장 연결 보정 | POLICY_DECISION / P2_REMEDIATION | Change 002 User Validation Complete / Publication Approved | Change 001의 표준 10개 한글 부서·자동 역할·복수 부서장 운영 rollout을 유지하고, Change 002에서 품질 부서장=품질 양식·구매품 IQC·LQC 운영, 생산관리 부서장=생산계획 연결·Item별 제조 양식으로 scope를 재정렬했다. 제조 부서장·일반 품질 사용자의 양식 관리 노출·mutation은 차단하고 migration `0078`·자동 검증을 완료 | TASK-ADMIN-001 사용자·부서 관리, TASK-ADMIN-002 양식관리 binding, TASK-PRODUCTION-CONTROL-001, TASK-QUALITY-OPERATING-MODEL-001, migration `0077` | 없음 | Yes | 단일 통합 PR `CI Gate` → migration `0078` → Backend·Frontend 공개배포 |
...
| 3.3A | TASK-PRODUCTION-CONTROL-001 Item별 생산계획·자동 실적·가로 막대 일정 | NEW_FEATURE / BUGFIX | Change 011 Main Merged / Azure Released / User Validation Complete | 모든 프로젝트의 전용 기간·실적 연결·9열 표·단일 2중 막대 일정과 행 삭제 순번 보정을 구현했다. PR #101 필수 CI 뒤 exact main SHA `8b19483e40655ce99c13cb470217ccddf444b1c0`로 병합하고 Azure release `31774236257`에서 Backend·Frontend와 public security smoke를 완료했다. | TASK-ADMIN-002·생산계획·구매·자재·제조·품질·물류 원본 데이터 | 기존 프로젝트는 Legacy 행·실행 이력을 유지하며 프로젝트별 override만 저장 | Yes | 완료 scope 재구현 금지; 운영 관찰 |
...
| 3.3F | TASK-PWA-PUSH-001 — 인앱 연동 PWA 기기 푸시 | APPROVED_FEATURE_IMPLEMENTATION / UAT_RUNTIME | Change 002 User Validation Complete / Publication Approved | 인앱 가시성 기준 기기별 구독·delivery, 현재/전체 기기 해제와 최소 Service Worker를 migration `0074`와 함께 운영 적용했다. 운영 VAPID는 Key Vault에 보관하고 `Enabled=true`·`DryRun=false`에서 실제 iPhone·Android 수신과 알림 상세 이동을 검수했다. Change 002는 현재 운영 설정과 secret-scope 권한을 추적 Azure 배포 정의에 동기화한다. | TASK-TEAMS-PWA-001 설치 경험·TASK-NOTIFY-004 delivery worker·현재 인앱 알림 수신자 정책 | 직원별 설치·알림 허용은 자율. 운영 secret 원문·사용자별 구독·과거 알림 소급 발송은 제외 | Yes | 단일 통합 PR `CI Gate` → Azure 공개배포 → 운영 설정 보존 확인 |
...
| 4.7 | TASK-QUALITY-OPERATING-MODEL-001 — 구매품 구분 기반 IQC / Change 004 Item별 LQC / Change 005 구분별 IQC 방식·양식 | APPROVED_FEATURE_IMPLEMENTATION | Change 006 Main Merged / Azure Released / User Validation Complete | migration `0070`·`0071`, Item별 LQC 생성 snapshot과 구분별 IQC 검사 없음·스캔형·상세형을 PR #91로 원격 `main`에 병합했다. 최신 PR head의 변경 분류·Backend·Frontend·Full-Stack E2E·`CI Gate`가 모두 통과했고, main SHA `064454d1`의 Azure release `31409582129`에서 migration→Backend→Frontend 교체와 공개 health·익명 인증 차단을 완료했다 | TASK-009A·TASK-ADMIN-002·TASK-012A·WORKFLOW-CONTINUITY-001·PRODUCTION-CONTROL-001·TASK-CI-COST-001 기존 계약 | 기존 확정 IQC/LQC 증빙·Pending 삭제 금지. 실제 Teams·메일 발송과 Web Push는 이 Task 범위 밖 | Yes | 완료 scope 재구현 금지; 운영 관찰 |
...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 024 Main Merged / Azure Released | PR #101 필수 CI 뒤 exact main SHA `8b19483e40655ce99c13cb470217ccddf444b1c0`로 Azure release `31774236257`을 실행했다. 변경 분류는 Backend·Frontend `true`, migration `false`였고 두 앱 ready와 public security smoke가 통과했다. | TASK-AZURE-PILOT-001, Change 018 승인형 workflow, TASK-CI-COST-001, TASK-PRODUCTION-CONTROL-001 Change 011, TASK-PROJECT-ASSIGNEE-DELEGATION-001 | 기존 운영 인증·PWA Web Push·Teams·메일 설정과 데이터 보존. Node runtime·CLI version parse P3 경고와 20일 운영 관찰 유지 | Yes | 운영 관찰; P3 runner 경고는 별도 유지보수 |
