...
현재 작업은 [PROJECT Change 008](../tasks/osan-project-001-change-008.md)의 오산 단계 기록·반려·초기화·알림 통합이며, 구현·직접 검증과 최종 후보 전체 회귀 및 사용자 검수를 마쳤다. 사용자가 원격 main 병합·공개배포를 승인했고 required CI와 main 병합을 완료했다. 공개배포는 이력 전환 중 오산 저장 임시제한에 대한 자동 승인 심사 거부로 대기 중이다. 상세 승인 경계와 재개 절차는 Change 008의 최신 기록을 따른다. 회귀에서 발견한 제조 양식 조회 경쟁을 보정했다. 관리자 페이지는 제외한다. 아래 사진 보정 및 개발 큐는 이전 실행 이력으로 보존한다.
