...
public static class G2OperationsEndpointExtensions
{
    public static IEndpointRouteBuilder MapG2OperationsEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPut("/operations/{date}", async (DateOnly date, SaveG2OperationsRequest request, G2OperationsStore store, ClaimsPrincipal user, CancellationToken token) =>
        {
            ...
            if (changes.Count == 0) return Validation("request", "저장할 생산, 수리, 납품 또는 불량 항목을 하나 이상 입력해 주세요.");
            if (changes.Any(c => (c.MetricCode is G2MetricCodes.MorningProduction or G2MetricCodes.AfternoonProduction or G2MetricCodes.MorningRepair or G2MetricCodes.AfternoonRepair or G2MetricCodes.Defect) && !Has(user, QmsPermissions.G2ProductionUpdate))
    ...
    {
        var result = new List<G2MetricChange>(); Add(result, G2MetricCodes.MorningProduction, request.MorningProduction); Add(result, G2MetricCodes.AfternoonProduction, request.AfternoonProduction); Add(result, G2MetricCodes.MorningRepair, request.MorningRepair); Add(result, G2MetricCodes.AfternoonRepair, request.AfternoonRepair); Add(result, G2MetricCodes.Delivery, request.Delivery); Add(result, G2MetricCodes.Defect, request.Defect); return result;
    }
