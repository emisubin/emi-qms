...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 005 Main Merged / Integrated Source Ready / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `67 Exact`·PITR restore와 세 workload latest revision readiness 완료. 제품 기준선·UL891 Change 003~004를 PR #65 merge commit `79b90b8`으로 원격 main에 통합 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~005 main merge, TASK-EXPERIMENT-PROMOTION-001 Change 002~004 | Front Door route 0, DNS·TLS·원본 업무 route 차단·인증·실제 provider·통합 source image 재배포와 migration `0068` handover 미완료 | Yes | 통합 source image·migration `0068` 재배포 → Edge·DNS·TLS·provider 검수 |
...
- 다음 Gate: 원격 main merge commit `79b90b8` 기준으로 Backend·Frontend image와 revision을 갱신하고 migration `0068`을 승인된 UAT handover 절차로 적용한다. origin direct 차단을 확인한 뒤에만 Edge·DNS·TLS·Entra 검증으로 진행한다.
