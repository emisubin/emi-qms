...
| 2 | [TASK-OSAN-ACCESS-001](../tasks/osan-access-001.md) — 사용자·사업부 전환 | IMPLEMENTED_AWAITING_BATCHED_USER_VALIDATION | 1 | 마지막 일괄 사용자 검수 대기 — Task 3 선행 차단 아님 |
| 3 | [TASK-OSAN-PROJECT-001](../tasks/osan-project-001.md) — 프로젝트 등록 | IMPLEMENTED_AWAITING_USER_VALIDATION | 1, 2 | Change 005 목록 전체 composition·paired 화면 확인 → 마지막 일괄 사용자 검수 |
...
Task 2는 local commit `2e29938f754f3d95444df2b341a921cfd1fca43f`로 고정했다. Task 3 Change 001은 Backend 582/582, Frontend 291/291, mock browser 1/1과 실제 3개 DB full-stack 1/1을 통과했다. Change 002가 만든 별도 오산 UI는 사용자 검수 의견에 따라 Change 003에서 제거했다. Change 004는 청주와 오산의 목록·상세 요약·부서 현황을 실제 공용 React 컴포넌트로 통합하고 오산 차이를 데이터 매핑·단일 `진행 관리` tab·현재 단계와 완료 수/7·금지 action 제외로 제한했다. Frontend 292/292, 집중 97/97, paired mock browser 1/1과 1440×900·390×844의 양쪽 화면 8개 직접 비교, fresh GPT-6 High 최종 `PASS / GO`, open P0/P1/P2/P3 `0/0/0/0`을 확인했다. Change 004 화면은 local synthetic 검수 서버에서 사용자 확인 대기이며 마지막 오산 일괄 검수 상태도 유지한다. 다음 제품 Gate는 Task 4 구현 승인이다. Push·PR·merge·Persistent UAT·provider·운영 적용은 별도 승인이다. Task 1의 legacy 검사 자원 2개 정리는 사용자가 직접 수행할 P3이며 다음 제품 Task를 막지 않는다.
...
| 99 | 오산 시범 운영 기획과 7단계 개발 로드맵 | Business Planning Approved / Task 1 USER_VALIDATION_COMPLETE / Task 2 IMPLEMENTED_AWAITING_BATCHED_USER_VALIDATION / Task 3 IMPLEMENTED_AWAITING_USER_VALIDATION | 사용자/개발/운영 | [TASK-OSAN-PILOT-001](../tasks/osan-pilot-001.md) | Task 1 DB 분리 기반과 사용자 검수 완료. Task 2는 자동 검증·독립 검증과 local commit 완료, Task 3은 등록·대상·7단계 snapshot 및 Change 005 목록 전체 composition·paired 시각 검증 GO 완료다. Change 005 현재 화면 검수, 마지막 일괄 사용자 검수와 후속 Task 4~6이 남았으며 다음 제품 Gate는 Task 4 구현 승인이다. Push·PR·merge·운영 적용은 별도 승인이다. |
