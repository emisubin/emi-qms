...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 018 Approval-Gated Release / Source Merged / Run Pending | Change 015~017은 PR #74, Change 018 source는 PR #76으로 원격 `main`에 게시했다. Merge SHA Full-Stack P2는 TASK-E2E-FULL-SUITE-001 Change 012 PR #77과 merge SHA CI `3/3`으로 해소했다. 수동 release는 최신 `main` full SHA와 두 명시 확인 뒤 immutable image → migration → Backend → Frontend → 공개 보안 검사 순서로 실행한다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~018 source, PR #77 merge SHA CI | 실제 운영 release 미실행. 에스컬레이션·Teams SSO는 후속 기획 | Yes | 별도 명시 운영 release → 실행 결과 검수 → Teams SSO·새 manifest 기획 |
...
- 상태/다음 순서: Change 015~017은 PR #74로 원격 `main`에 게시·CI 완료 / Change 018 승인형 GitHub 운영 release source·synthetic rollback 검증·GitHub Environment variable `4/4`·OIDC exact resource 역할 구성·PR #76 원격 `main` 병합 완료 / merge SHA Full-Stack P2는 TASK-E2E-FULL-SUITE-001 Change 012 PR #77 병합과 merge SHA CI `3/3`으로 해소 / 별도 운영 release 실행 대기
...
- 다음 Gate: Change 018 실제 운영 release를 별도 명시 실행·검수한 뒤 Teams SSO·새 manifest 기획으로 진행한다.
