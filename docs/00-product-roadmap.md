...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 018 Approval-Gated Release / Source Implementation | Change 015~017은 PR #74로 원격 `main`에 게시했다. Change 018에서 자동 `push` 없이 최신 `main` full SHA와 두 명시 확인을 요구하고, immutable image 게시 → migration 성공 → Backend → Frontend → 공개 보안 검사 순서와 실패 rollback을 구현·mock 검증했다. GitHub variable `4/4`와 OIDC exact resource 역할도 구성했다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~017 기준선, PR #74 merge | Change 018 게시와 별도 운영 release 미실행. 에스컬레이션·Teams SSO는 후속 기획 | Yes | Change 018 원격 `main` 게시 → 별도 명시 운영 release → Teams SSO·새 manifest 기획 |
