...
2026-09-25 후속 사용성 보정과 PC·모바일 메뉴 아이콘 통일은 사용자 검수·필수 CI 후 PR154로 원격 main에 병합하고 공개배포를 완료했다(main8d8ad05). 이번 버그 수정은 사용자 예외 지시에 따라 영구 공지 없이 업데이트 팝업만 안내했고, 09:54 KST 저장을 재개했다. 변경·검증·배포 근거와 한계는 [OSAN-UX Change001 제20-24절](../tasks/osan-ux-001-change-001.md#20-24-후속-버그-수정-병합공개배포-완료-2026-09-25-kst)을 따른다.

[OSAN-UX Change001](../tasks/osan-ux-001-change-001.md)은 사용자 검수·필수 CI를 마치고 PR153으로 원격 main에 병합, 2026-09-25 공개배포를 완료했다(main6c3f5d5). 최초 도입 점검과 저장 제한을 종료했고 현재 정상 이용 가능하다. 실제 검증·전환 중 보정 및 남은 자동배포 계정 권한은 해당 Task 제20-20절을 따른다.
...
현재 우선 작업은 [Gate 완료·공정 진행 요청·관리자 공통 알림 설정](../tasks/osan-progress-001-change-001.md#2026-09-16-후속-구현-gate-및-공정-진행-요청)이다. 신규 수량 1 고정, 용어·납품완료 표시를 함께 구현한다. 로컬 구현·직접 검증 후 사용자 검수를 진행하며, 이번 변경의 원격 병합·공개배포는 아직 승인·실행하지 않았다. 아래 내용은 이전 배포 이력이다.

[진행단계 이상 처리·오산 개인 알림 설정](../tasks/osan-progress-001-change-001.md)을 확정한 정책·시안으로 구현·직접 검증·사용자 검수·필수 CI를 마치고 PR #144로 원격 main 병합 및 공개배포를 완료했다. 인테리어 부스바 변경은 제외했다. 기존 상세 화면의 버튼/입력만 추가하고, 스텝퍼 표시와 개인 메일·푸시 설정을 함께 연결한다. 이 기록이 아래 과거 진행 순서의 해당 업무 정책을 갱신한다. 검증·검수·로컬 커밋·게시 상태는 해당 change에서 관리한다. 원격 병합·배포는 별도 승인 범위다.


현재 후속 작업: [PROJECT Change 008](../tasks/osan-project-001-change-008.md)의 안내 사진·납기 HOLD·모바일 단계 진입·디자인/QR 보정과 [PHOTO Change 002](../tasks/osan-photo-001-change-002.md)의 40MiB·HDR JPEG·HEIC 원본 업로드 수정을 통합해 사용자 승인 후 전체 검증·원격 main 병합·공개배포를 완료했다. 실제 아이폰 QR 및 사용자 HEIC 실기기 확인은 남은 검수 항목이다. 배포 근거는 Change008 최신 기록을 따른다.

후속 모바일 탐색·앱 내 QR 스캔·막대 위 7단계 스텝퍼·W/O 패널 연결은 사용자 승인 후 required CI·원격 main 병합·공개배포를 완료했다. 결과와 검증 근거는 [Change 008](../tasks/osan-project-001-change-008.md)의 2026-09-13 공개배포 기록을 따른다. 남은 확인은 실제 아이폰 PWA의 카메라 권한·패널 QR 인식이다. 공정별 이상/특이사항 알림은 이번 변경에서 제외한다.
...
현재 작업은 [PROJECT Change 008](../tasks/osan-project-001-change-008.md)의 오산 단계 기록·반려·초기화·알림 통합이며, 구현·직접 검증과 최종 후보 전체 회귀 및 사용자 검수를 마쳤다. 사용자가 원격 main 병합·공개배포를 승인했고 required CI와 main 병합을 완료했다. 추가 운영 승인 후 공개배포와 임시 저장 제한 해제까지 완료했다. 배포 및 기존 데이터 조회 확인의 근거·한계는 Change 008의 최신 기록을 따른다. 회귀에서 발견한 제조 양식 조회 경쟁을 보정했다. 관리자 페이지는 제외한다. 아래 사진 보정 및 개발 큐는 이전 실행 이력으로 보존한다. 후속 오산 PWA 연결 보정도 사용자 승인 후 required CI·main 병합·공개배포를 완료했다. 기기 등록·권한 허용 및 실제 수신 확인은 같은 Change 008의 최신 기록을 따른다.
...
- [TASK-INTERIOR-BUSBAR-001](../tasks/interior-busbar-001.md): 2026-09-09 사용자 명시 승인으로 청주 인테리어 부스바 신설 구현. 기존 오산 순서는 유지하며 전용 worktree에서 진행한다. 이카운트 발주 연동은 사용자 요청으로 보류한다. 주문·판매 검수 완료와 남은 권한·현장 검수·운영 준비는 해당 Task의 최신 상태를 따른다.
