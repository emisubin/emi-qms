...
### Busbar photo compatibility correction before PR149 release — 2026-09-17
User reported the former Osan location/device-metadata and JPEG upload errors in Busbar and explicitly requested all related image errors be checked and aligned with current Osan. This joins the already approved deletion merge/deployment; protected CI remains required. Preserve two separate front/back slots, actual worker attribution, immutable completed evidence, production/stock transaction and existing business-unit permissions.
Implemented: reuse Osan JPEG/PNG/HEIC validation and post-malware-scan metadata sanitization; preserve encoded originals and correct MIME, provide HEIC display derivative, maintain WebP compatibility, apply 40MiB combined front/back limit. Additive0118 records history MIME without modifying prior photo bytes. Frontend accepts mobile HEIC/HEIF and provides file/size guidance with compatible saved-photo previews. No production data or ERP changes during tests.
The initial deletion-only PR CI frontend exposed two obsolete assertions forbidding the newly approved administrator production-cancel button and one real calendar filter alignment regression caused by the deleted-record checkbox. Assertions now check the approved button; the checkbox occupies its own grid row to preserve month navigation alignment. Photo and regression validation and independent review are in progress; no merge/deployment yet.
Frontend validation: dedicated photo input tests7/7 passed; typecheck/eslint and production build passed. Full Busbar browser suite54/54 passed in3.3min, including front/back completion, worker/permission flows, deletion/restore, HEIC-compatible saved preview request and unsupported input rejected before upload. Desktop1440/mobile390 photo popup screenshots and1440 calendar filter screenshot were opened and checked. Browser tests use synthetic API fixtures and do not establish real HEIC decoding; actual file/multipart validation belongs to the backend tests still running.

Backend focused disposable DB validation6/6 passed: actual multipart originals scanned before sanitizer; JPEG/PNG/HEIC and color/orientation-correct WebP compatibility, mismatch/unknown rejection, MIME-correct originals/history, HEIC preview/publication, combined40MiB rejection without completion, legacy migration bytes preserved. Independent reviewer rechecked final code and closed both findings: HEIC publication derivative participates in20s timeout without retaining the global transaction lock beyond the wait; WebP pixel colors are compared against sRGB expectation. No remaining static Finding. Final oversized-WebP boundary test and remoteCI pending.
Final backend Release build passed0warnings/0errors. Final non-DB image/security run20passed/4DB-skipped includes >40MiB WebP rejection before decode; broader related image/codec run36passed/8DB-skipped. The dedicated Busbar DB run above has6passed/0skipped and covers the actual integration. General PostgreSqlMigrationTests could not authenticate to the local default QMS database (28P01); this is not counted as a pass. Its required CI run remains pending, while the focused0118 upgrade passed against the dedicated disposable DB. Implementation and independent review are complete; remote verification and deployment remain pending.
