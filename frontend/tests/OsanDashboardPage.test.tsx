...
describe('오산 홈·진행 현황 공용 목록', () => {
  ...
  it('고객사와 상태는 각각 복수 선택하고 날짜·검색·KPI는 기존 조건을 유지한다', async () => {
    ...
    const statusDialog = screen.getByRole('dialog', { name: '상태 선택' });
    fireEvent.click(within(statusDialog).getByRole('checkbox', { name: '공정 시작 전' }));
    fireEvent.click(within(statusDialog).getByRole('checkbox', { name: '공정 진행 중' }));
    fireEvent.click(within(statusDialog).getByRole('button', { name: '적용' }));
