...
| `AZURE-OSAN-CI-BACKEND-DB-01` | P2 | `RESOLVED_LOCAL` | Draft PR #121 첫 CI run `34104864066`에서 Backend 일반 job의 disposable DB 이름이 owned E2E prefix가 아니어서 전용 3-DB test 1개가 581개 통과 뒤 fail-closed했다. Job의 DB·service·health 이름을 같은 `emi_qms_e2e_*` 경계로 정렬해 582개 전체를 격리 runner에서 실행하며 수정 head 필수 CI로 재검증한다. |
...
## 17. Change 008 접근 정합성 재배포와 repair 종료

PR #123은 제품 Change 008을 exact main `d1e7d1fb20fa8976843b441ba7b5e46b54723aee`에 병합했고 Business migration `0088`을 DB-only run `34207947257`로 먼저 적용했다. Safe inspection을 위한 PR #124~#126과 현재 Directory overall authority 보정 PR #127을 순차 병합했으며, 최종 exact source는 PR #127 head `7e95129ce0cbf5b389ead02b7d6558c67b268675`, CI `34221079465` PASS, main `b41c932e2154a921cf8b0aab753fc6d0692b209f`다.

Full production release `34225420777`는 migration, readiness-aware backfill, Backend, Frontend와 public security를 모두 통과했다. Roleless active membership `1`건을 차단했고 기존 Cheongju 업무·G2·provider와 Osan provider/worker off를 유지했다. 배포 결과는 Backend `backend--0000040` / `sha256:16f3196151423c6db98e1e469651a0949471c77044dca1558392a0044c8818d2`, Frontend `frontend--0000029` / `sha256:a397373ec63c2d32d532e4058d043ea1e19ed587e6e9e2a2d28837d43e421ce4`, latest ready와 traffic `100%`다.

첫 post-deploy inspection `34226444177`은 현재 Directory overall `3`과 stale managed System Administrator removal `1`을 확인했다. 이는 backfill이 overall designation을 변경한 결과가 아니라 배포 뒤 통합 사용자 관리에서 overall 해제가 발생한 뒤 local managed role이 남은 상태였다. 실패한 첫 DB-only scope run `34228051686`은 validation 전 중단돼 mutation `0`이었다. 승인된 repair run `34228436474`는 current overall `3`을 보존하면서 그 역할 `1`건만 제거했다.

최종 inspect `34229045510`은 identity `23`, overall effective/configured/current `3/3/3`, 모든 membership·role·head·overall repair marker `0`, Cheongju·Osan System Administrator permission gap `0/0`을 확인했다. Directory ledger `4`, 두 business ledger `88`, identity contract Directory `1`/Business `86`을 유지한다. Public security와 latest-ready/digest 확인은 PASS했다. Browser integration timeout 때문에 이 최종 revision의 인증된 실제 계정 UI는 직접 재관찰하지 못했으며, 실제 총괄 selector·양 사업부 읽기/쓰기, 일반 사용자 selector 미노출, 부서 이동과 Osan 승인 저장은 사용자 smoke로 남긴다. Synthetic production 업무 record는 생성하지 않았다.
