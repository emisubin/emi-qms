...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 030 Exact-main Azure Released / Change 031 Local Validation Complete · Draft PR/CI Pending | 현재 공개 기준선은 G2 Change 004 제품 PR #119, exact main `7a2c7f172a4a0e4b0e69a29c72ac205af1299c74`, Azure release `33589472932`다. Change 031은 사용자 명시 순서 변경에 따라 오산 Task 1~3만 먼저 배포하도록 기존 운영 DB를 Cheongju로 유지하고 같은 server에 Directory·Osan DB, 분리 role/secret, 3단계 DB job, restore 전 public 연결 분리를 준비한다. Backend `582/582`, Frontend `297/297`, mock `13/13`, Full-Stack `66/66`과 배포 정적 검증을 통과했다. 일반 사용자는 membership 한 곳만 부여하고 dual-membership 전환 결함은 보류한다. | TASK-AZURE-PILOT-001, TASK-CI-COST-001, TASK-SITE-ACCESS-001, TASK-G2-OPERATIONS-002 Change 004, TASK-OSAN-ISOLATION-001, TASK-OSAN-ACCESS-001, TASK-OSAN-PROJECT-001 | Change 002 selector 사용자 검수, exact main merge, 3-DB PITR rehearsal, actual account·Cheongju·Osan 검증 | Yes | Change 031 Draft PR·필수 CI → selector 사용자 검수·exact main merge 승인 → DB/restore/app 순차 배포 |
...
| 2 | [TASK-OSAN-ACCESS-001](../tasks/osan-access-001.md) — 사용자·사업부 전환 | CHANGE_002_IMPLEMENTED_AWAITING_USER_VALIDATION | 1 | 단일 소속 selector 숨김 보정 사용자 검수 — Task 4 선행 차단 아님 |
...
| 7 | [TASK-AZURE-DEPLOY-001 Change 031](../tasks/osan-pilot-001-rollout-handoff.md) — 오산 1단계 개통 | LOCAL_VALIDATION_COMPLETE_AWAITING_DRAFT_PR_CI | 사용자 명시 partial sequence override로 Task 1~3·배포 승인. Backend 582/582·Frontend 297/297·browser 79/79와 배포 정적 검증 완료. Task 2 selector 사용자 검수와 exact main merge는 대기 | Draft PR·필수 CI → selector 사용자 검수·main merge 승인 → DB/restore/app 순차 배포 |
