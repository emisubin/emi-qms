...
이 문서는 EMI 프로젝트 통합관리시스템에서 “한 역할이 로그인부터 자기 업무 완료·인수인계까지 웹사이트를 어떻게 통과하는가”를 한 곳에 고정하는 전체 유저플로우 기준선이다. 1차 용도는 두 가지다.

1. 후속 NEW_FEATURE Task 기획 기준선 — 이후 기능 Task는 메뉴 위치, 진입점, 여정 연결을 이 문서에 맞춘다.
2. 사용자 온보딩·교육 — 각 역할이 자기 여정을 처음부터 끝까지 이해하는 안내로 사용한다.

이 문서의 현재 지위는 다음과 같다.

- 이 문서는 `TASK-USER-FLOW-001`([interview](../tasks/user-flow-001-interview.md) · [planning](../tasks/user-flow-001-planning.md)) Phase A의 **미게시 preview**다. 사용자 검수 전이며 게시본·최종본이 아니다.
- 기존 [`docs/02-business-flow.md`](02-business-flow.md)와 [`docs/04-permission-matrix.md`](04-permission-matrix.md)는 Phase A에서 **변경하지 않았다**. 두 문서와 현재 구현의 정렬(P2-002 해소)은 사용자 preview 검수 뒤 같은 Task의 Phase B에서 수행하며, P2 해소 전에는 이 문서를 완료·게시하지 않는다.
- 이 문서는 기획 기준선이며 기술 기준선이 아니다. 권한은 계속 서버 Policy가 authoritative하고, 이 문서는 권한·데이터·알림 계약을 새로 정의하거나 확대하지 않는다.
...
모든 역할 여정은 다음 3유형의 진입점을 공통 패턴으로 사용한다.
...
| 메뉴 직접 진입 | 상단 내비게이션 메뉴로 각 업무 화면에 직접 진입한다. | 구현 — 목표 골격은 4절 |

현재 구현에서 내 업무 카드의 이동 대상은 두 갈래다.

- **실제 입력 페이지 직접 연결(구현)**: 생산계획·담당자, 패널명·사이즈, 구매정보 업무는 카드에서 해당 입력 페이지로 직접 이동한다.
- **workflow 요약 fallback(구현)**: 자재 도착 이후 단계의 업무는 전용 입력 화면 도입 전까지 프로젝트 상세의 workflow 요약으로 이동한다. 각 단계 카드의 실제 입력 페이지 직접 연결은 해당 단계 도입 Task에서 추가한다(`예정`, 9절 매핑 참조).
...
deep link 진입 실패 시 공통 예외 E4(6절)를 따른다.

## 3. 18단계 마스터 플로우

18단계 표준 업무 프로세스와 역할 간 인수인계 전체를 하나의 마스터 flow로 고정한다. 각 단계 완료 시 시스템이 다음 담당자의 내 업무를 자동 생성하고 참조 대상자에게 참조 알림을 생성한다.
...
- 실선 채움: 구현(1~4단계 입력 화면). 옅은 노랑: 부분 구현(자재는 현재 자재 입고 입력 기반이 구현되어 있고 도착/입고 확정/키팅 분리는 `TASK-008A`·`TASK-010A` 예정). 점선 node: 입력 화면 미구현(도입 Task는 9절 매핑 참조).
...
목표 메뉴 골격은 **Home · 내 업무 · 프로젝트 · 부서 업무 · 알림 · 관리자**다. 미구현 메뉴·화면은 해당 도입 Task 승인·구현 전까지 Frontend에 노출하지 않는다.
...
flowchart LR
    ...
    MW --> PL["Pending List — 예정, TASK-007A 공통 진입"]
    PJ --> PL
    ...
    class MT,AD cond
    class HOME,QC,MF,LG,PL,ST planned
```

- “부서 업무”는 목표 골격의 묶음 개념이다. 현재 구현은 생산관리·구매·자재를 상단 메뉴에 개별 항목으로 표시하며, 묶음 UI로의 전환 여부·시점은 각 부서 업무 도입 Task에서 결정한다.
- Pending List는 별도 최상위 메뉴가 아니라 **내 업무·프로젝트 하위의 공통 진입**으로 위치만 확정한다(사용자 결정 3A). 화면 세부·첨부·유형 정책은 `TASK-007A`에 위임한다.
- 정산은 **영업의 프로젝트 하위**로 위치만 확정한다(사용자 결정 3A). 세금계산서·완료 권한 정책은 `TASK-014A`에 위임한다.

## 5. 현재 구현 대 목표 상태

| 메뉴/화면 | 현재 상태 (Repository 기준) | 목표 상태 | 도입 Task |
| --- | --- | --- | --- |
| Home | 없음 | PC·모바일 widget-slot 요약 홈. 위치만 확정, 세부 위임 | `TASK-HOME-001` |
...
| Pending List | 없음 | 이슈 단위 공통 모듈, 내 업무·프로젝트 하위 공통 진입 | `TASK-007A` |
...
모든 여정이 참조하는 공통 예외를 한 번 정의한다. 구현 구간 여정은 실제 경로 기준 단계별 예외를 7절에 상세 기록하고, 미구현 구간 여정은 “Pending 연결과 다음 담당자 알림” 수준까지만 기록한다.
...
### 7.1 영업 — 프로젝트 생성(1)과 세금계산서·완료(18)
...
- 단계별 예외(구현 구간 상세): 필수 필드 validation 실패 시 저장이 차단되고 입력 화면에서 오류를 안내한다. 권한 없는 사용자의 생성·수정은 서버 Policy가 차단한다(E1). 담당자 미지정 프로젝트는 E2 fallback으로 다음 업무 대상자가 정해진다. 삭제된 프로젝트는 영업이 **보관함에서 조회**할 수 있다. **복구는 영업 흐름이 아니다** — 복구 endpoint와 UI는 `Audit.Read.All` 권한에 한정되며, 현재 이 권한은 System Administrator만 보유하므로 복구는 관리자 여정(7.12)에 속한다.
...
- 구간 상태: 구현 상세
- 정상 흐름: 내 업무 카드 “패널명·사이즈 입력”(실제 입력 페이지 직접 연결) 또는 `프로젝트` 상세의 패널정보에서 진입한다. 패널명·사이즈를 직접 입력하거나 Excel preview/apply로 일괄 입력한다. 활성 패널의 필수 정보가 채워지면 설계 단계가 완료 판정되고 구매 단계로 이어지며 생산관리에 참조 알림이 간다. 구매 내 업무는 생산계획 완료 시 이미 선행 생성되어 있다(7.3 참조).
...
- 단계별 예외(상세): 필수 담당자 누락 시 완료가 차단된다. 담당자 부재는 E2. Pending 관리 화면에서 다른 부서 업무를 생성하는 능력은 `TASK-007A` 도입 시 제공한다(예정).
...
### 7.10 품질 전진검수·FAT — 전진검수(13)·FAT(14)
...
### 7.12 System Administrator — 관리자 여정
...
| --- | --- | --- | --- | --- |
| 영업(1) / 설계 / 생산관리 / 구매 | 상세(7절 각 여정) | 상세 | 참조 — `TASK-007A` 전 수준 | 상세(2.1 경로) |
| 자재 | 상세(구현분) | 상세 | 참조 | 상세 |
...
“상세”는 실제 구현 경로 기준 단계별 예외를 7절에 기록했다는 뜻이고, “참조”는 6절 공통 정의와 Pending 연결·다음 담당자 알림 수준까지만 기록한다는 뜻이다.

## 9. 후속 Task 매핑과 위임 결정 역색인

도입 Task ID는 [Product Roadmap](00-product-roadmap.md) 실행 큐 기준이며, 각 Task 문서가 생성되기 전까지 Roadmap을 참조 원본으로 사용한다.

| 도입 Task | 이 문서가 확정한 것 | 이 문서가 위임한 결정 |
| --- | --- | --- |
| `TASK-007A` Pending List | 내 업무·프로젝트 하위 공통 진입 위치, 이슈 단위 목록, E3 공통 예외 연결 | 화면 세부, 첨부 저장·업로드 보안, Pending 유형·상태 UI, 생산관리의 타 부서 업무 생성 화면 |
...
| `TASK-014A` 정산 | 정산의 영업(프로젝트 하위) 위치, 완료 조건 인용 | 세금계산서·완료 권한 정책, 화면 세부 |
| `TASK-HOME-001` Home | 메뉴 골격 내 Home 위치, 도입 전 미노출 | widget-slot 구성과 MVP 범위 |
| `TASK-MOBILE-001` | 동일 URL·overflow 0 원칙의 여정별 적용 선언 | 역할·화면별 narrow UX 상세와 우선순위 |
...
1. 이 문서는 승인·게시 후 전체 유저플로우의 canonical 기준선이 된다. 후속 Task가 화면·흐름·메뉴·진입점을 바꾸면 **같은 Task에서 이 문서를 함께 갱신**해야 하며, 갱신 없이 완료 처리하지 않는다.
2. Backend policy·[Product Roadmap](00-product-roadmap.md)·이 문서 사이에 충돌이 생기면 Backend와 승인된 최신 Task 계약을 우선 확인하고, 임의 선택 없이 같은 Task에서 이 문서를 정렬한다.
3. 미구현 메뉴·화면은 해당 도입 Task 승인·구현 전까지 Frontend에 노출하지 않는다. 도입 Task는 구현 시점에 이 규칙을 계약으로 준수한다.
4. 이 문서는 미확정 정책(검사 양식, Pending 첨부, 포장 기준 등)을 확정하지 않는다. 9절 역색인의 위임 결정은 각 도입 Task planning 승인 항목으로만 확정한다.
5. 알림 채널·event coverage 상태는 Roadmap 6.5를 원본으로 인용하며 이 문서에서 상태를 올리지 않는다.
6. Phase 경계: 이 preview가 사용자 검수를 통과하면 같은 Task의 Phase B에서 [`docs/02-business-flow.md`](02-business-flow.md)(current overview + 이 문서 link 정렬)와 [`docs/04-permission-matrix.md`](04-permission-matrix.md)(현재 code·policy 기준 정렬)를 갱신한다. Phase B의 P2 해소 전에는 이 문서를 완료·게시·merge하지 않는다.
