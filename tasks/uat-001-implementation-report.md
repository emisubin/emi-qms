...
## 29. Change 009 — 프로젝트 진행률·현재 단계 일치

### 해결한 업무 문제

- 프로젝트 목록은 초기 4단계만 계산해 전체 흐름 71%인 프로젝트를 18%로 표시했다.
- 전체 흐름은 가장 앞의 미완료 단계를 현재 단계로 선택해 납품·영업 인계까지 도달한 프로젝트도 구매정보로 표시했다.
- 구매정보·자재 도착·입고검사·입고확정은 업무상 건너뛴 뒤에도 진행률을 계속 낮췄다.

### 구현 결정

- 목록은 별도 축약 SQL 결과 대신 canonical workflow 진행률과 현재 단계를 사용한다.
- 현재 단계는 가장 뒤의 실제 도달 단계를 선택하고, 그 단계가 완료되었으면 다음 필수 단계를 선택한다.
- 네 개의 통과 가능 단계는 뒤 단계 도달 시 진행률에서만 완료로 세며 개별 stage 상태와 DB data는 바꾸지 않는다.
- 프로젝트 자체의 보류·완료·취소 표시는 기존처럼 우선한다.

### 자동 검증과 실제 projection

| 검사 | 결과 |
| --- | --- |
| 집중 회귀 | 2/2 통과 |
| 프로젝트 등록·목록·물류 확대 회귀 | 77/77 통과 |
| 패널정보·QR 회귀 | 38/38 통과 |
| QR full-stack 회귀 | 1/1 통과 — 현재 단계가 구매정보로 전진한 fixture를 구매 담당자로 검증 |
| Backend Release build | warning/error 0/0 |
| 실제 두 프로젝트 변경 전 | 목록 18%, 전체 흐름 71%, 현재 단계 구매정보 |
| 실제 두 프로젝트 변경 후 | 목록·전체 흐름 94%, 16/17, 현재 단계 세금계산서 |
| HTTPS 5174 browser | 104·105 desktop/mobile 확인, console error 0 |

공식 UAT 실행기가 소유권을 확인한 뒤 5174·5081을 정상 교체했다. 기존 fresh DB를 그대로 사용했고 migration·seed·data 변경과 실제 provider 발송은 실행하지 않았다. 구매정보·자재 도착·입고검사·입고확정의 실제 상태는 부분 완료로 유지하면서 진행률 계산에서만 완료로 반영됨을 API와 화면에서 확인했다. 상세 계약과 checklist는 [Change 009](uat-001-change-009.md)에 기록했다.

2026-08-04 사용자가 생산계획 진행률 막대와 Change 009 수정·Task 기록의 원격 `main` 병합을 명시적으로 승인했다. DB 입력 데이터, runtime artifact와 환경 설정은 게시 범위에서 제외한다.
