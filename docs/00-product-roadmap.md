...
| 5.1 | DESIGN-000 Design foundation | P2_REMEDIATION | Change 006 Local Main Merged / User Validation Complete / Remote Unpublished | Change 001~005 완료 foundation을 재구현하지 않고 독립 Graphite 실험의 공통 시각·구성, 장식용 왼쪽 rail 제거, table density, 클릭형 부서 disclosure와 업무 선택 전용 page 삭제를 최신 main fixed allowlist로 승격. Frontend lint·typecheck·unit `170/170`·build·격리 mock E2E `4/4`, 사용자 검수와 local main merge `f1f94ed` 완료 | 사용자 Graphite 구현·검수·local merge 승인, 최신 main 기반 clean promotion branch, 명시적 Azure Gate 재정렬 승인 | Backend·DB·migration·배포·실제 provider 제외. Figma publish와 App route split은 범위 밖. Remote push·PR 미승인 | Yes | 완료 scope 재구현 금지; TASK-AZURE-DEPLOY-001 비용 Gate·`PRE_TRAFFIC_GATE` 복귀 |
