...
| 3.3M | TASK-SITE-ACCESS-001 — 유지 세션 포함 사이트 접속 이력 | APPROVED_FEATURE_IMPLEMENTATION / P2_REMEDIATION | Latest-main Integration / Automated & Full-Stack Validation Complete / Independent Product Validation PASS / Documentation P2 Recheck Pending / User Validation Pending / Publication Approved | 사용자·브라우저 client·30분 활동 창별 접속 원장, 19개 고정 메뉴, 서버/DB 권위 시각, 명시적 로그아웃 종료, 별도 coverage·시간 해석 안내와 전체 감사 이력·선택 Excel 통합을 구현했다. 공개 G2의 migration `0084`·추적 97을 보존하고 사이트 접속을 `0085`·추적 98로 교정했다. 독립 검증은 제품·통합을 PASS했지만 상태 문서 불일치 P2 `SITE-ACCESS-FINAL-F01`을 발견해 보정 후 read-only 재확인 중이다. | TASK-AUDIT-001, 기존 Entra/MSAL identity, `Audit.Read.All`, migration `0083`, 공개 G2 `0084` | 사용자 화면 검수 대기. Persistent UAT·Azure 배포 미승인. Git 게시·원격 main 병합은 승인됐지만 P2 재확인 전 실행 금지 | Yes | `SITE-ACCESS-FINAL-F01` read-only 재확인 → Push·Ready PR·CI Gate·원격 main merge |
