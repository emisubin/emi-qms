...
export function OsanCustomerMatch({value,customerId,onChange,userKey,disabled}:{value:string;customerId?:string;onChange:(name:string,id?:string)=>void;userKey?:string;disabled?:boolean}) {
 ...
 const type=(text:string)=>{const exact=customers.filter(c=>normalize(c.name)===normalize(text));const matching=exact.length?exact:customers.filter(c=>normalize(c.name).includes(normalize(text))&&text.trim());onChange(text,matching.length===1?matching[0].customerId:undefined);};
 ...
 <small id={resultId} role="status">{error|| (matched?`연결된 고객사: ${matched.name}`:!value?'고객사 이름을 입력하세요.':candidates.length>1?'여러 고객사가 일치합니다. 고객사를 선택해 주세요.':'등록된 고객사와 연결해야 저장할 수 있습니다.')}</small>
