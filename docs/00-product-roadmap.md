...
| 3.3B | TASK-UL891-PRODUCTION-PLAN-001 실물 세트별 생산계획 | NEW_FEATURE | Experiment Complete / User Validation Pending | Fable 2-pass·migration `0064`·세트 overlay·전체/세트 표·일정 동시 전환·완료 판정·Backend `430/430`·Frontend `142/142`·PC/390px·일반/stress lifecycle·checkpoint `e6f3fa6` 완료 | TASK-PRODUCTION-CONTROL-001·TASK-UL891-SET-001 | 대표 repo·main·Persistent UAT 미반영, 사용자 최종 일괄 검수 대기 | Yes | 재기획·재구현 금지; 마지막 일괄 검수 |
...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 005 Main Merged / Active Workloads Ready / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `67 Exact`·PITR restore와 세 workload latest revision readiness 완료 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~005 main merge | Front Door route 0, DNS·TLS·원본 업무 route 차단·인증·실제 provider·통합 main image 재배포 미완료 | Yes | 제품 기준선 통합 완료 → UL891 porting·회귀 → 통합 source image 재배포 → Edge·DNS·TLS·provider 검수 |
