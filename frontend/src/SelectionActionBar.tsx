...
export function SelectionActionBar({ count, allSelected, onToggleAll, onClear, actions, mobilePrimary }: Props) {
  ...
  return <div className="selection-action-bar" role="group" aria-label="목록 선택 작업">
    ...
    <div className="selection-action-desktop">{actions.map(action => <button type="button" key={action.id} disabled={!count || action.disabled} onClick={action.onClick}>{action.label}</button>)}{count > 0 && <button type="button" onClick={onClear}>선택 해제</button>}</div>
    <div className="selection-action-mobile">
      {primaryActions.length > 0 && <button type="button" className="selection-action-primary" disabled={!count || primaryActions.every(action => action.disabled)} aria-haspopup="dialog" aria-expanded={menu === 'primary'} onClick={() => open('primary')}>{mobilePrimary?.label}</button>}
      <button type="button" disabled={!count} aria-haspopup="dialog" aria-expanded={menu === 'more'} onClick={() => open('more')}>더보기 <span aria-hidden="true">⋯</span></button>
    </div>
