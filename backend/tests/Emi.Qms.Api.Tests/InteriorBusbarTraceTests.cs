...
public sealed class InteriorBusbarTraceTests
{
    ...
    public async Task MigrationPreservesHistoricalQuantityOnlyShipmentAndExistingPhotos()
    {
        ...
        await using(var migration=new NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root,"database/migrations/0114_interior_busbar_panel_trace.sql"),TestContext.Current.CancellationToken),c))
