# TASK-OSAN-PROJECT-001 Change008 — 오산 프로젝트 관리·순차 진행·사진 승인

2026-09-10 사용자 구현 및 로컬 검수 화면 제공 승인. 공개배포 금지. 원격 반영은 이번 범위에 없다.
기준 main4822157을 기존 작업 이력을 보존하여 통합한 codex/osan-project-management, 소스 /private/tmp/emi-osan-photo-upload-fix. 원본 checkout WIP 및 기존 runtime은 임의 변경하지 않는다.

- 오산 title 표시를 장비명, productName 표시를 part분류로 통일. 생성·엑셀 순서 장비명/코드/part분류/수량/거래처/PO/W/O/납기. 기존 Excel 헤더도 계속 읽는다.
- 한국 날짜 기준 D-day: 홈·진행 현황 장비명 옆, 프로젝트 목록 납기일 옆 괄호. 기존 홈 정렬·숨김 정책 유지.
- 기존 7단계를 대상별 순차 제한. 이전 단계 미완료면 입력·완료 거부. 과거 비순차 완료 이력은 보존. 모든 대상 포장 완료 시 프로젝트 자동 완료 유지.
- 사용자 확정: 프로젝트 수정·삭제는 관리자만. 삭제는 목록에서 숨기고 이력·사진 보존. 진행 시작 후 수량 변경 금지. 수정 충돌 검출·서버 권한 및 사업부 경계 유지.
- 사용자 확정: 사진 대상·단계별 관리자 1회 승인. 신청자만 사용하며 저장 후 재잠금. 변경 전 사진·승인/변경 이력 보존. 기존 업로드 검사·메타데이터 정리·컬러 보존 재사용.
- 새 migration0094로 변경 기록·사진 승인/수정본을 추가. 기존 migration과 데이터는 덮어쓰지 않는다. 실제 DB/권한/경쟁/멱등성 집중검증, 화면 PC·모바일 확인 및 독립 review 후 local commit.

현재: 구현·관련 자동 검증·독립 검토 완료. 사용자 최종 검수 대기. 공개배포·원격 변경 미실행.

## 구현과 검증

- 관리자 수정·삭제, 수정 충돌 검출, 프로젝트/진행 완료 요청 잠금, 단계 선행조건을 서버에서 적용했다. 진행 전 수량 감소는 대상 행을 삭제하지 않고 비활성화하며 재증가 시 같은 ID를 복원한다. 진행 이력·원본 사진은 보존한다.
- 사진 요청·승인·신청자 저장·재잠금과 이전 사진 조회를 구현했다. 동일 저장 재시도는 중복 반영하지 않고, 불확실한 응답 후에는 같은 요청을 유지한다.
- Backend Release 빌드 성공(경고/오류 0). 집중 검증 8개 통과: 실제 PostgreSQL 진행·관리 2개, 감사/경로 5개, 실제 3DB HTTP 권한·사진 왕복 1개. migration0094 재적용도 별도 통과. 사진 다운로드 PNG 바이트 일치 확인.
- Frontend 관련 기존 6개 파일 69개 테스트 및 관리/사진/진행 관련 4개 파일 35개 테스트 통과. 최종 관리/사진 14개 재검증 통과(앞 결과와 중복, 합산하지 않음). 타입 검사·lint·제품 빌드 통과. 빌드에는 큰 JS 청크 안내가 남는다.
- 브라우저 자동 검증 3개 통과: PC/모바일 생성, Excel 편집·부분 저장·중복 확인, 한국 날짜 자정 D-day 변경.
- 실제 합성 API 연결 화면에서 관리자 정보 수정, 선행 단계 제한, 컬러 사진 완료·신청·승인·교체·재잠금·원본 이력 확인. PC와 390px 화면 확인, 모바일 사진 넘침 보정 후 문서 폭 390px 및 사진 filter none 확인. 진행 시작 후 수량 입력 비활성 확인.
- 독립 reviewer management_review가 계약·누적 diff·검증 근거를 검토하고 P0–P2 없음, 로컬 커밋/사용자 검수 GO 판정. 전체 회귀는 사용자 검수 후 최종 후보에서 수행하며 이번 관련 검증을 전체 회귀로 보고하지 않는다.

## 로컬 검수 환경

- source: /private/tmp/emi-osan-photo-upload-fix, branch codex/osan-project-management.
- UI http://127.0.0.1:5194 / API http://127.0.0.1:5098. 오산 dev-admin, 합성 프로젝트 ‘검수 장비 A’ 제공.
- 별도 Docker project emi-qms-e2e-osan-management-20260910, disposable tmpfs Directory/청주/오산 3DB. 실제 provider 비활성. 기존 5186 환경과 운영 데이터는 변경하지 않았다.
- 실행 소유: 이 작업의 API session38860 및 frontend session96601. 기동 스크립트 /private/tmp/osan-management-preview.sh는 종료 시 자신의 합성 자원만 정리한다.
- 남은 일: 사용자 검수 및 필요한 보정. 원격 push·병합·공개배포는 이번 승인 범위가 아니다.

## 검수 후 UI 보정 (2026-09-10)

사용자가 수정 구현 승인. 프로젝트 상세의 관리자 수정·삭제를 우측 상단 목록으로 옆에 같은 버튼 스타일로 배치하고, D-day를 홈/진행 현황 16px 및 프로젝트 PC/모바일 15px로 확대한다. 오산 거래처 표시를 고객사로 통일하고 프로젝트 열은 장비명/part 분류/고객사/Code/수량/납기일/상태/진행률 순서로 변경한다. 검색은 유지하고 세 화면 필터는 고객사별·상태별만 제공한다. 고객사 필터는 일치 검색이며 대시보드의 권한 범위·홈 제외 조건과 함께 서버 페이지 분할 전에 적용한다. 기존 Excel ‘거래처’ 헤더 입력 호환은 유지한다. DB 필드·관리자 권한·사진 승인 계약은 그대로다.

프런트 관련 4개 파일 44개 테스트, 타입 검사·lint·제품 빌드 통과. 브라우저 검증 3개(생성/목록/청주 보존, D-day 날짜 전환, Excel) 통과. PC1440/모바일390 PNG를 직접 열어 확인했고 실제 API 연결 화면에서 상단 버튼 높이26px 및 모바일 문서 폭390px 확인. 모의 API의 CORS 기준과 다른5195 테스트는 실패하여 기존 허용5173에서 재실행했고, 승인된 열 순서 변경에 따른 비교 기대값을 보정했다. 독립 reviewer 추가 diff 검토 P0–P2 없음. Backend Release 빌드 경고0/오류0 및 쿼리경계·Excel·실제 PostgreSQL dashboard 집중 검증 3/3 통과. 실제5099 API에서도 고객사 exact/prefix 구분 및 고객사 옵션 보존 확인. 기존5098 PID5358와 소유 wrapper/합성3DB를 유지하고, 같은 안전한 DB/provider 설정의 최신 API5099(PID12053, seed/startup migration 비활성)를 추가 기동했다. UI5194는 최신5099에 연결하며 현재 frontend session68161이 소유한다. 기존 프로젝트·사진을 보존했다. 최신 API PID/ready 증거는 /private/tmp/osan-management-preview-5099.pid 및 /private/tmp/osan-management-preview-5099-ready.json에 있다. 이 보정은 구현·관련 검증 완료, 사용자 검수 대기 상태다. 공개배포/원격 반영 승인 없음.

### 모바일 D-day 잘림 보정

확대된 16px 글자가 기존 작은 줄 높이와 overflow 경계에 잘리는 문제를 브라우저의 실제 문자 영역 검사로 재현했다. D-day 줄 높이를20px로 확보하고 모바일 진행 목록 제목의 위 위치를6px로 조정해 진행률 선과 겹치지 않도록 했다. 글자 크기·카드 크기·단계 위치는 유지한다. 동일 재현 검사가 수정 전 실패, 수정 후 통과했으며 PC1440/모바일390의 홈·진행 및 자정 D-Day 전환 확인, 모바일 PNG 직접 시각 확인 완료. CSS 보정이므로 전체 제품 회귀는 실행하지 않았다. 종료돼 있던 frontend5194는 동일API5099/기존 합성DB 연결로 재기동(PID13568, /private/tmp/osan-management-frontend-5194.pid)했다. 원격 반영·공개배포 없음.

### 사용자 제공 7단계 작업 설명 반영

사용자가 제공한 입고검사/배치검사/배선검사/8계통/동작검사/출하검사/포장 설명을 그대로 반영했다. 사진 등록 지시문은 굵게 표시하고 도장60~150㎛·색차△e1.5이하 등 수치와 순서를 보존했다. 표시용 공백/HTML 공백 표기만 일반 공백으로 정리했다. 이전의 설명·안내 사진 미제공 자리표시자는 현재 설명으로 대체하고 안내 사진 영역은 렌더링하지 않는다. 향후 사용자 사진 제공 후 해당 영역을 다시 구현한다. 기존 완료 증빙 사진 입력·조회와 사진 첨부 선택 정책은 변경하지 않는다.

진행 상세 관련15개 테스트, 타입검사 및 변경 컴포넌트lint 통과. 실제 합성API 화면에서 7단계를 순서대로 열어 문구 확인, PC 및 모바일390px 스크린샷 직접 확인, 안내 사진 영역0개 및 가로 넘침 없음 확인. 로컬5194 반영, 사용자 검수 대기. 원격 반영·공개배포 없음.
### PC 전체 단계 보기와 상세 팝업, 설명 가독성 보정 (2026-09-10)

사용자 구현 승인에 따라 PC(861px 이상)는 7단계를 한눈에 표시하고, 각 단계 클릭 시 큰 팝업에서 설명·완료 사진·작업자·완료 일시를 조회하도록 변경했다. 완료된 단계도 설명을 함께 제공한다. 완료 버튼은 기존 순차 처리·권한 기준을 그대로 적용하며 완료 확인 창을 닫으면 단계 팝업으로 복귀한다. 모바일은 기존 이전/다음 단계 이동을 유지하고 설명을16px로 확대하고 좌우 여백을 줄였다. PC 팝업 설명은17px다. 안내 사진 영역은 계속 숨기며 완료 증빙 사진·사진 수정 승인 계약은 보존한다.

진행 상세17개 테스트, 타입 검사 및 lint 통과. PC1440 화면에서7단계 목록과 완료 단계의 큰 설명·컬러 사진·완료 기록을 직접 확인했다. 실제 합성API 연결 화면에서 완료 확인 창 Escape 복귀와 후속 단계 완료 잠금을 확인했다. 모바일390px 스크린샷 직접 확인 및 가로 넘침 없음(문서 폭390px), 기존 화살표 이동 유지 확인. 기존 전체 스택 등록 시나리오의 PC 단계 탐색 기대값을 새 화면에 맞췄으며 해당 전체 스택 시나리오·전체 회귀는 이번 UI 보정에서 재실행하지 않았다. 로컬5194 제공, 사용자 검수 대기. 원격 반영·공개배포 없음.

### 프로젝트 QR 조회·개별/다중 인쇄 (2026-09-10)

사용자가 시안을 승인하고 구현 지시했다. 상세 QR 버튼 및 프로젝트 목록 선택/전체 선택 후 다중 출력. 30×30mm 또는50×50mm는 여백·QR·장비명·part분류·W/O 3줄을 포함한 라벨 전체 크기다. 승인 시안 revision4, 사용자 제공 EMI 원본 로고 적용. 모바일 설명14px/태블릿16px, 기기 폭 안의 단계 팝업에서 설명·완료 증빙 사진·작업자·완료 일시 조회. QR은 /osan/qr/{projectId} 고정 링크이며 로그인 후 같은 경로 복귀, 서버가 허용한 오산 사업부와 기존 ProjectRead/project access scope로 조회한다. QR 화면은 조회 전용이고 삭제 프로젝트는 조회 불가. 원격 반영·배포는 포함하지 않는다. 합성 runtime/브라우저·인쇄 규격·판독 및 권한 검증과 독립 review 후 로컬 커밋 예정.

구현·관련 검증 및 독립 검토 완료, 사용자 검수 대기. QR 기능6개·사업부24개·인증25개·기존 진행17개 테스트 통과. 타입검사·변경 파일 lint 통과. 제품 빌드 성공(기존 큰 번들 경고 유지). 기존 진행 테스트의 잘못된 Testing Library exact 옵션3곳도 타입검사에서 발견해 제거했다. 오산 QR endpoint/catalog/origin/ZXing 단위9개 및 실제 disposable3DB HTTP통합1개 통과: 미인증401, 잘못된 사업부/미배정403, 배정 일반 사용자200, 없는 프로젝트404, SVG·PNG·캐시/보안 헤더 및 PNG 판독 포함.

실제 합성API 화면에서 휴대폰390px(문서390/팝업358/설명14px), 태블릿834px2열, 컬러 완료사진·작업자/시간, 프로젝트2개 선택 출력 확인. 30/50mm 라벨 실제 화면 크기 및 캡처 QR을 ZXing으로 디코딩해 프로젝트 주소 일치 확인. 자동 grid 페이지 분할에서 두 번째 페이지 QR 잘림을 발견하여 명시적 페이지(30mm40개/50mm15개)로 수정했다. 45개·18개PDF를2페이지로 렌더링하고 페이지2의 QR·3줄이 잘리지 않음을 직접 시각 확인했다. 인쇄 helper 분리 후6개 테스트·타입검사·lint 재통과. 독립 reviewer 최초/페이지분할 보정분 P0–P2 없음. 합성 화면·인쇄 증거는 /private/tmp/osan-qr-*에만 보관하며 tracked파일에 실제 데이터나 생성PDF를 넣지 않는다. 실제 프린터 치수·휴대폰 카메라 및 Entra 실계정 로그인 검수는 미실행이다. 로그인 테스트는 기존 MSAL redirectStartPage가 QR경로를 유지함을 검증한다.

현재 UI5194는 API5100(Debug 최신 QR)을 사용한다. 기존API5099의 합성3DB/provider 비활성 설정을 확인해 재사용하며 시드·startup migration은 비활성으로 기동했다. 기존5098/5099API 및 데이터 소유컨테이너는 유지했다. API PID /private/tmp/osan-qr-preview-5100.pid, frontend PID /private/tmp/osan-management-frontend-5194.pid. 로컬 QR주소는127.0.0.1:5194이며 실기기 조회는 해당 기기에서 접근 가능한 정식주소가 적용된 환경에서 검수한다. 공개배포·push·merge 없음.

추가 운영형 화면 확인: 개발용 React 갱신 스크립트는 엄격한 CSP와 충돌하므로 별도 빌드된 Dev 인증 화면에 공개 환경 CSP를 적용해 검증했다. 실제 QR 인쇄 프레임과2개 라벨 생성 성공. 공통 Teams SDK의 외부 validDomains 리소스만 기존 connect-src 정책으로 차단됨을 관측했으며 QR 기능에는 영향이 없고 정책을 넓히지 않았다. 이 검증은 실제 Entra/provider 호출 검증을 대체하지 않는다.

Backend Debug 테스트 프로젝트 포함 빌드 경고0/오류0 통과. 추가 Release solution 빌드는 API DLL 생성 후 테스트 어셈블리 단계가 장시간 무출력 상태여서 종료했으며 전체 Release solution 완료로 기록하지 않는다. 기능·권한 검증은 통과한 Debug 빌드와 실제3DB 테스트를 근거로 한다. 전체 제품 회귀와 실기기·실프린터 검수는 남아 있다.

### QR 발급·인쇄 단위 보정 (2026-09-10)

사용자가 프로젝트당1개/A4모아찍기를 정정하고 구현 승인했다. 발급 단위는 활성 진행 대상 패널1개이며 프로젝트상세 및 다중프로젝트 출력창에서 패널별 선택 가능. 각 QR payload는 /osan/qr/{projectId}/{targetId}, 스캔 시 프로젝트 정보/전체요약과 해당 패널의7단계·기록만 표시한다. 잘못되거나 비활성/다른프로젝트의 대상은 거부하며 다른 패널로 대체하지 않는다. 아직 배포하지 않은 프로젝트QR 이미지 endpoint는 패널QR endpoint로 대체한다. 30×30mm 또는50×50mm는 실제 용지 크기이며 QR1개당1페이지. QR2개면2장이다. QR·여백·기존3줄 내용은 한 장 안에 보존하고, 출력선택창에서 프로젝트명·패널명으로 라벨을 구분한다. 기존 A440/15개 페이지 정책은 이 승인으로 대체한다. 원격반영/공개배포는 이번 범위에 없다.

보정 구현·검증 완료, 사용자 검수 대기. 프런트 QR7개/사업부24개/인증25개 총56개 테스트와 타입검사·변경파일lint·제품빌드 통과(기존 큰 번들 경고 유지). Backend Debug 테스트프로젝트 및 Release API 빌드 경고0/오류0, 라우트/origin/실제QRdecode9개 및 disposable3DB HTTP통합1개 통과. 활성 패널2개의 서로 다른 정확한URL, 다른프로젝트/비활성/없는대상404, 기존401/403/200 경계 확인. 전용통합DB 정리 완료. 독립review 패널/프로젝트권한과1라벨1페이지 로직 P0–P2없음.

실제 합성UI에서 패널별 서로 다른경로 및 각7단계 확인, 프로젝트2개→패널3개 펼침→원하는2개 선택 출력 확인. PDF 각2페이지, MediaBox30mm 약84.96pt 정사각형/50mm 약142.08pt 정사각형(Chromium 반올림), A4없음. 각페이지300dpi렌더를 직접 열어 QR/3줄 잘림 없음 확인했고, 네 페이지 모두 ZXing으로 정확한 패널별URL 판독 성공. 증거 /private/tmp/osan-target-*.pdf/png. 실제용지·드라이버는 사용자 프린터에서30/50mm·배율100%·여백없음으로 검수해야 한다.

브라우저검증 중 모바일/태블릿 공통스타일이 재사용한 Excel선택 체크박스를 숨기는 문제를 발견하여 오산목록 QR선택만 표시하도록 범위를한정해 보정했다. 기존공통모바일 Excel숨김정책은 보존한다. 목록 내부8px높이 제목규칙의 인쇄팝업 침범도 height:auto로 보정했다. 소유API5100만 최신Debug 패널QR코드로 재기동했으며 기존합성데이터/다른runtime 보존, 시드·마이그레이션 미실행. 공개배포/push/merge 없음.

### 사용자 검수 완료 및 공개배포 승인 (2026-09-10)

사용자가 패널별 QR·개별 라벨 인쇄를 포함한 최종 후보67b9759의 사용자 검수 완료를 알렸고, 이어 공개배포 진행을 명시 승인했다. 승인된 범위의 원격 branch 게시·PR·main 병합·기존 Azure 수동 release를 진행한다. 과거 항목의 검수 대기/배포 미승인 표시는 당시 이력이며 현재 상태는 사용자 검수 완료·공개배포 승인·최종 자동 검증 대기다. 이번 승인에 CI/보호 규칙 우회는 포함되지 않는다.

최종 전체 회귀는 PR CI의 Backend·Frontend/UI·일반 Full-stack·사업부 접근·오산 전용 suite가 담당한다. required Gate 통과 후 main의 정확한 SHA를 배포하며, migration0094의 추가 테이블/뷰/활성 대상 필드 적용과 앱 교체·공개 보안 검사를 확인한다. 기존 운영 데이터·provider 설정은 보존한다. 배포 실행 결과는 이 기록에 이어 남긴다.

최종 CI34483521753에서 Frontend5건 실패 확인: 안내사진 자리표시자·QR선택 열에 대한 옛 기대값4건, 저장 성공과 승인정보 재조회 사이에 기존 사진수정 버튼이 잠시 나타나는 상태1건이다. 성공 응답 직후 승인 상태를 비워 즉시 재잠금하도록 보정하고 서버권한·실패시 같은 요청 재시도는 보존했다. 관련 사진5개/등록12개 로컬 테스트 통과. 독립 reviewer는 제품 보정 P0–P2 없음, 테스트 exact문구 P2는 부분문구 일치로 보정 후12개 통과로 해소했다. 실패한 CI를 통과로 보지 않고 수정 후보로 필수 전체 검증을 다시 진행한다.

CI34484013281에서 Frontend397개 단위테스트·lint/type/build 통과, mock UI15개 성공/오산목록1개 실패. 마지막 실패는 QR선택 체크박스 부재·8열을 기대하던 이전 브라우저 계약이었다. 모바일 QR선택 표시와9열·청주 데이터셀 비교를 승인된 QR선택 열을 구분하도록 갱신했다. 오산/청주 PC·모바일 등록/목록/상세, D-day, Excel3개 브라우저 시나리오가 로컬에서 모두 통과했다. 이 후속 보정은 테스트만 변경하며 제품코드는30f6960과 동일하다.

최종 CI34484850268: Frontend397개·mock UI16개·일반 Full-stack64개·사업부 접근1개·오산1개 통과. Backend619개 중617통과/2실패(고객사 옵션 정렬의 DB locale 차이, migration0094 신규관계 감사분류 누락)로 Gate 미통과이며 아직 병합·배포하지 않았다. 고객사 옵션은 Ordinal정렬 후 동일 집합을 검증하도록 보정했고 격리DB 집중 테스트1개 통과했다. 독립 검토에서 사진승인 요청 테이블은 가변 업무상태여서 단순 감사제외가 P2임을 확인했다. 신규0095에서 공통 감사 trigger를 추가하고, 관리이력/바이너리증빙만 기존 원장/append-only 증빙으로 분류한다. 관련 성공감사·동일저장 재시도 검증 후 CI를 재실행한다. 이 보정은 기존 감사 계약 누락을 복구하며 사진 바이너리를 공통 감사에 복제하지 않는다.

0095 보정 검증 완료: Debug API/tests 빌드 경고0오류0, 감사DB1개·실제3DB HTTP1개(27초)·AuditInfrastructure12개 통과. 실제 신청/승인/저장을 각각2회 호출해 성공 감사 action별1개/총3개를 확인했다. 신규trigger98개 및 migration최신0095 확인, 기존업무호환0086은보존. 독립 reviewer 최종 P0–P2없음, 감사누락P2 해소. 바이너리증빙은 공통감사복제없음·fingerprint원문은MetadataOnly로보존하지 않음. 최종CI는 이보정후 후보로 갱신한다.

### 최종 병합·공개배포 완료 (2026-09-11)

PR136 최종 head e6ece3e8205e097bbe6cb4afc7271f0d95ccefe3의 CI34489631395가 전부 통과했다. Backend619/619·Frontend397·mock UI16·일반 Full-stack64·사업부1·오산1 및 Workflow Validation/CI Gate 성공. 사용자 검수·공개배포 승인에 따라 main2a1e8dfd6cd846ee18c8e5f214aa807d6da1ffc9로 정상 merge했으며 head/main tree동일, main CI34493977084도 성공했다. 보호규칙 예외·우회 없음.

기존 Azure workflow34494010353로 exact main 배포 성공. migration0094/0095를 포함한 migration·Backend·Frontend·public security PASS. 기존bootstrap/backfill/provider설정변경 없음. Backend48/image sha256:c30beed5979cd8485909642cdd7105df50784097b8dcd6c2998447fe4211b41a, Frontend36/image sha256:a2d26f4ef312264af07ff1db52fb7e7d36615c4502944445d0a495d713364626. 두 앱 latest=ready, workflow Healthy/Running 확인. 공개 https://pms.emiinc.co.kr health200·익명API401 확인.

로그인된 공개사이트에서 D-day·상세 QR/관리자버튼, 패널1개→용지1장과30/50mm선택·QR이미지정상로드, 해당패널 QR직접경로의7단계·설명·작업자/완료일시·기존컬러사진로드(filter none)를 확인했다. 배포직후 기존세션의 상세조회401은 해당페이지새로고침의로그인확인후정상화됐고이후QR/사진조회성공. 검증용운영쓰기·사진교체·실물인쇄는 하지않았다. 실제프린터는 승인된용지크기/100%설정을따른다. 구현·자동검증·사용자검수·원격main·공개배포 완료. 이종료기록은 로컬문서커밋으로남긴다.

### 후속 기획 1 — 관리자·소속·권한 체계 확정 (2026-09-11)

사용자는 후속 오산 기능을 구현하기 전에 정책·화면·인앱 알림·메일 양식을 먼저 확정하도록 지시했고, 이번에는 아래 1번 체계를 명시 확정했다. 아래는 향후 구현 계약이며 앞의 공개배포 완료 범위에 포함되지 않는다. 제품 코드·운영 계정·DB·실제 발송·원격 반영·배포는 이번 정책 확정에서 변경하지 않는다.

- 일반 소속 캠퍼스는 청주 또는 오산 한 곳만 지정한다. 관리 권한은 소속과 분리하며 한 사람이 청주·오산 관리자 권한을 동시에 가질 수 있다. 관리자 선택만으로 청주 소속을 자동 지정하지 않는다.
- 청주 관리자·오산 관리자·총괄 관리자의 지정·변경·해제는 박수빈 개발자만 가능하다. 다른 총괄 관리자도 관리자 권한을 부여하거나 회수할 수 없다. 화면과 서버 모두 동일하게 제한한다. 실제 구현에서는 표시 이름이 아닌 확인된 계정 식별자로 이 권한을 연결한다.
- 청주 부서장은 청주 관리자 또는 총괄 관리자가 지정·변경·해제한다. 오산 부서장은 오산 관리자 또는 총괄 관리자가 지정·변경·해제한다. 박수빈 외의 총괄 관리자도 부서장 지정이 가능하다.
- 담당 캠퍼스 관리자는 해당 캠퍼스 사용자의 소속·일반 업무 권한을 관리할 수 있고, 총괄 관리자는 양쪽을 관리한다. 이 권한은 관리자 지정 권한을 포함하지 않는다.
- 관리자 메뉴는 청주 관리·오산 관리로 구분한다. 양쪽 관리자 및 총괄 관리자는 두 관리 범위에 접근할 수 있다. 관리 대상 캠퍼스를 명확히 표시하며, 관리 메뉴 전환이 본인 소속 변경을 뜻하지 않는다. 관리자 권한 설정은 박수빈에게만 제공한다. 구체적인 화면 디자인은 별도 기획 단계에서 확정한다.
- 최초 운영 전환 시 기존 관리자 계정을 모두 총괄 관리자로 전환하고 사용자가 이후 정리한다. 적용 전 실제 전환 대상 계정 목록을 확인·제시한다. 이번에는 전환하지 않는다.
- 개발·디버깅 기간에는 총괄 관리자가 청주·오산 관리자에게 가는 모든 인앱·메일 알림을 수신한다. 같은 사건의 수신 대상이 겹쳐도 중복 발송하지 않는다. 향후 전체 알림 수신 해제가 가능하도록 관리자 권한과 별도 구분한다.

현재 상태: 정책 1 확정·문서 정합성 확인 완료. 다음 기획은 단계 상태·반려·수정 승인·초기화의 처리 규칙이다. 사용자와 필요한 양식을 모두 확정한 후 구현으로 진행한다.

### 후속 기획 2 — 단계 상태·반려·수정 승인·초기화 확정 (2026-09-11)

사용자의 후속 답변으로 아래 처리 규칙을 확정한다. 앞의 배포 구현과 구분되는 기획 계약이며 제품 구현·운영 변경은 아직 하지 않는다.

- 완료된 대상·단계는 각각 독립된 건으로 관리한다. 앞 단계가 반려되어도 이미 완료된 후속 단계의 기록과 완료 상태를 유지하며, 앞·뒤 단계 각각의 반려 재등록 또는 수정 승인 조건에 따라 둘 다 수정할 수 있다. 최초 미완료 단계의 신규 완료에는 기존 선행 단계 완료 조건을 유지한다.
- 반려·초기화는 담당 캠퍼스 관리자 또는 총괄 관리자만 가능하며 사유를 입력한다. 선택한 대상·단계만 되돌리고 기존 사진·코멘트·처리 이력을 보존한다. 반려 또는 수정 승인으로 열린 단계는 해당 프로젝트의 진행 등록 권한이 있는 누구나 1회 저장할 수 있고 저장 후 다시 잠긴다.
- 수정 승인 상태에서는 수정과 관리자 초기화가 가능하고 반려는 불가능하다. 일반 등록자는 수정만, 관리자에게는 초기화도 표시한다. 동일 제한을 서버에서 적용한다. 초기화 시 기존 수정 승인을 종료하며, 초기화 전에 열린 수정 화면이나 승인을 사용한 저장은 거부하고 최신 상태를 다시 확인하도록 한다. 앞서 제안한 ‘수정 승인 중 초기화도 불가’ 기준은 이 사용자 결정으로 대체한다.
- 반려 알림은 해당 대상·단계의 최초 등록자 및 이후 사진·코멘트 수정 등록자 모두에게 인앱·메일로 보낸다. 같은 사용자는 한 번만 수신한다. 관리자 승인·반려 처리만 하고 완료/수정 데이터를 등록하지 않은 사람을 이 등록자 집합에 자동 포함하지 않는다.
- 프로젝트 완료 알림은 해당 프로젝트의 최초 완료 시 한 번만 보낸다. 반려·초기화 후 재완료되더라도 프로젝트 완료 알림은 다시 보내지 않는다. 재등록 시 단계 완료·수정 완료 알림은 기존 확정 대상에게 계속 발송한다. 초기화는 알림 없이 담당자·시각·사유와 이전 기록을 이력에 남긴다.
- 반려·초기화 시 해당 단계와 프로젝트 진행률/상태를 다시 계산한다. 완료 프로젝트에 미완료 단계가 생기면 진행 중으로 전환하고 모든 대상의 7단계가 다시 완료되면 자동 완료한다. 재완료 알림 금지 여부와 프로젝트 완료 상태 계산은 별개다.

현재 상태: 기획 1·2 확정. 다음은 사진·코멘트 입력 기준 및 현재 기록/이력의 표시 기준 확정이다. 이후 화면·인앱·메일 시안과 발송 규칙을 모두 정한 뒤 구현한다.

### 후속 기획 3 — 사진·코멘트·현재 기록·이력 확정 (2026-09-11)

사용자 답변으로 아래 입력·표시 기준을 확정한다. 제품 변경은 화면·알림·메일 등 사전 기획을 마친 뒤 진행한다.

- 대상·단계의 완료 및 수정 저장에는 사진이 필수이며 최종 사진 묶음은 최소 1장, 최대 10장이다. 수정할 때 유지한 기존 사진과 새로 추가한 사진을 합산한다. 이전의 선택 첨부 및 최대 5장 기준은 향후 구현에서 이 기준으로 대체한다. 기존 사진 없는 과거 완료 기록은 보존하며 소급 변경하지 않는다.
- 코멘트는 선택 입력으로 0~1000자이며 수정할 수 있다. 사진만 저장할 수 있지만 코멘트만으로 사진 없는 완료·수정 저장은 할 수 없다.
- 수정 시 기존 사진을 유지하면서 일부 제외하거나 새 사진을 추가할 수 있다. 제외·교체 전 사진과 이전 코멘트는 해당 저장 이력에 보존한다. 수정 가능 여부와 1회 저장 후 잠금은 기획 2의 권한·상태 계약을 따른다.
- 현재 기록에는 최종 저장된 사진·코멘트와 해당 저장의 최종 등록자·시간을 함께 표시한다. 기존 사진을 유지한 코멘트 수정도 새 저장 건이며, 최초 완료자의 이름·시간을 최신 기록의 등록 정보로 계속 표시하지 않는다.
- 이력은 저장 건마다 당시 사진·코멘트·등록자·시간을 묶어 조회한다. 반려·승인·초기화 기록도 시간순으로 함께 조회하며 각각 처리한 사용자·시간과 해당 사유를 표시한다. 과거 저장 묶음은 이후 편집으로 덮어쓰지 않는다.
- 사진 장수는 10장으로 확정했다. 현재 서버의 장당 5MiB·전체 15MiB 제한과 형식/모바일 처리 기준은 장수 변경과 별개이며 이번 답변을 용량 확대 승인으로 해석하지 않는다. 용량 기준 변경 필요 여부는 구현 전에 정리한다.

현재 상태: 기획 1·2·3 정책 확정 및 문서 정합성 확인. 다음은 PC·모바일·태블릿의 단계 입력/상세/이력과 관리자 화면 디자인이며, 인앱 알림·메일 양식 및 남은 세부 기준까지 확정한 뒤 구현한다. 이번 변경은 문서만이며 제품 코드·운영 DB·발송·원격 반영·배포는 변경하지 않았다.

### 후속 기획 4 — 단계 화면 시안 확정 (2026-09-11)

사용자는 사진 영역을 확대한 3차 시안(http://127.0.0.1:5203/?revision=3)을 확정했다. [승인 시안 보존본](../docs/design/osan-stage-review/revision-3.html)을 단계 상세·등록/수정·이력 화면의 구현 기준으로 사용한다. 캠퍼스 관리자 화면은 이번 확정 범위에 포함되지 않는다.

- 실제 사이트의 공통 스타일·EMI 로고·왼쪽 메뉴·7단계 카드와 연결한다. 모바일은 기존 로고와 좌우 단계 이동을 이어간다.
- PC 상세는 왼쪽 단계 설명, 오른쪽 큰 사진 영역으로 배치한다. 기본 현재 기록 화면은 세로 스크롤 없이 표시하고 사진 선택 및 코멘트를 함께 제공한다. 긴 코멘트 전체 보기와 누적 이력은 별도 보기로 제공한다.
- 모바일도 사진 기본 표시 크기를 확대한다. 사진 클릭 시 화면을 채우는 확대 보기를 열고 확대 상태에서 이전·다음 사진을 전환한다. 실제 사진은 원본 비율과 색상을 보존한다.
- 최종 저장 기록과 저장·반려·승인·초기화 이력 및 상태별 버튼은 기획 1~3의 확정 계약을 따른다. 시안의 예시 데이터·일러스트·역할/상태 전환 도구·가상 저장 동작은 제품 기능이 아니다.
- 시안 확인: 1366×768 PC 현재 기록에서 본문 높이와 내용 높이 일치(490px), 사진 높이 337px; 확대 열기·닫기 및 사진 전환 버튼 확인. 앞 시안에서 PC·모바일 배치를 직접 확인했고, 실제 제품 구현 후 화면 폭·상태별 검증은 별도로 수행한다.

현재 상태: 단계 화면 시안 사용자 확정. 다음은 캠퍼스별 관리자 메뉴 시안이며 이후 인앱 알림·메일 양식과 남은 세부 기준을 정한다. 이번에는 승인 시안과 기획 기록만 보존하며 제품 구현·운영 변경·원격 반영·공개배포는 하지 않는다.

### 후속 기획 4 정정 — 3차 시안 확정 취소 및 4차 재검토

사용자가 앞의 3차 시안 확정을 명시적으로 취소했다. 위 승인 기록과 revision-3.html은 과거 이력으로만 보존하며 현재 구현 기준으로 사용하지 않는다. 기획 1~3의 업무 정책은 유지한다.

현재 요청은 기존 진행 상세 디자인·상하 배치·모바일 단계 이동에서 크게 벗어나지 않는 재시안이다. 바뀌지 않는 홈·메뉴·프로젝트 정보는 시안에서 제외하고, 단계 상세·사진/코멘트 등록 및 수정·이력 화면만 제시한다. 4차 시안은 http://127.0.0.1:5203/?revision=4 에서 사용자 검토 중이며 아직 확정되지 않았다. 단계 설명 접기/펼치기 등 시안의 제안도 승인 전이다. 제품 구현·배포는 진행하지 않았다.

### 후속 디자인 범위 정정 및 오산 인앱 알림 시안 확정

사용자는 기존 PMS 디자인을 유지하고 다음 변경만 적용하도록 범위를 정정했다. 청주 관리자 메뉴는 유지하고 오산 관리자 메뉴만 신설한다. 오산 알림은 청주와 동일한 기능을 오산 디자인 규칙으로 제공한다. 기존 단계 상세에는 반려·초기화 버튼, 코멘트, 사진 최대 10장/부분 수정에 필요한 기능만 추가한다. 최종 등록 정보는 데이터만 변경하고 수정 승인 후 누구나 1회 수정은 동작만 변경한다. PC 팝업은 기존 배치·디자인을 유지하며 요소 크기만 조정한다. 이력 조회 페이지는 새로 만들고, 메일은 양식 내용을 정한다. 앞의 4차 단계 시안도 확정된 구현 기준이 아니며 새로운 탭·좌우 분할·단계 설명 접기를 임의 적용하지 않는다.

사용자는 오산 인앱 알림 시안(http://127.0.0.1:5203/?design=notifications)을 명시 확정했다. [확정 알림 시안 보존본](../docs/design/osan-notifications/approved-preview.html)을 목록·상세의 화면 기준으로 사용한다.

- 청주 알림의 읽음 상태 필터, 프로젝트별 묶음, 개별/프로젝트별/전체 읽음, 상세 및 관련 화면 이동 구성을 오산 페이지 스타일로 제공한다. 변경되지 않는 공통 메뉴·홈은 시안에서 제외했다.
- PC 목록과 모바일 상세를 직접 확인했다. 예시 알림의 상세 전환 및 읽음 상태 변경을 확인했다.
- 예시 데이터·가상 읽음 동작은 제품 구현이 아니다. 시안의 알림 설정·선택 내보내기·관련 페이지 이동은 안내만 제공하며 실제 동작의 구현/검증 완료를 뜻하지 않는다. 알림 발송 대상과 정책은 기존 확정 계약을 유지한다. 긴급/차단 KPI는 청주 기능 구성에서 가져온 표시이며 신규 긴급 분류 정책을 확정한 것은 아니다.

현재 상태: 오산 인앱 알림 목록·상세 디자인 확정. 다음 디자인 대상은 청주 관리자 메뉴를 유지한 오산 관리자 페이지이며, 단계 이력 조회 및 기존 화면의 최소 추가 범위와 메일 내용을 이어서 정한다. 제품 코드·운영 데이터·실제 발송·원격 반영·공개배포는 변경하지 않았다.

### 범위 조정 — 관리자 페이지 후속 이관

사용자는 관리자 페이지를 이번 기획에서 제외하고 다음에 진행하도록 지시했다. 오산 관리자 목록·사용자 수정·가입 승인·관리자 지정 화면 시안은 미확정이며 이번 구현 대상으로 사용하지 않는다. 관리자 페이지 신설·재설계는 후속 작업이다. 기존 청주 관리자 화면은 유지한다.

캠퍼스 관리자 구분·관리자 지정 권한·알림 수신 대상에 대한 앞선 업무 정책은 이번 화면 제외 요청으로 폐기하지 않는다. 실제 알림 구현에 필요한 관리자 식별 및 계정 전환 방법은 화면 신설과 구분해서 확인하고, 운영 계정 변경을 이번 지시만으로 실행하지 않는다.

현재 상태: 오산 알림 화면 확정, 관리자 페이지 기획 보류. 다음은 단계 이력 조회 페이지이며, 기존 단계 화면의 최소 추가 요소 및 메일 내용·남은 발송 세부 기준을 정한다. 제품 구현·운영 변경·원격 반영·배포 없음.

### 단계 조회 최소 변경 — PC 설명/사진 좌우 분리

사용자는 기존 진행 단계 조회 페이지에서 수정해 나가며 우선 왼쪽 설명·오른쪽 사진으로 구분만 하도록 요청했다. frontend/src/osan-progress.css에서 861px 이상 기존 단계 팝업을 2열로 배치했다. 컴포넌트·사진 크기·버튼·색상·모바일 스타일은 변경하지 않았다. 관련 OsanProgressPage 테스트 17개 통과. 기존 로컬 PMS 5194는 실행 모드 조회 오류로 실데이터 화면 확인이 불가능했다. 실제 컴포넌트와 현재 CSS를 예시 데이터에 연결한 격리 확인 화면(5203/?view=pms-split)에서 PC 좌우 배치를 직접 확인했다. 임의 재작성 시안이 아니며 실제 데이터/API 변경은 없다. 팝업 스크롤과 크기는 이번 요청에 따라 추가 조정하지 않았다. 사용자 확인 대기, 원격 반영·배포 없음.

### PC 단계 조회 배치 재조정 — 설명 접기/펼치기 + 사진/코멘트

사용자는 PC에서 등록보다 조회에 집중하도록, 최상단 한 줄의 단계 설명 펼치기/접기와 그 아래 왼쪽 사진·오른쪽 코멘트 구성을 요청했다. 기존 컴포넌트 기반 확인 화면(5203/?view=pms-lookup)에 적용하고 1366×900에서 배치 및 설명 펼치기/접기를 확인했다. 최종 등록 정보와 이력 버튼은 코멘트 아래에 배치했다. 이번 재구성은 예시 데이터 확인 화면이며 실제 API 계약·운영 데이터·공개배포는 변경하지 않았다. 모바일 배치는 유지한다. 이전 설명/사진 2열 CSS 변경은 중간 WIP로 남아 있으며 최종 구현 시 최신 배치 결정과 함께 정리해야 한다. 사용자 확인 대기.

### 조회 화면 보정 — 사진 확대·팝업 바깥 닫기·코멘트 입력 시안

사용자가 PC·모바일 사진 클릭 확대, 팝업 바깥 클릭 닫기 및 코멘트 입력을 요청했다. 기존 컴포넌트 기반 격리 확인 화면(5203/?view=pms-lookup-v2)에 사진 확대, 바깥 클릭과 Escape/닫기, 완료 창·수정 영역의 선택 코멘트(최대 1000자, 글자 수)를 추가했다. PC 수정 입력 8자 및 모바일 완료 입력 9자, 양쪽 사진 확대와 바깥 클릭 닫기를 직접 확인했다. 바깥 클릭 시 기존 React 닫기 상태도 갱신하도록 cancel 이벤트를 전달한다. 실제 컴포넌트 원본에 코멘트/API 필드를 추가한 것은 아니며 미리보기 빌드에서만 화면을 확장했다. 기존 사진 수/필수 여부 문구와 저장 동작은 아직 구현 전 기존 기준이 남아 있어 최종 제품 구현 시 확정된 최소 1장·최대 10장 정책으로 함께 변경해야 한다. 실데이터 변경·공개배포 없음.

### 사진 장수 정책 정정 — 기존 최대 5장 유지

사용자는 사진 장수 정책을 기존대로 유지하도록 지시했다. 앞의 최대 10장 확대 계획을 취소하고 현재 코드의 최대 5장을 유지한다. 장수에 대한 정정이며 앞서 확정한 필수 사진 첨부, 선택 코멘트 0~1000자, 기존 사진 유지·일부 제외·추가, 이전 저장 이력 보존 기준은 유지한다. 최종 저장 묶음은 기존 유지 사진과 새 사진 합계 최소 1장·최대 5장이다. 장당 5MiB·전체 15MiB 제한도 그대로 유지한다. 과거 문서와 시안의 최대 10장 표시는 이 최신 결정으로 대체하며 제품 구현 때 5장으로 일치시킨다. 이번에는 정책 기록만 변경했고 운영 데이터·원격 반영·배포는 하지 않았다.

### 다중 사진 조회 시안 — 한 장씩 넘기기

사용자는 등록 사진을 세로로 모두 나열하지 않고 넘겨서 조회하도록 요청했다. 기존 컴포넌트 기반 예시 화면에 사진 5장을 유지하고 한 번에 1장, 이전·다음 버튼 및 현재 장수(예: 2 / 5)를 표시하도록 변경했다. PC·모바일 전환과 현재 사진 확대를 직접 확인했다. 기존 코멘트·이력·바깥 클릭 닫기는 유지하며 제품 코드/API·운영 데이터 변경·배포는 없다. 시안: 5203/?view=pms-photo-carousel.

### 사진 조회 시안 보정 — 이름 제거·모바일 버튼/스와이프·처리 버튼 위치

사용자 요청에 따라 임의로 붙인 사진별 이름을 제거하고 순서/전체 장수만 표시한다. 크게 보기 버튼은 사진 아래로 분리하며 사진 클릭 확대도 유지한다. 모바일은 가로 스크롤(스와이프)과 이전·다음 버튼을 모두 제공한다. 사진 수정·수정 요청·승인·반려·초기화 버튼은 권한/상태에 따라 이력 보기 옆 같은 영역에 표시한다. 수정 승인 상태에는 반려를 표시하지 않고 초기화는 유지한다.

기존 컴포넌트 기반 격리 시안 5203/?view=pms-gallery-v3에 반영했다. PC 1366×900 배치와 모바일 390×844를 직접 확인했고, 모바일 크기에서 가로 스크롤로 1→2, 다음 버튼으로 2→3, 이전 버튼으로 3→2 전환을 확인했다. 실제 휴대전화 터치 검증은 아직 하지 않았다. 예시 사진/사용자 및 미리보기 변환에 적용한 상태이며 제품 API 저장·알림·권한 구현 완료를 뜻하지 않는다. 원격 반영·공개배포 없음.

### PC 사진 조회 시안 — 최대 5장 동시 표시

사용자는 PC 사진 영역을 넓히고 5장을 모두 표시하며 개별 클릭 확대를 요청했다. 기존 컴포넌트 기반 격리 시안에 PC 사진/코멘트 폭을 2.5:1로 조정하고 사진을 3열 2행으로 배치했다. 모바일은 한 장씩 버튼/가로 스크롤을 유지한다. 미리보기에서 사진 DOM을 이동하던 방식을 React 내의 사진 목록 래퍼로 보정해 화면 폭 전환 시 오류도 수정했다.

1366×900에서 사진 5장 전체가 보이는 배치와 다섯 번째 사진 확대를 직접 확인했다. 390×844로 전환 후 정상 렌더링 및 다음 버튼 1→2 전환을 확인했다. 현재 시안: 5203/?view=pms-pc-gallery-v2. 제품 API·실데이터·원격 반영·배포 변경 없음.

### 현재 확정 기준 — 단계 조회 시안 사용자 승인 (2026-09-11)

사용자가 pms-pc-gallery-v2 시안을 “이번 시안 확정”으로 승인했다. [승인 시안 보존본](../docs/design/osan-stage-lookup/approved-preview.html)을 현재 단계 조회 디자인 기준으로 사용한다. 앞선 취소된 시안과 중간 배치 기록은 과거 이력이며 이 기준을 덮어쓰지 않는다.

- PC: 최상단 단계 설명 접기/펼치기, 넓은 왼쪽 사진 영역(최대 5장, 3열 2행), 오른쪽 코멘트·최종 등록 정보.
- 모바일: 한 장씩 이전·다음 버튼과 좌우 스와이프. 사진별 임의 이름 없음.
- PC·모바일 사진 클릭 확대 및 확대 팝업 바깥 클릭 닫기. 모바일 크게 보기 버튼은 사진 밖에 표시.
- 이력 보기 옆에 상태·권한별 처리 버튼을 배치하며 코멘트 입력은 선택, 최대 1000자. 사진 장수는 기존 최대 5장 유지.

이번 승인은 조회 시안의 디자인 확정이다. 예시 데이터와 가상 이력/처리 동작은 실제 저장·권한·알림 구현 완료가 아니다. 관리자 페이지는 이번 범위 제외를 유지한다. 남은 사전 기획은 실제 저장 이력의 상세 표현과 메일 양식·발송 세부 기준이며, 제품 통합 구현·검증·원격 반영·배포는 별도 상태로 진행한다. 이번에는 승인본 및 Task 기록만 보존했다.

### 저장 이력 상세 시안 — 검토 대기

사용자의 요청에 따라 기존 이력 보기 팝업을 최신순 기록 목록으로 구성했다. 저장 건을 펼치면 당시 사진·코멘트·등록자·시간을 함께 표시하며 반려·수정 승인·초기화는 처리자·시간·사유/처리 내용을 표시한다. 최신 저장 건은 기본 펼침, 과거 건은 개별 펼침으로 구성하고 사진 클릭 확대를 유지한다. PC·390px 모바일의 기록 배치를 직접 확인했다. 예시 7건을 사용하며 과거 사진은 동일 합성 자산을 재사용한 시안용 자료다. 실제 이력 데이터/API 구현을 뜻하지 않는다. 확정된 조회 화면 보존본은 변경하지 않았으며 이력 시안은 5203/?view=pms-history에서 사용자 검토 대기다. 원격 반영·공개배포 없음.

### 저장 이력 상세 시안 사용자 확정 (2026-09-11)

사용자가 pms-history 시안을 “확정”으로 승인했다. [이력 상세 승인본](../docs/design/osan-stage-history/approved-preview.html)을 해당 화면의 구현 기준으로 보존한다. 바로 앞 기록의 검토 대기는 이 승인으로 종료한다.

확정 범위: 최신순 목록, 최신 저장 건 기본 펼침 및 과거 건 개별 펼침, 저장 당시 사진·코멘트·등록자·시간 묶음, 반려·승인·초기화 처리자·시간·사유/처리 내용, 개별 사진 확대, PC·모바일 배치. 합성 예시 기록과 과거 사진 재사용은 디자인 검토용이며 실제 제품에서는 저장 건별 원래 자료를 조회해야 한다.

현재 상태: 단계 조회·이력 상세·오산 인앱 알림 시안 확정. 관리자 페이지는 이번 범위 제외. 다음 사전 기획은 메일 알림 양식과 남은 발송 세부 기준이다. 이번에는 승인본과 기록만 보존했으며 실제 이력 API·알림 발송·원격 반영·공개배포는 진행하지 않았다.

### 메일 양식 사용자 확정 및 실발송 검증 보류 (2026-09-11)

사용자가 메일 양식을 확정했다. [확정 메일 양식](../docs/design/osan-mail/approved-templates.md)에 프로젝트 생성·단계 완료·반려·수정 완료·프로젝트 완료의 제목/본문/이동 버튼과 함께 제시한 공통 적용 기준을 보존한다.

사용자 승인으로 subin.park@emiinc.co.kr 단일 수신자에게 합성 단계 완료 테스트 메일을 시도했으나 기존 Gmail SMTP 인증에서 535 5.7.8로 거부되어 발송되지 않았다. 실제 프로젝트 상태·운영 설정 변경은 없었다. 사용자는 발송 설정을 다시 넣을 때 재시도하도록 보류했다. 자동 재시도하지 않으며 비밀번호를 문서에 보존하지 않는다.

현재 상태: 단계 조회·이력 상세·오산 인앱 알림·메일 양식 확정. 메일 실제 수신 검증은 보류이며 제품 통합 구현·검증·원격 반영·배포 완료가 아니다. 관리자 페이지 제외 유지. 이번 변경은 확정 양식과 기록 보존만 수행했다.

### 최종 추가 계약 — 초기화 집계 반영 및 관리자 코멘트 단독 저장

사용자가 최종 정책 정리를 수용한 뒤 아래 두 사항을 구현 범위에 추가했다. 이 기록은 앞의 사진 필수 규칙에 대한 관리자 예외를 포함한 최신 기준이다. 이번 요청은 구현 내용에 반영하는 것이며 제품 코드 실행·배포 완료를 의미하지 않는다.

1. 초기화는 해당 대상의 해당 단계 완료 상태를 미완료(0)로 되돌린다. 이전 사진·코멘트·등록자·시간과 초기화 처리자·시간·사유는 이력으로 보존하되 현재 완료 기록이나 완료 집계에 포함하지 않는다. 대상 및 프로젝트 진행률을 기존 집계 공식으로 재계산하여 낮추고 홈·프로젝트·진행 현황·상세·QR 조회의 집계를 일치시킨다. 다른 완료 단계는 유지하며 전 단계 초기화로 확대하지 않는다. 프로젝트 완료 조건이 깨지면 현재 완료 상태도 해제하고 실제 남은 완료 상태에 따라 재계산한다. 최초 프로젝트 완료 알림 발송 이력은 보존하여 재완료 시 재발송하지 않는다. 초기화 알림 없음과 기존 수정 승인 무효화는 유지한다.
2. 해당 오산 프로젝트에 관리자 권한이 있는 사용자는 각 단계의 완료·수정 저장에서 사진 없이 코멘트만으로 저장할 수 있다. 사진 0장이면 공백만으로 된 입력을 제외한 코멘트가 필수이며 최대 1000자다. 일반 사용자는 기존 최소 1장 사진 필수를 유지한다. 사진을 첨부하는 경우 기존 최대 5장·장당 5MiB·전체 15MiB 및 형식 제한을 유지하며 코멘트는 선택이다. 관리자 예외는 사진 필수 조건에만 적용하고 단계 순서·포장 선행조건·수정 승인 및 1회 저장 잠금 정책을 우회하지 않는다. 서버에서 관리자 권한과 입력 조건을 검증한다. 코멘트만 저장해도 정상 완료/수정으로 집계하며 현재 등록자·시간, 저장 이력과 해당 알림에 반영한다. 사진 없는 완료 기록에는 업로드 대기처럼 보이지 않도록 사진 미첨부 상태와 저장 코멘트를 표시한다.

직접 검증 범위: 초기화 후 해당 단계 0 및 집계 감소/프로젝트 완료 해제, 후속 완료 단계 보존, 과거 자료 조회, 재완료 알림 중복 방지, 관리자 코멘트 단독 저장 성공, 관리자 사진·코멘트 모두 없음 거부, 일반 사용자 사진 없음 거부, 권한 없는 사용자의 예외 사용 거부.

사용자는 직전 인앱 알림 문구안과 최종 정리를 수용했다. 인앱 5종(프로젝트 생성·단계 완료·반려·수정 완료·프로젝트 완료)의 제목/본문, 목록·상세 표시 및 이동 기준도 확정 상태다. 관리자 페이지 제외와 메일 실발송 검증 보류는 유지한다. 이번에는 정책 기록만 갱신했다.

### 실제 구현 착수 (2026-09-11)

사용자 “시작해”로 확정 정책/시안의 제품 구현·관련 검증을 승인했다. codex/osan-project-management에서 origin/main 8dd2a42를 fetch 후 로컬 merge하여 최신 로그인·오산 부서별 입력 권한을 보존했다. 원본 checkout WIP와 기존 runtime은 변경하지 않았다. root가 frontend/연결을 담당하고 독립 backend 단계·알림 구현을 위임했다.

진행 중: additive 0096 단계 저장 스냅샷/관리 이벤트 및 0097 오산 알림, comment/사진부분수정/관리자예외/반려·초기화 및 1회수정, 공용 알림 연결과 승인된 화면 적용. 프런트 단계·사업부 관련 49개 및 추가 정책 4개 통과, typecheck 통과. 전체 작업 검증 완료는 아니며 backend 실제DB/권한/동시성/알림 집중검증과 독립review 및 실제 화면 확인이 남아 있다.

기존 기본5432 인증 불일치로 테스트가 데이터생성 전 실패하여, 작업 전용 tmpfs PostgreSQL 컨테이너 emi-osan-stage-20260911(127.0.0.1:55496)을 생성했다. 테스트별 고유 합성 DB만 사용한다. 운영DB·계정변경·외부발송·원격push/merge·공개배포는 실행하지 않는다. 메일 실발송 검증 보류 유지.

### 제품 구현 및 사용자 검수 대기 (2026-09-11)

확정된 진행단계 조회·코멘트·사진 부분 수정·공유 1회 수정·관리자 반려/초기화·저장 이력·오산 인앱/메일 알림을 실제 제품에 연결했다. 0096/0097은 추가 migration이며 운영 DB에는 적용하지 않았다. 총괄 관리자 반려 알림은 Directory의 활성 지정과 identity를 조회하고 오산의 활성 사용자 경계와 함께 확인한다. 관리자 페이지와 실제 관리자 계정 전환은 이번 범위에서 제외한다.

검증: frontend 진행/사업부 49개 및 추가 기록 정책 4개 통과. 마지막 화면 보정 후 단계/기록 23개 재검증 통과, TypeScript·대상 ESLint·backend Release build 통과. 전용 PostgreSQL에서 단계 저장/초기화/반려/공유 승인/경쟁 요청/기록 보존 관련 4개, 알림 관련 11개 및 전체 ThreeDatabaseBoundary HTTP 시나리오 통과. 해당 HTTP 시나리오는 비기여 총괄 관리자 포함, 비활성 지정·identity 및 일반 비기여 캠퍼스 관리자 제외, 기여자 전체 포함, 인앱/메일 수신자 일치를 검증한다. 독립 리뷰의 총괄 관리자 수신 누락 지적은 수정 후 재검토와 실제 DB 테스트로 해소했다.

실제 UI: PC 1366×900과 모바일 390×844에서 5장 갤러리/이전·다음/사진 확대·현재 코멘트·이력을 확인했다. 알림 목록에서 대상 단계로 직접 이동했고, 관리자 사진 없는 코멘트 완료 및 해당 단계만 초기화하는 동작을 실제 화면에서 실행했다. 초기화 후 이력 버튼을 숨기던 기존 CSS를 제거하고 초기화/이전 완료 이력이 표시됨을 재확인했다. 물리 모바일 카메라와 실제 손가락 스와이프는 아직 사용자 검수 대상이다.

검수 환경: http://127.0.0.1:5204/ → 최신 API 5112. source는 /private/tmp/emi-osan-photo-upload-fix, branch codex/osan-project-management. 작업 전용 tmpfs 3DB emi_qms_e2e_osan_stage_20260911_{directory,cheongju,osan}만 사용한다. API 5111과 시안 5203 및 원본 checkout은 보존했다. 메일/provider 발송 비활성화이며 실제 수신은 기존 인증 설정 재입력 이후 별도 검증한다. 예시 프로젝트 STAGE-REVIEW-20260911에는 입고검사 사진 5장, 배치검사 코멘트 완료 후 초기화 이력이 있다. dev-admin에서 사업부 오산 선택 시 관리 기능을 검수할 수 있다.

다음: 사용자 검수 → 보정이 있으면 영향 범위 검증 → 최종 후보 전체 회귀 및 required CI → 해당 원격 반영/배포 명시 승인. 이번에는 로컬 구현·관련 검증만 수행했으며 전체 회귀·사용자 검수·원격 반영·공개배포 완료로 간주하지 않는다.

### 사용자 검수 지적 — 알림 메뉴 위치 및 시안 불일치 (2026-09-11)

오산 알림 메뉴를 ‘내 업무’의 홈 바로 아래로 이동했다. 알림 설정에서도 같은 메뉴가 활성화되게 맞췄다. 실제 Chrome의 5204 화면에서 메뉴 순서를 확인했다.

사용자 지적으로 승인 알림 HTML과 실제 공용 Notifications 화면을 대조했다. 승인본은 제목/설명, 읽지 않음 우선 필터, 프로젝트별 알림 행(제목·본문·유형·시간·버튼)이며 현재 PC 구현은 기존 공용 표의 별도 열과 큰 내보내기 도구 영역을 표시한다. 설명 누락, 필터 순서 및 배치·간격 차이가 남는다. 앞 기록의 ‘확정된 화면 적용’은 알림 시안 동일성 완료를 의미하지 않으며 이 부분을 정정한다. 이번 요청에서는 메뉴 위치를 수정하고 차이를 확인했다. 알림 목록·상세의 승인 시안 보정은 미완료로 추적한다. 신규 기능/메일 정책/관리자 페이지 변경 및 원격 반영·배포 없음.

### 알림 승인 시안 보정 완료 (2026-09-11)

사용자가 확정 시안으로 수정을 지시했다. 실제 공용 NotificationsPage의 조회·읽음·그룹·선택·내보내기 동작을 유지하고 오산에만 승인본 표현을 적용했다. 알림 제목/설명, 2개 요약 카드, 읽지 않음→전체→읽음 필터, 간결한 선택 영역, 프로젝트별 알림 행, 제목·본문·유형·시간·동작 버튼, 알림 상세의 프로젝트/처리자/본문/관련 화면 이동을 보정했다. 청주의 기존 화면과 알림 설정 페이지는 변경하지 않았다. 모바일은 승인본의 카드 높이62px, 글자10/20px 및 필터/그룹 여백을 적용했고 공용 모바일 체크박스 숨김 스타일이 행을 좁히는 문제를 오산 범위에서 보정했다.

기존 오산 알림 내보내기가 일반 export 경로 차단에 걸리는 연결 누락도 보정했다. POST /api/notifications/export는 화면을 notifications로 고정하며 다른 screen값/타인/타사업부/혼합/필터불일치 선택을 거부한다. 일반 export 경로는 그대로 차단하고 기존 Excel service·사용자 조회 범위·감사 기록을 재사용한다. 새 업무 데이터/수신 정책/운영 권한 부여는 없다.

검증: 사업부/알림 UI 25개와 선택 export 4개 통과, TypeScript/대상 ESLint/Release build 통과. 실제 분리 3DB HTTP 알림 export 시나리오 1개가 파일내용·행수·헤더·수신권한·사업부격리·screen조작 거부·성공 감사 기록을 확인했다. 독립 리뷰에서 발견한 프로젝트 일괄 읽음 오류 피드백 누락을 수정하고 실패 응답 테스트를 추가했다. 재검토 후 미해결 P0–P2 없음.

직접 확인: 승인 HTML을 로컬5206에서 열어 비교했고 실제5204에서 PC1366×900/모바일390×844 목록·상세·필터·메뉴 위치 및 가로넘침 없음을 확인했다. 합성 알림 한 건의 선택 내보내기에서 파일 생성 성공 메시지를 확인했다. 첫 실제내보내기 실행은 검수 API contentRoot가 저장소루트여서 동봉폰트를 찾지 못해 실패했고, 전용 실행스크립트의 contentRoot를 실제 API폴더로 정정 후 재실행 성공했다. 검수주소 http://127.0.0.1:5204/notifications, API5112, 기존합성DB 보존. 승인본 예시문구/데이터 대신 실제 저장된 알림내용을 표시한다. 외부 메일·원격push/merge·공개배포·운영DB 변경 없음. 사용자 재검수 대기.


### 실사용 검수 병행 및 최종 후보 회귀 (2026-09-11)

사용자가 “검수는 실제 사용하면서 하자. 다음 작업 시작하자”로 최종 후보 회귀 진행을 요청했다. 실사용 검수는 계속하며 사용자 검수 완료나 원격 push·병합·배포 승인으로 해석하지 않는다. 제품 기준은 `3d2f722`, branch `codex/osan-project-management`이다. 검수 서버 5204/API 5112와 기존 합성 DB는 보존한다.

Frontend 최초 결과: 전체 unit 최초 413/417 통과. 실패 4건은 구정책의 요청자 전용 편집 기대 2건과 테스트 환경의 dialog 메서드 누락 2건이다. 확정된 공유 1회 저장 정책과 별도 단계 이력 조회에 맞춰 테스트를 보정했으며 해당 2파일 17건 통과, 이전 사진·코멘트·처리자·초기화 사유를 조회하는 독립 테스트 1건을 추가해 통과했다. 이 테스트 보정에서는 제품 동작을 변경하지 않았다. 전체 모의 브라우저 16건, TypeScript, ESLint(오류 0, 기존 main.tsx 경고 1), CI 분류/게이트 검사와 Azure 정적 검사 통과. 실제 DB backend 전체 및 일반/사업부/오산 전용 브라우저 회귀 결과는 아래에 확정한다. 외부 메일 발송 검증과 원격 required CI는 이 로컬 회귀에 포함하지 않는다.


회귀 보정: 일반 브라우저 최초 64건 중 63건 통과/1건 실패. 청주 제조 양식의 초기 조회가 StrictMode에서 중복 실행되고 늦은 응답이 편집 중 입력을 조회 상태로 되돌리는 기존 결함을 확인했다. `ProductionControlTemplateWorkspace`에 조회 generation/cleanup을 적용하여 폐기된 조회와 이전 사용자·domain 응답을 무시한다. 저장 성공 후 의도한 편집 종료는 보존한다. 반례는 보정 전 실패를 확인한 뒤 보정 후 StrictMode·사용자 전환·domain 전환·정상 저장 4건 통과. root가 구현 담당자와 분리하여 diff/반례를 검토했고 미해결 P0–P2 없음. 실패했던 실제 브라우저 시나리오 1건 재검증 통과.

브라우저 최종: 일반 64개(최초 실패 1개 보정 후 통과), 사업부 접근 1개, 오산 등록 1개로 고유 66개 통과, skip 없음. 오산 등록 첫 시도는 새 unit의 잘못된 `exact` 옵션으로 TypeScript 빌드가 실패하여 제품 시나리오가 실행되지 않았다. 옵션 제거 후 새 임시 환경에서 build와 시나리오를 통과했다. 최종 frontend production build/TypeScript·변경 파일 lint 통과. frontend unit은 기존 417개 중 최초 실패 4개 보정 후 해당 17개 통과, 별도 이력 1개와 조회 경쟁 반례 4개 추가 통과로 최종 고유 422개를 검증했다. 전체 suite를 불필요하게 반복하지 않고 변경·실패 범위를 재확인했다.

환경 정리: 일반/사업부/오산/실패 재검증은 서로 다른 tmpfs DB·bounded role·포트만 사용했으며 각 환경의 DB/role/process/container/network cleanup을 확인했다. 회귀가 갱신한 117개 tracked 이미지와 1개 생성 Excel은 `/private/tmp/osan-final-regression-screenshots-20260911/manifest.json`에 따라 임시 증거로 보존한 뒤 원래 파일로 복구했고 신규 생성 이미지 5개도 임시 증거로 옮겼다. 사용자 WIP와 기존 검수 환경에는 변경하지 않았다.


Backend 전체 실행: 제품 backend 소스 `3d2f722`와 기존 Release assembly의 최신 상태를 확인한 뒤 전용 tmpfs `osan_stage_backend_final_20260911_01`에서 총 642건을 1회 실행했다. 최초 641건 통과/1건 실패/skip 0(49.2분). 유일한 실패는 `MigrationRelationCoverage_ExactlyClassifiesEverySchemaRelation`의 제외 relation 합계 기대값 61이었다. 0096/0097의 `osan_stage_records`·`osan_project_completion_notifications`는 이미 명시 분류돼 missing/stale/중복 검사를 통과했으며 합계만 63으로 보정했다. root가 두 분류와 diff를 독립 확인했다. 전체 실행의 DB·container·network 정리를 확인했다. 원본 결과는 `/private/tmp/osan-stage-backend-final-20260911-01/backend-full.trx`와 `run.log`, 실행 source/assembly 근거는 같은 폴더 `source-manifest.json`에 보존한다.


최종 확정: 제외 relation 합계 보정 후 Release 재빌드와 해당 실패 검사 1/1 재검증 통과. Backend 고유 642개, frontend unit 고유 422개, 모의 브라우저 16개, 실제 브라우저 고유 66개를 최초 결과와 필요한 재검증을 합쳐 확인했으며 미해결 실패와 skip은 없다. backend 제품 소스·migration 변경은 없다. 재빌드 전후 API DLL 해시는 동일하지 않아 바이너리 동일성을 주장하지 않으며, 초기 실행 manifest와 최종 source 기준을 구분한다. 최종 빌드·형식·타입·로컬 CI 정적 검사 및 필요한 독립 검토 완료. 검수 화면 5204와 API 5112는 최종 HTTP 200 응답을 확인했다.

현재 완료 범위는 로컬 구현·회귀·결함 보정이다. 사용자는 실사용 검수를 이어간다. 원격 push/PR/required CI/main 병합/공개배포/운영 DB 적용은 실행하지 않았다. 실제 메일 수신은 기존 발송 인증 설정 재입력 이후 별도 확인하며 물리 모바일 카메라·스와이프는 실사용 검수 대상으로 유지한다. 다음 원격 반영은 해당 범위 승인 후 원격 CI와 사용자 검수 상태를 확인하여 진행한다.

### 새 Gmail 인증 및 단일 테스트 발송 (2026-09-11)

사용자가 새 발신용 Gmail을 만들고 앱 비밀번호를 발급했다. 숨김 입력으로 받아 접근 제한된 임시 파일에서만 사용했으며 비밀번호를 코드·문서·출력에 기록하지 않았다. 새 계정의 SMTP 인증 성공을 확인하고, 승인된 `subin.park@emiinc.co.kr` 한 곳에 합성 진행단계 완료 메일 1건을 발송했다. 현재 제품의 `OsanNotificationTemplates`로 렌더링한 HTML을 사용하고 테스트임을 명시했다. 실제 업무·DB 상태는 변경하지 않았다.

최초 제품 MailKit 발송 시도는 로컬 .NET TLS 연결에서 `SslHandshakeException`이 발생했으며, 별도 발송 없는 인증 진단에서 연결 단계 실패를 재확인했다. 인증서 검증을 유지한 Python SSL 연결에서는 인증과 발송이 성공했다. 첫 시도 후 보낸편지함 0건을 확인한 뒤 발송했으며, SMTP 접수 성공과 최종 보낸편지함 1건을 확인했다. 따라서 새 계정 인증과 확정 HTML의 실제 전송은 확인했지만 Azure 운영의 제품 발송 경로 및 수신자 도착 확인까지 완료했다고 간주하지 않는다. 운영 Gmail 참조·발송 worker 설정·공개배포는 변경하지 않았다. 운영 계정 전환은 자동 업무 메일 발송 영향과 함께 별도 확정 후 적용한다.

### 운영 발신 계정 전환 (2026-09-11)

사용자가 공개 PMS의 발신 계정 변경 및 대기 업무 메일 발송 영향을 승인했다. 발신 주소와 SMTP 계정을 `emi.projectsystem@gmail.com`으로 전환했다. 기존 Key Vault 참조를 이전 명시 버전에 고정한 뒤 canonical 두 비밀의 신규 버전을 저장하고, 두 참조를 함께 신규 명시 버전으로 변경했다. 앱 비밀번호는 파일 입력으로 전달했으며 값은 출력하지 않았다. 독립 검토에서 앱 공통 비밀 범위·발신 주소·이전 서버 종료·롤백 조건을 확인했다.

기존 `backend--0000049`에서 같은 이미지 digest를 사용하는 설정 리비전 `backend--mail-20260911`을 만들었다. 신규 리비전 Healthy, 트래픽 100%, 이전 리비전이 활성 목록에서 제거된 것을 확인했다. 발신 주소/SMTP 사용자 모두 gmail-username, 비밀번호는 gmail-app-password 참조를 유지한다. 환경의 secretRef에 붙은 빈 value 필드 생략을 제외하고 컨테이너 설정은 동일하며 DB·권한·프로그램 이미지 변경은 없다. 공개 주소는 비인증 요청에 HTTP 401을 반환했다. 제한된 신규 서버 로그에 인증/TLS/오류 징후는 없었지만 실제 운영 업무 메일 발송 성공 증거는 아직 없다. 앞선 단일 테스트의 SMTP 접수·보낸편지함 확인과 운영 워커 경로의 발송 검증을 구분한다.

이전 비밀 버전은 복구용으로 보존했다. 롤백은 두 참조 복구와 서버 재기동이 함께 필요하다. 새 비밀번호의 로컬 임시 파일은 삭제했다. 이번 변경은 운영 메일 설정 전환이며 대기 중인 제품 코드의 push/main 병합/공개배포가 아니다. 다음 실사용 알림에서 운영 발송 및 수신 도착 확인이 남는다.

### 후속 기능 최종 검수 및 공개배포 승인 (2026-09-11)

사용자가 남은 확인을 모두 완료했으며 main 병합·공개배포를 모두 명시 승인했다. 후속 단계 기록·반려·초기화·사진/코멘트·이력·인앱/메일 알림과 회귀 보정을 원격 PR로 올리고 required CI 후 정상 병합한다. 운영은 기존 수동 release로 정확한 main의 additive 0096/0097 migration 및 Backend·Frontend를 배포한다. 관리자 페이지·관리자 체계 신규 전환은 제외하며 기존 권한/3DB/실데이터/새 Gmail과 provider 설정을 보존한다. 이전 1회 보호규칙 예외 승인은 재사용하지 않는다. 배포 결과는 실행 후 기록한다.

원격 PR139 최초 CI34574644216에서 frontend420/422 통과, 제조 양식 저장 후 GET이 이전 fixture를 반환하는 검사와 관리자 전환/채널 결과 로드를 기다리지 않는 Web Push 검사가 실패했다. 저장 이후 fixture를 갱신하고 계정 전환 및 실제 결과 행 로드를 기다리도록 테스트만 보정했다. 관련3파일101개·타입·lint 통과, 제품 추가 변경 없음. 후속 required CI로 확인한다.

배포 독립검토에서0096 snapshot 후 구 backend 저장이 새 현재기록에 빠질 수 있는 전환구간을 확인했다. 기존 frontend의 동일 이미지에 임시 기동 guard를 적용해 오산 POST/PUT/PATCH/DELETE만503으로 제한하고 기존 인증경계·조회·청주업무·health를 유지한다. guard는 replica재시작에도 유지되며 정상전환/구revision종료후 migration을 시작한다. 동일nginx digest/UID101에서 HTTP12개와재시작12개,기본기동해제를 검증했다. exactmain migration→backend/frontend Healthy후 원래기동설정으로복구한다. migration후 구backend로롤백되면 guard를유지하고 새기록정합성을보정한뒤해제한다. Gmail참조는image-only배포에서보존한다.

### 후속 main 병합 완료 및 배포 전 운영 제한 승인 차단 (2026-09-11)

PR139 head7dd34a92b27495111583ebdd5ee43c8c6847b4c2의 CI34575126932 최종 통과. Backend642·Frontend422·mock16·일반Full-stack64·사업부1·오산1 및 CI Gate 성공. 첫 통합 시도는 일반64/사업부1 성공 후 오산 검증 서버가 readiness 전 종료됐고 시작로그는 기존cleanup에서 삭제됐다. 제품변경 없이 임시진단을 넣은 로컬 오산1건이 성공했고 자원cleanup도 확인했다. 진단삽입은 원복했으며 실패한 원격job만 재실행해66건과Gate 성공을 확보했다. 최초시작실패의 세부원인은 확정하지 않았다.

사용자 승인에 따라 정상 main f96fa5490e452d9505a6e4f5a8d97c67ad64b7d5로 병합했고 후보/main tree 동일, main CI34579226825도 성공했다. 보호규칙 변경·예외 없음.

배포 전 임시 frontend 기동 guard 적용 명령은 실행 전에 자동 승인 심사에서 거부됐다. 확인된 이유는 공개배포 승인을 /api/osan의 모든 쓰기 요청을503으로 잠시 중단하는 구체적인 운영 조치의 승인으로 인정하지 않았기 때문이다. 우회·설정변경·동일행동 재시도 없음. guard·migration·image배포·workflow dispatch는 실행하지 않았고 Backend mail-20260911/Frontend38 latest=ready 유지 확인. 새 Gmail 설정과 실제 데이터는 보존된다. 사용자의 구체적인 오산 저장 임시중단·정상전환후재개 승인이 필요한 상태다. guard 원본과 동일기반 반례결과는 /private/tmp/osan-release-20260911/guard-review, 운영복구 metadata는 같은상위폴더 apps-before.json에 보존했다. 승인 후 exactmain 수동 release와 배포후검증을 이어간다.


### 공개배포 및 오산 저장 재개 완료 (2026-09-11)

이전 차단 기록 이후 사용자가 안전한 배포와 임시 오산 저장 제한·복구를 승인했고, 재개 요청에 따라 실행했다. main `f96fa5490e452d9505a6e4f5a8d97c67ad64b7d5`의 [배포 실행 34579964572](https://github.com/emisubin/emi-qms/actions/runs/34579964572)이 성공했다. 공식 release migration 및 Backend·Frontend·PublicSecurity 검증 PASS. database bootstrap·membership backfill·inspection은 실행하지 않았다. 운영 DB 초기화·삭제와 관리자 체계 전환은 없다.

이력 snapshot 전환 구간은 동일 frontend 이미지의 임시 guard로 오산 쓰기만 제한하고 구 revision 종료를 확인한 뒤 배포했다. Backend50 및 Frontend40 정상 전환 후 기본 기동 설정을 복구했다. 최초 CLI 빈 문자열 해제는 command/args에 빈 문자열 배열을 저장하여 Frontend41이 준비되지 않았다. 현재 설정의 나머지 필드를 유지하면서 command/args를 빈 배열로 복구했고 Frontend42만 active·Healthy, latest=ready를 확인했다. 존재하지 않는 오산 경로의 POST는 임시503 대신 정상 인증401을 반환했다. 실제 업무 데이터 생성 없이 제한 해제를 확인했다. 공개 health200·익명 api/me401 유지.

배포 전후 secret 참조와 유효 환경값을 대조하여 DB·Gmail/provider 설정 보존을 확인했다. 실제 로그인된 공개 화면에서 오산 알림 메뉴, 기존 프로젝트의 단계 완료 수와 상태, 단계 사진·원 등록자·시각, 이전 완료·수정요청·승인 이력을 조회했다. 이전 사진을 크게 열어 컬러 이미지 로드를 직접 확인했다. 이는 기존 프로젝트 표본의 읽기 검증이며 전체 운영 레코드 전수 대조나 새 업무 알림 발송 검증을 대신하지 않는다. 메일 수신을 포함한 사용자 최종 확인은 앞선 승인 기록을 따른다.

제품 main 병합·required CI·공개배포·임시 제한 복구 완료. 공개 주소 https://pms.emiinc.co.kr. 최종 배포 기록은 이 Task와 roadmap만 로컬 커밋하며 다른 WIP·검수 runtime은 보존한다.


### 오산 PWA 푸시 연결 보정 (2026-09-11, 구현 요청)

사용자가 운영 인앱/PWA 상태 확인 후 문제 해결을 요청했다. 운영 서버의 WebPush Enabled=true·DryRun=false와 키 설정을 확인했지만, 실제 오산 내 알림 설정은 business_unit_capability_disabled로 차단됐다. 기존 인앱 조회/메일 범위에 빠져 있던 오산 기기 등록 API·푸시 대기열 생성·worker의 명시적 사업부 DB target 전달을 보정한다. 기존 사업부 membership/승인/DB 분리·수신자 범위·등록 이후 알림만 전송·기기 세대와 중복 방지·메일 정책을 유지한다. 관리자 페이지는 추가하지 않는다. 실제 DB 검증에서 0097의 메일 전용 guard도 푸시 기록을 차단함을 확인하여 기존 migration을 수정하지 않고 후속 migration으로 승인된 오산 푸시 범위만 허용한다.

UI는 기존 기기 설정을 재사용하고 오산 첫 실행 안내를 연결한다. 최초 안내는 StrictMode에서도 정상 표시하며 캠퍼스별로 숨김 상태를 저장한다. 캠퍼스에서 기기를 끄면 해당 캠퍼스 서버 연결만 해제하여 다른 캠퍼스가 공유하는 브라우저 구독을 끊지 않는다. 로그아웃 시 기존 브라우저 구독 해제는 유지한다. 푸시 링크의 사업부를 최초 API 요청 전에 선택하고 접근 허용은 서버에서 계속 판단한다.

검증 및 최종 상태는 아래에 확정한다. 이번 보정의 원격 게시·병합·공개배포 및 실제 테스트 알림 발송은 아직 수행하지 않았다. 앞선 배포 승인·완료와 구분한다.


보정 완료: 0098_osan_web_push_delivery.sql은 기존 함수만 교체하여 INSERT 시 실제 오산 프로젝트·RecipientOnly 알림·수신자·활성/승인 사용자·본인 기기/세대/활성화 시각을 검증한다. 0097의 메일 허용식과 기존 데이터는 유지하며 후속 상태 UPDATE를 제한하지 않는다. background dispatcher는 오산에서 푸시만 생성하고 청주용 업무/digest planner를 실행하지 않는다. 구독 조회와 provider 결과 처리에 명시적 사업부 DB target을 전달한다.

검증: Backend Release build 오류/경고0, 관련15건 PASS/skip0(신규3DB통합·기존push·migration concurrency/ledger). 잘못된 경로/메서드·소속 철회·다른 사용자 기기 조회·수신자와 프로젝트/세대/채널 등6개 DB 위조 INSERT 거부, 현재 캠퍼스만 provider 성공/410 처리, 중복·재등록 후 과거알림 제외를 확인했다. 최초 합성 계정 소속 누락을 정정했고, 실제 0097 DB guard 차단은0098로 해결했다. 외부 provider는 fake, 전용 tmpfs DB/container/network cleanup 완료. Frontend 5파일60건 PASS, 타입·대상lint·build PASS(기존 bundle크기 경고). 실제 컴포넌트를 합성 설정으로 PC1440/mobile390에서 직접 보고 가로넘침 없음을 확인했다. 확인용 임시 HTML·서버는 정리했다.

작성자와 분리된 gpt-6-astra/high 리뷰에서 초기 diff 및0098 추가분 모두 미해결 P0–P2 없음(요청 모델 기준). 실행 근거는 /private/tmp/osan-push-backend-tests.log, osan-push-frontend3.log, osan-push-api-tests.log와 type/lint/build 로그에 보존한다. 구현·직접 검증·독립 리뷰 완료, 사용자 검수·이번 수정본의 원격 병합/공개배포는 미실행. 운영 적용 시0098과 Backend·Frontend를 함께 release한 뒤 설치된 PWA의 알림 설정에서 현재 캠퍼스 기기 켜기/권한 허용과 실제 수신을 확인해야 한다. 현재 운영 사이트가 이미 고쳐졌다는 뜻은 아니다.


### PWA 보정 병합·공개배포 승인 (2026-09-11)

사용자가 PWA 알림 수정본의 main 병합·공개배포를 명시 승인했다. 범위는6372296의 오산 PWA 연결과0098 guard 보정이다. 메일 발송 현황 조회 기능 추가와 관리자 페이지는 제외한다. 원격 required CI 후 exact main으로 기존 수동 release를 실행하고 provider/DB 연결과 기존 데이터를 보존한다.0098은 함수만 교체하며 snapshot/backfill이 없고 기존 메일 predicate를 유지하므로 이전0096 전환의 임시 저장 제한을 반복 적용하지 않는다. 실제 기기 알림 수신은 설치·기기 권한에 따라 별도 확인한다.


### PWA 보정 공개배포 완료 (2026-09-11)

[PR140](https://github.com/emisubin/emi-qms/pull/140) 후보7980faebd46a4b908f6ae4a196b5ca3a0bb59590의 required CI34586312267이 모두 통과했다. Backend643·Frontend428·mock16·일반full-stack64·사업부1·오산1 PASS. main94b67922df4750eaf39b29fb636978c135002403로 정상 병합했고 후보/main tree 동일 및 main CI34589203858 성공을 확인했다. 보호규칙 변경·예외 없음.

[배포34589284351](https://github.com/emisubin/emi-qms/actions/runs/34589284351)이 성공했다. 첫 환경 승인 명령은 자동 심사에서0098이 사용자 승인 범위에 포함되지 않는다는 이유로 실행 전 거부됐다. 앞선 사용자에게 DB 보정 포함 수정본을 설명한 답변, 이후 병합·배포 승인, exactmain의0098 함수 변경 범위를 읽기 전용으로 대조해 동일 동작의 재심사를 요청했고 승인됐다. 사용자 추가 승인이나 정책 설정 변경·우회는 없다. 다음 공식 환경 승인 후0098과 Backend51·Frontend43으로 정상 전환했다. bootstrap·membership backfill·inspection·데이터 초기화·저장 중단은 실행하지 않았다.

공개 https://pms.emiinc.co.kr 의 health200·익명api/me401, 두 서버 latest=ready를 확인했다. 배포 전후 유효 환경값과 비밀 참조가 동일하여 기존 DB·Gmail/provider 설정을 보존했다. 실제 로그인된 Chrome에서 /notification-settings?businessUnit=OSAN을 열어 이전 사업부 기능 차단 오류가 사라지고 정상 기기 설정이 표시됨을 확인했다. 확인 계정의 오산 연결기기0개, 일반 브라우저에서는 설치된 앱에서 설정하도록 안내한다. 실제 기기 등록·권한 허용·실제 푸시 수신은 사용자 기기에서 후속 확인하며 새 테스트 알림을 발송하지 않았다. 메일 발송 현황 조회 추가는 배포 범위에 포함하지 않았다.

제품 병합·required CI·공개배포 완료. 최종 기록은 Task와 roadmap만 로컬 커밋하며 사용자 WIP는 보존한다. 검증 로그는 /private/tmp/osan-pwa-final-ci.log 및 osan-pwa-release.log, 설정 비교 자료는 osan-pwa-apps-before.json / osan-pwa-apps-after.json에 보존했다.

### 모바일 탐색·QR 스캔·단계 요약 및 W/O 패널 연결 (2026-09-12)

사용자가 구현을 승인했다. 시작 기준은 로컬 d79432a이며 origin/main 94b6792의 직전 공개 제품과 후속 문서 기록을 보존한다. 기존 작업 경로 /private/tmp/emi-osan-photo-upload-fix, codex/osan-project-management를 이어간다. 별도 원본 checkout WIP는 변경하지 않는다.

확정 범위: 홈 제목에 장비명·part 분류·빨간 D-day, 아래 bold W/O·납기일. 프로젝트 표 고객사 뒤 W/O. 진행 목록 제목 동일, 단계 아래 part 분류·수량·고객사·bold W/O·코드·납기일. 기존 카드/행 크기의 작은 원형 7단계 스텝퍼와 단계별 완료 대상 수, 원형과 수평 정렬된 bold 전체 진행률. 우측 하단 44px 원형 메뉴, 146px 펼침 메뉴(홈·프로젝트·진행 현황·알림·QR 스캔). 기존 확정 시안은 이 대화의 osan-inline-stepper.html, osan-floating-menu.html, osan-qr-scan.html이다.

아이폰 외부 카메라 QR은 브라우저로 열리는 관측을 반영하여 내부 QR 스캔으로 확정했다. 후면 카메라, 기기 안에서 QR 해독, 기존 패널 QR 조회 route로 이동, 권한/없는 대상 처리는 기존 서버 조회 기준. 다른 도메인·임의 경로 QR로 외부 이동하지 않으며 카메라 종료·권한 거부·재시도·백그라운드 중단을 처리한다. 공개 frontend 응답의 기존 camera=()가 이를 차단하므로 nginx 양쪽 템플릿을 camera=(self)로 한정 보정한다. 마이크/위치 등 다른 제한은 유지한다. 배포나 운영 설정 적용은 이번 범위에 포함하지 않는다.

같은 비어 있지 않은 W/O를 갖는 권한 내 오산 프로젝트의 패널을 진행 상세 선택창에서 함께 제공한다. 목록 카드를 합치거나 데이터를 병합하지 않는다. 다른 프로젝트 패널 선택은 해당 프로젝트의 상세로 이동하며 일괄 쓰기 선택 범위는 현재 프로젝트 내부로 유지한다. 기존 데이터·이력·QR 소유·DB 분리 보존. 검증과 독립 검토 결과는 작업 종료 시 아래에 기록한다.

구현·직접 검증 완료: Frontend 관련 6파일 60건 PASS, 타입·대상 lint·production build PASS(기존 bundle 크기 경고). 독립 생성한 합성 QR 이미지의 실제 jsQR 해독과 카메라 거부·재시도·늦은 권한 응답·백그라운드 중단·성공 후 정리를 확인했다. Backend 전용 disposable DB에서 관련 패널의 정확한 W/O/접근 범위와 endpoint catalog 2건 PASS, DB/container/network 정리 완료. Azure 정적 검증 PASS. 신규 endpoint의 HTTP 권한 거부 전체 조합은 이번에 별도 실행하지 않았으며 기존 서버 권한 helper와 store 접근 범위를 검토했다.

합성 데이터로 실제 App의 PC 1440 및 모바일 390 화면을 확인했다. 기존 카드 높이와 스텝퍼 크기, 메뉴·QR 대화상자, 관련 패널 선택 및 프로젝트 W/O 열을 확인했다. 최종 확인에서 표 grid의 열 수 누락으로 진행률이 다음 줄로 밀리는 문제를 보정하고 다시 직접 확인했다. 독립 reviewer의 미해결 P0–P2 없음(요청 gpt-6-astra/high, 실제 모델 값은 도구에서 미보고). 검증 근거는 /private/tmp/osan-mobile-targeted.log, osan-related-backend.log, osan-mobile-build-final.log 및 관련 합성 화면에 보존한다.

현재 상태: 로컬 구현·직접 검증 완료, 사용자 검수·실제 아이폰 PWA 카메라 스캔·최종 후보 전체 회귀·원격 CI/병합·공개배포는 미실행. 실제 데이터나 운영 설정을 변경하지 않았다. 다음은 사용자 검수와 아이폰에서 후면 카메라 허용 후 기존 패널 QR 조회 연결 확인이다.

2026-09-12 검수 서버 열기: 사용자 요청으로 최신 fec98be frontend를 http://127.0.0.1:5212, API를5113에 별도 기동했다. 기존 API5112는 migration ledger 미일치 상태였으며 작업 전용 합성 3DB에 공식 migrate-only를 적용한 뒤 새 API의 실제 오산 dashboard200과 브라우저 홈 표시를 확인했다. 기존 합성 프로젝트 보존, reset/bootstrap/seed 미실행, 외부 provider 비활성화. 기존5204/5112는 종료하지 않았다. 기동 스크립트 /private/tmp/osan-mobile-api-ready.sh, 로그 osan-mobile-api-ready.log 및 osan-mobile-preview.log. 이 주소는 같은 컴퓨터 전용이며 실물 아이폰 카메라 검수 주소가 아니다.

2026-09-12 스텝퍼 위치 보정: 사용자 요청대로 홈·진행 현황의 단계명/완료 수량을 위쪽에 붙여 유지하고 원형 스텝퍼는 기존 하단 진행률 막대 위치로 분리했다. 전체 진행률은 원형의 중심과 수평 정렬. 실제 검수 API 화면390/1440px에서 홈126px·진행39/52px 높이 보존과 겹침/정렬 확인, 관련 테스트13건 PASS. 검수 서버5212에 반영, 원격/공개배포 없음.

2026-09-13 확정 막대+원형 시안 구현: 실제 전체 진행률로 채워지는 연속 막대 위에 단계 원형을 겹쳐 표시한다. 홈은 단계명·수량을 스텝퍼 가까이 배치하고 진행 현황은 단계 그리드와 동일한 좌우 경계로 입고검사부터 원형 중심을 맞췄다. 완료/부분 완료/미완료 집계 보존. 실제5212 검수 화면 PC1440·mobile390에서 홈126px/진행52·39px 유지 및 전체 진행률 중심 정렬 확인, 관련13건 PASS, diff check PASS. 로컬 화면 반영 완료, 원격 병합·공개배포 없음.

2026-09-13 병합·공개배포 승인: 사용자 최종 변경 요약 확인 후 이번 후보의 원격 main 병합·공개배포를 명시 요청했다. 공정별 이상/특이사항 알림은 제외. 확정 화면과 기존 검수 이력을 기준으로 최종 후보 전체 회귀는 PR CI가 책임 실행하며 Backend·Frontend·mock·일반 full-stack·사업부·오산 전용 suite의 실제 성공을 확인한다. 최신 main94b6792를 제품 변경 없이 병합 이력으로 통합했다. 최종26afc03 delta 독립 검토 추가 P0–P2 없음. 신규 migration이나 provider 설정 변경은 없으며 Backend→Frontend 공식 release 순서를 유지한다. 실제 아이폰 PWA 카메라 확인은 배포 후 실기기 검수로 남는다.

PR141 최초 CI34733911221에서 frontend439건은 통과했으나 mock browser는 새 related-panels 응답과 W/O 열 기대값이 빠져 실패했다. 제품 코드는 유지하고 해당 fixture·열 개수/공용 셀 대응만 보정했으며 관련 browser3건 PASS. 후속 PR CI에서 전체 검증을 확인한다.

PR141 CI34734270628에서 frontend439·mock16·일반full-stack64·사업부1은 통과했으나 오산 상세 진입에서 신규 related-panels GET이 capability 허용 목록에 빠진 결함을 발견했다. 정확한 프로젝트 GUID 아래 해당 GET만 허용하도록 보정했다. 전용 합성 3DB full-stack1건에서 상세 진입, 동일 W/O 패널 조회200, 청주·POST·추가 경로403을 검증했고 임시 DB/역할/process/container/network 정리를 확인했다. 독립 reviewer 추가 P0–P2 없음. 근거 /private/tmp/osan-mobile-real-gate-fix.log. 최신 후보의 required CI 성공 후 병합·배포한다.


### 모바일 탐색·QR·W/O·스텝퍼 공개배포 완료 (2026-09-13)

[PR141](https://github.com/emisubin/emi-qms/pull/141)의 최종 후보 c193610314baed87c34ff6916aeb354d140612b6에서 [required CI34735541792](https://github.com/emisubin/emi-qms/actions/runs/34735541792)가 모두 통과했다. Backend644·Frontend439·mock16·일반full-stack64·사업부1·오산1 PASS. 기존 승인·사용자 화면 검수와 독립 검토 뒤 정상 main d76790e37d8f17c2e7359af38e889f35e5c585bf로 병합했다. 후보/main tree 동일, main CI34736861114 성공. 보호규칙 변경·관리자 예외 없음.

[공식 Azure 배포34736903149](https://github.com/emisubin/emi-qms/actions/runs/34736903149) 성공. 승인된 exact main의 Backend52→Frontend44가 모두 latest=ready이고 Single 모드를 유지한다. 기존 migration 확인 단계 성공, 신규 SQL 없음. bootstrap·membership backfill·inspection·reset·임시 저장 제한은 실행하지 않았다. 배포 전후 두 앱의 환경값·비밀 참조가 동일하여 DB·Gmail·PWA/provider 설정을 보존했다. 공개 health200·익명api/me401과 기존 CSP 유지, 앱 내 QR 스캔용 camera=(self)를 확인했다. 실제 데이터 수정·시험 메일/푸시 발송은 하지 않았다.

로그인된 공개 Chrome에서 재인증 후 홈·프로젝트·진행 현황을 정상 조회했다. PC와390px 모바일에서 막대+단계 원형, 제목 part/D-day, W/O 표시·프로젝트 표 열, 작은 펼침 메뉴를 직접 확인했다. 동일 W/O의 다른 프로젝트 패널이 선택 목록에 나타나고 선택한 프로젝트/패널로 정상 이동하는 것을 확인했다. 확인 후 브라우저 viewport를 원래 크기로 복원했다. 배포 전후 화면 프로젝트 집계121/시작전62/진행중59/완료0 동일. 이는 확인 화면 기준이며 전체 운영 데이터의 개별 전수 대조를 수행했다는 뜻은 아니다.

구현·자동 검증·사용자 화면 검수·원격 main 병합·공개배포 완료. 실제 아이폰 설치 PWA의 후면 카메라 권한·물리 QR 스캔은 실기기 후속 확인으로 남는다. 공정별 이상/특이사항 알림은 제외했다. 검증 근거는 /private/tmp/osan-mobile-final-backend.log, osan-mobile-final-frontend.log, osan-mobile-final-fullstack.log, osan-mobile-release.log와 앱 before/after 설정 비교에 보존한다. 마지막 Task·roadmap 기록만 별도 로컬 커밋하고 원본 checkout과 다른 작업 WIP는 보존한다.

### 단계 안내 사진·납기 HOLD·모바일 진입 및 QR 보정 (2026-09-14, 로컬 검수 대기)

사용자가 6항목 구현을 승인했다. 단계 공통 예시7장(입고 Rack/Box2장, 배치·배선·8계통·동작·출하 각1장, 포장 미제공), 완료 단계 연회색, 모바일 제목 전체 노출, 프로젝트 최초 진입 시 가장 앞선 미완료 단계/패널 선택(동률 기존 패널순서, QR는 지정 패널, 전부 완료는 포장), QR 카메라 영상은 열리나 해독 안 되는 문제를 보정한다. 납기 HOLD는 기존 프로젝트 수정 권한·흐름을 그대로 사용한다. 납기와 기록 보존, 홈 목록/KPI 제외, 다른 목록 HOLD 표시, 작업 계속 허용, 처리 이력 보존. 반려/초기화도 미완료, 명시 단계 링크 우선. 공개배포 승인 없음. 기준41a7a69 및 배포main d76790e. 기존WIP/runtime 보존, 관련 검증과 독립 검토 후 로컬 검수로 제공한다. 물리 아이폰 QR 확인은 별도 실제 기기 근거가 필요하다.


로컬 구현·검증: 안내 사진 7장 원본을 별도 공통 자산으로 연결하고 PC·모바일 확대/닫기를 제공했다. 포장 사진은 미제공 상태로 유지한다. HOLD는 기존 프로젝트 수정 API·권한·수정 토큰을 사용하며 변경 사유와 처리자/시각/전후 값은 같은 transaction 이력에 보존한다. 사용자 요청의 권한 기준은 기존 프로젝트 수정 권한과 동일이며 별도 HOLD 관리자 제한을 추가하지 않았다(현행 프로젝트 수정 자체의 관리자 guard는 보존). additive migration0099는 boolean 기본 false만 추가한다. 최초 모바일 진입은 가장 앞선 미완료 단계, QR는 지정 패널, 명시 단계 우선이며 삭제/없는 QR 패널은 다른 대상 대신 명확한 오류를 표시한다. QR 중앙 원본 해상도 해독+전체 축소 영상 fallback을 추가했고 카메라 영상 외부 전송은 없다.

자동 검증: 관련 frontend 61건 PASS, HOLD 전용 합성 PostgreSQL 통합 테스트 PASS(사유·저장/해제·홈 집계 제외·납기 보존·경쟁 토큰·작업 계속·이력 확인), 수정 frontend lint/typecheck 및 frontend/backend build PASS. 기존 bundle 크기 경고는 남는다. 독립 reviewer의 QR 패널 없음 오류 P2를 수정하고 실제 반례 테스트25건 통과 후 종결, 미해결 P0–P2 없음. 전체 제품 회귀/required CI는 이번 로컬 구현 단계에서 실행하지 않았으며 사용자 검수 후 최종 병합 후보에서 수행한다.

실제 화면: 로컬 API5113/frontend5212에 작업 전용 합성3DB migration을 적용하고 기존 데이터는 reset/seed 없이 보존했다. 외부 provider 비활성화 유지. PC1440·mobile390에서 안내 사진 컬러 원본 확대, 완료 단계 연회색, 입고 완료 후 배치검사 자동 진입, 긴 장비명/part 전체 노출과 페이지 가로 넘침 없음(390/390)을 확인했다. 기존 합성 검수 프로젝트에서 HOLD 저장 후 홈4→3/카드 제외, 해제와 원납기 보존을 확인했다. 이 프로젝트의 입고검사는 검수 코멘트로 완료하여 다음 단계 진입을 검증했고 긴 이름 검증 뒤 원래 이름을 복원했다. 운영 데이터는 변경하지 않았다.

검수 주소 http://127.0.0.1:5212/?businessUnit=OSAN (같은 컴퓨터 전용). 실제 아이폰 설치 PWA/인쇄 QR 인식은 미확인이다. 해독 취약점은 독립 인코더 합성 영상으로 재현·보정했으나 실제 사용자 장애의 유일 원인으로 확정하지 않는다. 로컬 구현·관련 자동 검증·독립 검토 완료, 사용자 검수·원격 반영·공개배포 미실행. 근거 /private/tmp/osan-followup-tests-final.log, hold-test.log, hold-ui-test.log, osan-progress-final.log, osan-followup-lint.log, osan-followup-type.log, osan-followup-frontend-build.log, osan-followup-backend-build.log. 새 공개배포 승인 없음.


2026-09-14 완료 사진 배경 재보정: 사용자 확인 요청으로 공개 모바일 완료 단계는 설명을 숨기고 사진 영역은 #FFC0C0임을 실제 화면에서 확인했다. 앞선 변경은 바깥 단계 카드만 회색으로 바꿔 사진 영역 배경을 누락했고 모바일 완료 설명을 노출하는 불필요한 변경도 있었다. 사용자 수정 요청에 따라 모바일 완료 단계 설명/안내 사진 숨김을 복원하고 실제 .osan-progress-photo-region 배경을 #F1F2F3으로 수정했다. PC 설명 접기와 미완료 단계 안내 사진은 유지. 관련 진행 상세25건 PASS. 작업 전용 합성 검수 프로젝트 배치검사에 파란 체크무늬 검증 PNG를 저장하여 390px 모바일에서 사진 로딩 완료 후 회색 사진/버튼 영역과 설명 숨김을 직접 확인했다. 운영 데이터 변경 없음. 변경 범위는 이 두 표시 보정이며 로컬 커밋/서버 반영, 원격·배포 미실행.


2026-09-14 기존 디자인 복원: 사용자 확인 후 모바일 진행 목록의 카드 높이/제목/단계 배치 조정만 예외로 승인되었다. 그 외 완료 단계 바깥 카드, PC 단계 목록 및 QR 단계 색상 override를 제거하고 앱 스캔 후 기존 QR 전용 조회 경로를 복원했다. 인식 보정, HOLD, 안내 사진, 모바일 일반 진행현황 미완료 단계 자동 진입은 유지한다. 공개 d76790e 대비 osan-progress.css 차이는 사진 영역 #FFC0C0 → #F1F2F3 한 줄뿐이며 기존 크기/간격/배치가 보존된다. 로컬 합성 완료 사진 화면에서 바깥 카드·완료 기록 흰색, 사진 영역 회색, 모바일 설명 숨김을 직접 확인했다. 진행 상세·QR 스캐너 관련33건 PASS(/private/tmp/osan-design-restore.log). 원격/공개배포 없음.

2026-09-14 완료 문구 간격: 사용자 요청대로 모바일 작업 완료 문구와 사진 영역 사이 하단 여백을17px→29px로12px 늘렸다. 다른 크기/배치 및 PC 팝업의 기존16px 여백은 유지한다. 실제390px 화면에서29px 간격 확인, diff check PASS. 작은 스타일 보정으로 제품 테스트 반복 없이 시각 확인했다. 로컬 반영, 공개배포 없음.

2026-09-14 단계 카드 하단 여백: 사용자 요청으로 모바일 단계 카드의 기존 최소높이657px을 제거하고 마지막 내용 아래 padding24px을 적용했다. 완료 안내문과 미완료 완료버튼 모두 내용 끝을 따라 테두리가 내려간다. 실제390px에서 완료/미완료 두 상태의 마지막내용→테두리25px(안쪽24+테두리1), min-height0과 서로 다른 자동 카드높이를 확인했다. 기존 다른 배치 보존, diff check PASS. 작은 스타일 변경으로 별도 제품 테스트 반복 없음. 로컬 반영, 원격·배포 없음.

## 통합 공개배포 승인 및 후보 (2026-09-14)
- 사용자 두 변경 함께 원격 main 병합·공개배포 명시 승인. 사진 Change 002와 통합, 기존 공개 main d76790e 기준.
- HOLD 0099 유지, 미배포 사진 migration 번호를 0100(40MiB), 0101(HEIC)로 정리. 충돌 2파일 해소, 자동 병합 endpoint 계약 확인.
- 통합 FE44개·typecheck 및 격리 DB/API26개 통과(실패/skip0). 독립 코드 검토 GO. Linux HEIC·실제 ClamAV 한도 및 required CI 확인 후 배포.

## 통합 공개배포 완료 (2026-09-14)
- PR142 정상 병합, main `42d77466b2781ee2e5c336db25b5d7568c9bad1a`. 보호 규칙 변경 없음.
- PR CI34796615123: Backend668, Frontend456, mockE2E16, full-stack64, 사업부1, 오산1 모두 PASS. main CI34798885863 PASS.
- 동일 pinned Linux/amd64 AzureLinux distroless·Magick14.17.1에서 합성HEIC decode/sRGB/JPEG표시 PASS. pinnedClamAV 스트림/파일 한도 각100MiB 확인. 독립review 조건 해소, 미해결P1/P2없음. 통합PC1440/mobile390 직접 확인.
- 공식 Azure34798945045 PASS: migration·Backend·Frontend·PublicSecurity 성공. 추가0099/0100/0101 적용, bootstrap/backfill/reset 미실행. Backend53·Frontend45 latest=ready Running. 배포 전후 두 앱 env 전체 동일. public health200, 익명api/me401.
- 공개 로그인·오산 목록122개·기존 패널 완료 기록 및 사진 로딩 확인. 전체 운영 데이터의 전후행단위 동일성 검증은 하지 않았으며 운영 시험등록/삭제도 하지 않음.
- 실제 아이폰 PWA QR 및 실제 사용자 HEIC 파일 검수는 남은 실기기 확인 한계.

## QR 조회·인식 및 앱 화면 후속 (2026-09-14)
사용자 구현 승인: 완료 QR단계는 사진·코멘트 우선, 설명/안내사진 기본접힘. 작은QR 인식 보정, PC진행목록 긴 이름 줄바꿈, 설치형PWA만 페이지확대제한(일반브라우저확대/스크롤/사진크게보기 유지). 기존 QR·권한·DB·라벨30/50mm와 하단3줄 보존. 관련 단위/브라우저검증 후 로컬검수; 이번 후속 원격/배포 승인은 없음.

- 구현: QR 완료 단계 설명을 기본 접힌 details로 바꾸고 완료 코멘트를 표시. PC 목록 이름을 모바일처럼 줄바꿈하며 카드 높이를 내용에 맞춤. QR 카메라 안내영역과 실제 중앙 인식영역을 맞추고 확대/원본 해상도 타일을 순환 판독하며 지원 기기의 연속초점·배율 조절을 추가. 이전 카메라의 늦은 응답이 새 카메라를 건드리지 않도록 소유 track을 분리. standalone/iOS 설치앱에서만 페이지 확대를 제한하고 일반 탭은 원래 viewport/제스처를 유지.
- 검증: QR 페이지·스캐너·독립 QR 이미지 디코드·PWA 모드 분기 23건 PASS, TypeScript build PASS, Vite build PASS. 변경 TS lint 오류0(기존 main Fast Refresh 경고1), 기존 번들 크기 경고 유지. 모바일390×844에서 완료 설명 접힘/펼침·안내사진·완료사진·코멘트 표시, PC1440×1000 목록 이름/단계/스텝퍼 배치를 직접 확인. 긴 실제 운영 이름 및 실제 아이폰 카메라/설치앱 제스처는 미검증.
- 독립 검토: 늦은 카메라 요청의 경쟁 조건 P2 보정 및 회귀 테스트 확인 후 종결, 미해결 P1/P2 없음. 실제30/50mm 인쇄물 스캔 성공률은 실기기 검수 전이므로 완전 해결로 보고하지 않음. 이번 후속은 로컬 구현/검증/커밋까지이며 원격 반영·공개배포·운영 데이터 변경 없음.

## Windows PWA 기존 창 재사용 후속 (2026-09-14)
- 사용자 메일/브라우저 앱 열기 중복 창 수정 승인. manifest launch_handler를 navigate-existing으로 지정하여 지원 브라우저의 동일 설치앱 실행 시 기존 창에서 요청 URL로 이동하도록 함. 앱 id/start_url/scope 유지, 브라우저 간 설치 제한은 범위 제외.
- 알림 클릭을 직렬화하고 새 창이 아직 matchAll에 나타나지 않아도 반환받은 client를 재사용. 닫힌 창/이전 실행 실패는 후속 클릭을 막지 않으며 외부·잘못된 URL은 PMS 알림 목록으로 대체. 기존 앱 내부 QR/메뉴 이동 및 Teams 외부 열기 흐름 보존.
- 실제 worker 실행을 포함한 관련 테스트7건 PASS, TypeScript build PASS, Vite build PASS(기존 번들 크기 경고). UI 디자인 변경 없음. Windows 설치앱 메일/주소창버튼/작업표시줄 실기기 검수는 미실행이며 설치앱이 갱신된 manifest를 받은 후 확인 필요. 다른 브라우저/프로필 설치앱 사이의 창 공유는 보장하지 않음. 원격 반영·공개배포 미실행.
- 독립 reviewer 실제 diff/테스트 검토 GO, 추가 P1/P2 없음. 모의 브라우저 API 테스트와 Windows 실환경 확인의 경계를 유지한다.

### 오산 PWA 연결 만료·미등록 복구 (2026-09-25)

사용자 승인: 오산 전체 계정의 푸시 상태를 읽기 전용 진단한 뒤, `UserRequest` 연결 해제는 유지하고 만료·미등록 연결 복구를 구현한다. 운영 조회 기준 34개 활성 접근 계정 중 활성 연결 13, 만료 2, 사용자 해제 2, 미등록 17이었다. 푸시 서비스 접수와 실제 단말 표시를 구분한다. 이번 범위는 코드·격리 검증이며 운영 구독 변경, 시험 푸시, 원격 병합·공개배포는 포함하지 않는다.

- 본인 endpoint 상태 응답에 해제 사유·최근 오류, 본인 캠퍼스 설정에 명시적 해제 기록 유무를 추가한다. 다른 사용자의 endpoint 사유는 노출하지 않는다. schema 변경 없음.
- 오산 설치 앱 실행·화면 복귀 시 연결을 확인한다. 과거 최초 안내를 닫았어도 만료·미등록이면 기존 푸시 팝업을 재사용한다. 알림 권한 요청·구독 등록은 버튼 클릭 이후에만 실행한다. `나중에`는 계정·캠퍼스별 현재 세션에서 최대 24시간 안내를 숨기며 서버 opt-out으로 취급하지 않는다.
- 404/410 endpoint는 브라우저에서 새 구독을 발급받아 저장한다. 새 구독 저장 실패 시 재시도하며 정상 구독은 재발급하지 않는다. 기기 권한 거부·상태 조회·등록 실패에는 안내와 재시도를 제공한다. 전역 알림 종류·고객사 수신 정책과 청주 최초 안내 정책은 유지한다.
- `UserRequest` 기록이 있는 사용자는 복구 안내를 보수적으로 제외한다. 로컬 endpoint가 사라진 경우 기기 식별이 불가능하므로 같은 계정의 다른 기기 해제도 자동 복구 억제 근거로 삼는다. 서버도 복구 요청을 기존 user lock/transaction 안에서 다시 검사해 동시 해제 요청을 덮어쓰지 않는다. 기존 설정 화면의 명시적 켜기 동작은 유지한다.
- 계정·캠퍼스 변경 중 늦은 응답과 브라우저 구독 변경을 차단한다. 관리자용 사용자 전체 상태 화면 신설·과거 실패 메일 재발송은 이 수정에 포함하지 않는다.

검증: frontend 관련 3파일 30건 PASS, typecheck·대상 ESLint·production build PASS(기존 bundle 크기 경고). 실제 API/격리 PostgreSQL/fake provider lifecycle 1건 PASS·skip0: 410 사유, UserRequest·새 endpoint 복구 거부와 구독/이벤트 불변, 타 사용자 정보 비노출, 오산/청주 분리, 명시적 켜기 보존. 증거 `/private/tmp/osan-push-recovery-backend-atomic.log`. 합성 데이터와 실제 컴포넌트로 PC1440×900·mobile390×844 팝업을 직접 확인했으며 가로 넘침·브라우저 오류 없음. 검증용 임시 페이지·서버·탭 정리 완료.

독립 코드 검토의 P2 2건(복구 저장과 사용자 해제 경쟁, 계정 전환 중 브라우저 변경)을 보정하고 재검토에서 모두 해소했다. 실제 아이폰에서 재연결 후 업무 푸시가 표시되는지는 배포 이후 사용자 기기에서 확인해야 한다. 운영 복구 완료로 기록하지 않는다.

배포 후속 승인(2026-09-25): 사용자 원격 병합·공개배포 승인, 버그 수정 공지 게시글 생략. 별도 업데이트 팝업 준비 완료. PR156 첫 CI의 통합 테스트는 새 configuration 응답 필드의 기대값 누락으로 1건 실패/63건 통과하여 `hasUserDisabledSubscription: false`를 명시하도록 보정했다. 제품 동작 변경은 없으며 필수 CI 재검증 후 병합한다.
