...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 026 Publication Approved / CI and Release Pending | Change 018 승인형 workflow와 변경 인지형 release를 유지한다. Change 026은 전체 흐름 상태 전용 표시를 exact latest `main`으로 병합한 뒤 Backend·Frontend만 교체하고 migration을 생략한다. 기존 Web Push·Teams·메일·Key Vault 참조와 업무 데이터는 보존한다. | TASK-AZURE-PILOT-001, TASK-CI-COST-001, TASK-WORKFLOW-CONTINUITY-001 Change 018 | 기존 운영 인증·알림 설정·업무 데이터 보존 | Yes | Ready PR `CI Gate` → exact-main Azure release → 공개 화면 검수 |
