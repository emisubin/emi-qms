...
export function OsanNoticeBoard({userKey, noticeId, compose, admin, enabled, onList, onOpen, onCompose}:{userKey?:string;noticeId?:string;compose?:boolean;admin:boolean;enabled:boolean;onList:()=>void;onOpen:(id:string)=>void;onCompose:()=>void}) {
 ...
 <table className="board-table"><thead><tr><th className="number">번호</th><th>제목</th><th className="author">작성자</th><th className="date">등록일</th><th className="file">첨부</th><th className="read">읽음</th></tr></thead><tbody>{items.map((item,i)=><tr key={item.noticeId} className={item.pinned?'pinned':''}><td className="number">{item.pinned?<span className="pin">공지</span>:(list?.totalCount??0)-(page-1)*20-i}</td><td className="title"><button onClick={()=>onOpen(item.noticeId)}>{item.title}</button></td><td>{item.authorDisplayName}</td><td>{new Date(item.createdAtUtc).toLocaleDateString('ko-KR')}</td><td className="file">{item.attachmentCount||'—'}</td><td className={`read ${item.isRead?'':'unread'}`}>{item.isRead?'읽음':'안 읽음'}</td></tr>)}</tbody></table>
