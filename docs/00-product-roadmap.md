...
| 99 | 오산 시범 운영 기획과 7단계 개발 로드맵 | Business Planning Approved / Task 1 USER_VALIDATION_COMPLETE / Task 2 CHANGE_002_IMPLEMENTED_AWAITING_USER_VALIDATION / Task 3 USER_VALIDATION_COMPLETE / Phase 1 Local Validation Complete · Draft PR/CI Pending | 사용자/개발/운영 | [TASK-OSAN-PILOT-001](../tasks/osan-pilot-001.md) | Task 1 DB 분리 기반과 Task 3 프로젝트 등록·목록·상세의 사용자 검수 완료. Task 2 Change 002는 actual active 목적지가 두 곳 이상인 총괄에게만 selector를 보이게 보정해 Frontend 297/297, browser 3/3, 실제 3-DB 격리 2/2를 통과했고 사용자 검수는 대기한다. 사용자는 Task 4~6보다 먼저 Task 1~3만 배포하는 순서 변경을 승인했다. `TASK-AZURE-DEPLOY-001 Change 031`은 기존 Cheongju DB 보존, Directory·Osan 추가, DB 준비와 PITR 뒤 serving 연결을 준비하고 Backend 582/582·Frontend 297/297·browser 79/79·배포 정적 검증을 통과했다. 일반 사용자는 membership 한 곳만 부여한다. 다음 제품 Gate는 Task 4 구현 승인, 배포 Gate는 Draft PR·CI 뒤 selector 사용자 검수와 exact main merge 승인이다. |
