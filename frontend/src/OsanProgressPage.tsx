...
export interface OsanProgressPageProps {
  projectId: string; initialTargetId?: string; developmentUserKey: string | undefined; mutationAllowed: boolean; onBack?: () => void;
}
...
function OsanProgressWorkspace({ projectId, initialTargetId, developmentUserKey, mutationAllowed, onBack }: OsanProgressPageProps) {
  ...
  function changeSelection(ids: string[], nextMode: 'individual' | 'batch') {
    ...
    pendingCompletion.current = null; setFiles([]); setFileError('');
  }
  ...
  function refresh() { if (busyRef.current) return; pendingCompletion.current = null; setError(''); setRefreshing(true); setReload(value => value + 1); }
