...
backend_pid=""
backend_log="$(mktemp -t emi-qms-osan-project-backend.XXXXXX.log)"
operation_log="$(mktemp -t emi-qms-osan-project-setup.XXXXXX.log)"
backend_pid_file="$(mktemp -t emi-qms-osan-project-backend.XXXXXX.pid)"
...
values
  ...
  ('50000000-0000-0000-0000-000000000002', 'OSAN', true);
...
if [[ "${self_test_mode}" == "--self-test-backend-startup-failure" ]]; then
  backend_expected_command_marker="emi-qms-osan-project-startup-failure-injection"
  backend_expected_url_marker=""
  backend_readiness_attempts=3
  bash -c 'sleep 0.2; exit 98' emi-qms-osan-project-startup-failure-injection >"${backend_log}" 2>&1 &
  backend_pid="$!"
