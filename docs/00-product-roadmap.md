...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 006 Main Merged / Change 007 Image Publish Approved / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `67 Exact`·PITR restore와 세 workload readiness 완료. 원격 main `496b88b`에 Graphite·UL891 `0068`·프로젝트 진행률 보정·Teams `1.0.2`·PWA manifest/icon 통합과 PR·main CI 완료. Azure에는 기존 Change 005 image가 ready이고 최신 main image는 미게시 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~006 main merge, 원격 main `496b88b`, 사용자 Teams·PWA asset·image 게시 승인 | Front Door domain validation·TLS·인증·actual provider, 최신 main image·migration `0068` handover 미완료 | Yes | 최신 main image 게시 → migration `0068`·revision 재배포 → Edge·TLS·provider 검수 |
