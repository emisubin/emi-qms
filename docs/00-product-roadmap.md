...
[진행단계 이상 처리·오산 개인 알림 설정](../tasks/osan-progress-001-change-001.md)을 확정한 정책·시안으로 구현·직접 검증을 마치고 사용자 검수를 기다린다. 기존 상세 화면의 버튼/입력만 추가하고, 스텝퍼 표시와 개인 메일·푸시 설정을 함께 연결한다. 이 기록이 아래 과거 진행 순서의 해당 업무 정책을 갱신한다. 검증·검수·로컬 커밋·게시 상태는 해당 change에서 관리한다. 원격 병합·배포는 별도 승인 범위다.
