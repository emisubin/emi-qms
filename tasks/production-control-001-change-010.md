...
| --- | --- | --- | --- | --- |
| `CI-FRONTEND-FORM-TEMPLATE-001` | P2 | `RESOLVED` | 동일 version의 중복 선택 동기화가 빠른 편집 진입을 취소해 CI와 실제 빠른 조작에서 저장 버튼이 사라질 수 있다. | 같은 version이면 후행 초기화를 생략했고 집중 unit `3/3`·연속 `10/10`·Frontend 전체 `177/177`·lint·typecheck·build, PR·main 표준 CI `3/3`을 통과했다. |
| `CI-FULLSTACK-FORM-TEMPLATE-FIXTURE-001` | P2 | `RESOLVED` | 첫 PR Full-Stack 격리 DB의 기본 Item에는 현재 제조 양식이 없어 회귀가 `수정`을 기다렸고, 두 번째 실행은 reload 뒤 이미 유지된 관리자 값을 다시 선택해 앱의 정상 사용자 전환 규칙에 따라 목록으로 이동했다. 제품 수정 실패가 아니라 테스트 준비조건·route 처리 누락이었다. | 현재 양식이 없으면 UI로 한 번 생성하고 reload한 뒤 양식 관리 heading을 확인하며, 유지된 사용자를 다시 선택하지 않고 `수정 → 저장 유지`를 검증하도록 fixture를 self-contained하게 만들었다. |

## 게시 결과

- PR #81: Backend·Frontend·Full-Stack `3/3 PASS`
- 원격 `main` squash merge: `d86e9f0cd417ddca445d9980188375c847d057bb`
- merge SHA main CI run `31152166786`: Backend·Frontend·Full-Stack `3/3 PASS`
- Azure 운영 재배포는 Change 010 승인 범위에 포함되지 않아 실행하지 않았다.
