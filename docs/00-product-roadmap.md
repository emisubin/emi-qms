...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 014 Public API·Login Complete / Source Sync | Change 013 PR #72 원격 main 병합, immutable Frontend image 게시·교체와 공개 root/PWA·origin 403·TLS를 완료했다. Backend exact internal host allowlist로 readiness `200`, 익명 보호 API `401`을 확인했고 실제 비상 관리자 계정으로 관리자 화면 접근을 검증했다. Change 014는 이 runtime 계약을 배포 원본에 동기화한다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~013 기준선, 사용자 공개 Azure 배포·main 병합 승인 | Teams 관리자 승인·조직 catalog·Teams/Gmail actual provider 미완료. 에스컬레이션 세부 정책은 후속 기획 | Yes | Change 014 원격 main 반영 → Teams 승인·설치·event별 provider 검수 |
...
- 상태/다음 순서: Change 011 migration `0069`·최신 revision 교체 완료 / 세 Container Apps `3/3 Running` / Change 013 PR #72 원격 main 병합·immutable Frontend image 게시·교체 완료 / Front Door domain·managed TLS·공개 root/PWA·origin 403 확인 / Change 014에서 exact Backend internal host allowlist로 readiness `200`·익명 보호 API `401`과 실제 비상 관리자 계정의 관리자 화면 접근 확인 / Teams `1.0.4` 관리자 승인 요청 제출 / Change 014 source 동기화 뒤 provider 검수가 다음 Gate
