...
public sealed class InteriorBusbarCommercialTests
{
    ...
    public async Task CommercialMigrationPreservesHistoricalRowsWithoutInventingPrices()
    {
        ...
        await new Npgsql.NpgsqlCommand(await File.ReadAllTextAsync(Path.Combine(root, "database/migrations/0109_interior_busbar_commercial_data.sql"), TestContext.Current.CancellationToken), connection).ExecuteNonQueryAsync(TestContext.Current.CancellationToken);
