...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 017 External Notifications / User Receipt Complete | Change 013 edge·TLS, Change 014 API·관리자 로그인, Change 015 Frontend 사전 인증, Change 016 Teams Graph·web 검증 뒤 Change 017에서 사용자 승인으로 Dispatcher·Teams Activity·Gmail SMTP actual을 활성화했다. Latest Backend revision Ready, 최신 수동 Teams Activity `6/6 Sent`·Mail `3/3 Sent`, Open Pending/Processing/Failed `0`, 사용자 실제 수신 확인까지 완료했다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~017 기준선, 기존 대기 알림 일괄 발송·worker 활성화·실제 수신 확인 | 에스컬레이션 세부 정책과 Teams SSO·새 manifest는 후속 기획 | Yes | 사용자 승인 순서에 따라 Change 015~017 원격 게시 → 승인 게이트형 GitHub→Azure 배포 연결 → Teams SSO·새 manifest 기획 |
...
- 상태/다음 순서: Change 011 migration `0069`·최신 revision 교체, Change 013 edge·TLS, Change 014 API·관리자 로그인, Change 015 Frontend 사전 인증, Change 016 Teams Graph·web 검증 완료 / Change 017 외부 알림 worker·Teams Activity·Gmail SMTP actual 활성화, 최신 수동 Teams Activity `6/6 Sent`·Mail `3/3 Sent`, Open Pending/Processing/Failed `0`, 사용자 실제 수신 확인 완료 / 사용자 승인 순서에 따라 Change 015~017 원격 게시 뒤 승인 게이트형 GitHub→Azure 배포 연결
...
| 2026-08-06 | `TASK-AZURE-DEPLOY-001 Change 017`의 Teams client·메일함 실제 수신을 완료하고 Change 015~017 원격 게시 뒤 승인 게이트형 GitHub→Azure 배포 연결을 Teams SSO보다 먼저 진행 | 실제 provider 수락과 사용자 수신을 모두 닫고, 원격 `main`을 운영 배포 원본으로 만들되 GitHub Environment 승인 전에는 Azure 운영 변경이 시작되지 않게 하기 위함 | 23장~25장, TASK-AZURE-DEPLOY-001 Change 015~017 |
