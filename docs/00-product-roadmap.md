...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 032 Osan Phase-1 Released / Change 007 Hotfix Local Validated · PR CI Pending | PR #121 exact main `b405a9cb...`와 release `34181334545`로 Directory·Cheongju·Osan DB, restore rehearsal, migration, Cheongju backfill과 등록 전용 Backend·Frontend를 공개했다. 운영 검수에서 사용자 이름·계정 ID와 총괄의 Osan membership·selector 누락을 확인해 Change 007이 identity-bound 표시 병합, 한 저장 총괄 지정, 복수·마지막 총괄 보호와 양 사업부 local-first profile을 구현했다. | TASK-AZURE-PILOT-001, TASK-CI-COST-001, TASK-SITE-ACCESS-001, TASK-G2-OPERATIONS-002 Change 004, TASK-OSAN-ISOLATION-001, TASK-OSAN-ACCESS-001, TASK-OSAN-PROJECT-001 | 일반 사용자 한 사업부, DB 분리, Cheongju 데이터·provider 보존, Directory `0004` additive, existing overall backfill, exact main hotfix와 공개 권한 검수 | Yes | Change 007 local commit → PR #122 final CI 1회 → exact main merge → migration/backfill → Backend/Frontend → 운영 사용자 검수 |
...
| 2 | [TASK-OSAN-ACCESS-001](../tasks/osan-access-001.md) — 사용자·사업부 전환 | CHANGE_003_IMPLEMENTED_AWAITING_USER_VALIDATION | 1 | selector 세 경우와 통합 사용자 승인·수정 화면 사용자 검수 |
...
| 7 | [TASK-AZURE-DEPLOY-001 Change 031](../tasks/osan-pilot-001-rollout-handoff.md) — 오산 1단계 개통 | CHANGE_003_USER_VALIDATION_PENDING | PR #121 head `11c1185e...`는 기존 전체 검증 완료 상태로 보존한다. Change 003은 local commit 검수 전이며 PR·Azure에 미반영이다. | Change 003 검수 → PR #121 갱신 → 최종 head 전체 CI/회귀 1회 → main merge 승인 → DB/restore/app 순차 배포 |
...
후속 검수·CI·main 병합 뒤 오산 phase-1은 Change 032로 공개됐다. 운영 화면에서 확인된 이름·계정 ID 및 총괄 양 사업부 접근 누락은 `TASK-OSAN-ACCESS-001 Change 007`로 보정한다. Directory `0004`와 기존 총괄 local-first backfill을 사용하며, 일반 사용자 단일 사업부·기존 Cheongju 데이터·provider와 Osan worker off 계약은 유지한다. 사용자는 Change 007의 PR·main merge·Azure hotfix까지 승인했고 final CI 뒤 migration→backfill→Backend→Frontend 순서로 진행한다.
