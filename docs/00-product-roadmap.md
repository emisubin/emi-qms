...
| 6.5 | TASK-G2-OPERATIONS-002 — 납품 목표·불량·홈 임시 시뮬레이션 | NEW_FEATURE | Local Implementation Complete / Automated Validation Complete / User Validation Complete / Publication Approved | 사용자 승인 Codex 직접 기획·구현. Additive migration `0084`로 납품 목표와 불량을 정식 데이터에 추가하고, 불량 차감 재고·기존 forecast expiry·권한/CAS를 유지했다. 홈 첫 그래프 아래 생산 현황표에서 생산·납품·불량을 저장 없이 바꾸면 그래프·KPI·재고가 즉시 갱신되고 재조회 시 폐기된다. 납품 목표 주황 점선, 표 행 순서, 예상 파랑·휴일 빨강 우선, 390px 날짜 입력 축소와 Change 001 직접 입력 중앙 정렬·증감 버튼 제거를 자동·격리 browser와 사용자 검수로 확인했다. | TASK-G2-OPERATIONS-001 완료 계약, 사용자 2026-09-01 순서·Fable 예외·병합 승인 | Persistent UAT·migration 운영 적용·Azure 공개배포 미승인 | Yes | Ready PR 필수 CI → main 병합 → exact main SHA 운영 적용 별도 승인 |
...
| 97 | G2 납품 목표·불량·홈 임시 시뮬레이션 | Local 구현·자동 검증·사용자 검수 완료 / 원격 main 병합 승인 | 사용자/제조/영업/물류/개발 | TASK-G2-OPERATIONS-002 | 납품 목표·불량 정식 저장, 불량 차감 재고, 저장 없는 홈 임시 예상값, 모바일 날짜 입력 축소와 직접 입력 중앙 정렬. Persistent UAT·Azure 운영 적용은 별도 승인 |
