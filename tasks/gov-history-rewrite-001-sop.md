...
- P0/P1과 신규 P2 0
- Risk owner의 public 재개 명시 승인

Public 재개는 history rewrite의 자동 후속 단계가 아니다.
...
- CI 성공 전 P2 Closed와 public 재개를 금지한다.

## 9. Partial failure와 복구

| 상황 | 조치 |
| --- | --- |
| Support pending/rejected 또는 reference 재노출 | Private 유지, residual risk 결정 또는 Support 후속 요청 |
...
| CI 반복 실패 | Public 재개 금지, 별도 fix Task |
...
- 승인 없는 public 재개, backup 삭제와 worktree 정리
