...
| 3.3L | TASK-AUDIT-001 — 로그인·데이터 변경 감사 원장 | APPROVED_FEATURE_IMPLEMENTATION / POLICY_DECISION | Local Release Candidate Ready / User Validation Pending / Remote Unpublished | 대화형 로그인·명시적 로그아웃, 성공 데이터 변경, validation·충돌 실패 시도와 기존 권한 거부를 관리자 전체 감사 이력에서 통합 조회하고 선택 Excel로 내보내는 local 구현. Change 002에서 endpoint 분류 `185/185`(포함 156·제외 29), relation 분류 `145/145`(추적 94·제외 51), 중앙 PostgreSQL transaction 대표 검증과 local 동시 mutation `50/50`을 v1 acceptance로 승인하고 409/412를 `Conflict`로 확정했다. Audit append 실패의 업무 rollback과 잔존 `Duplicate` 제거를 보정한 뒤 Backend `567/567`, migration `59/59`, Frontend `235/235`, desktop·375px visual과 독립 재검증 PASS·Open P0/P1/P2 `0/0/0`을 확인했다. | 기존 Entra/MSAL identity, `authorization_audit_events`, `data_export_events`, `Audit.Read.All`, migration `0082` | 156-route 1:1 matrix는 모든 route의 업무 규칙을 증명하지 않으므로 v1 acceptance에서 제외. 실제 Microsoft 로그인·Azure 50명 부하·Persistent UAT·Azure 운영 적용 미실행 | Yes | 승인된 local commit 반영 / Git 게시·Persistent UAT·Azure release 별도 승인 → 운영 적용 후 사용자 검수 |
