...
| 3.3A | TASK-PRODUCTION-CONTROL-001 Item별 생산계획·자동 실적·가로 막대 일정 | NEW_FEATURE | Experiment Complete | Fable interview 확인·2-pass planning·local implementation·Change 002~008 양식 통합·단일 현재 양식·1:1 연결·제조 저장 오류·OQC 합격 단일 연결·계획표 헤더·날짜 축·항목별 담당자·필요 인원·코멘트와 8열 생산계획표, Change 009 `생산관리 전체 전달사항` 조회 위치 보정·자동 검증·사용자 검수 완료 / `USER_VALIDATION_COMPLETE` | TASK-ADMIN-002·생산계획·구매·자재·제조·품질·물류 원본 데이터 | 대표 repo·main·Persistent UAT 미반영, 기존 프로젝트는 Legacy/snapshot 유지 | Yes | 완료 scope 재구현 금지 |
| 3.3B | TASK-UL891-PRODUCTION-PLAN-001 실물 세트별 생산계획 | NEW_FEATURE | Change 008 User Validation Complete / Change 004 Publication Approved | Fable 2-pass 본체와 Change 002~008의 전체 세트 기본계획·실적 연결 편집·일정표 색/날짜선/테두리·담당자 표시를 통합 원격 `main` 기준선에 이식. UL891 단일 현재 설계·활성 42면 projection·migration `0068`과 제조·품질·물류 현재 순번 `1..42`·영구 code `P52` 분리를 자동·실제 화면에서 검증. Backend `482/482`, Frontend `175/175`, Mock `4/4`, 전체 isolated Full-Stack `56/56`, Change 004 집중 Full-Stack `1/1` 완료 | TASK-PRODUCTION-CONTROL-001·TASK-UL891-SET-001·DESIGN-000 Change 006·TASK-EXPERIMENT-PROMOTION-001 Change 002 | Git Ready PR·CI·merge Gate, Persistent UAT·Azure 통합 image 재배포 미완료 | Yes | Ready PR·CI·merge → 통합 source image 재배포 |
...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 005 Main Merged / Active Workloads Ready / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `67 Exact`·PITR restore와 세 workload latest revision readiness 완료. 제품 기준선 통합과 UL891 Change 003 port 자동 회귀까지 완료 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~005 main merge, TASK-EXPERIMENT-PROMOTION-001 Change 002~003 | UL891 통합 후보 사용자 재검수·Git 게시, Front Door route 0, DNS·TLS·원본 업무 route 차단·인증·실제 provider·통합 source image 재배포 미완료 | Yes | UL891 통합 후보 사용자 재검수·게시 → 통합 source image 재배포 → Edge·DNS·TLS·provider 검수 |
...
- 상태/다음 순서: Change 003~005 main 병합, Foundation·secret-scope RBAC·image 게시·DB role bootstrap·migration `67 Exact`·PITR restore rehearsal 완료 / 세 Container Apps provisioning `Succeeded`·running·latest revision ready, Backend·ClamAV internal ingress·Frontend external ingress / Front Door route 0과 public traffic·external notification 비활성 / 통합 제품 기준선 image 재배포는 UL891 porting·회귀 뒤 별도 Gate
...
- Finding: `AZURE-IDENTITY-001`, `AZURE-DB-ROLE-001`, `AZURE-BACKEND-PROBE-HOST-001`, `AZURE-FRONTEND-NGINX-MAP-001` P1과 `AZURE-PORTAL-ARTIFACT-001`, `AZURE-WEB-IMAGE-PUBLISH-001` P2 `RESOLVED`. 실제 DNS·managed TLS·origin 업무 route 차단·인증·provider smoke와 통합 main image readiness는 다음 운영 Gate에서 검증
...
- 다음 Gate: 5174 제품 기준선과 Azure 원격 기준선 통합을 원격 main에 반영한 뒤 UL891 5175 변경을 이 통합 기준선에 선택 이식·전체 회귀한다. 그 결과 source SHA로 Backend·Frontend image와 revision을 갱신하고 origin direct 차단을 확인한 뒤에만 Edge·DNS·TLS·Entra 검증으로 진행한다.
...
| 90 | 실험 계보 Full-Stack 전체 회귀 | `EXPERIMENT_COMPLETE / BATCHED_FINAL` | 개발/품질 | TASK-E2E-FULL-SUITE-001 | 실제 역할 18단계 lifecycle과 Change 008 현재 구매·키팅·제조 투입·품질 판정·8개 상세 탭·달력월 정산 정책 검증. lifecycle 1/1, Aggregate Pending 1/1, IQC Pending 1/1. 대표 repo·main·게시 제외 |
