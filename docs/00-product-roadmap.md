...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 025 Publication Approved / CI Pending | `TASK-ADMIN-003 Change 002`의 migration `0078`·Backend·Frontend와 `TASK-PWA-PUSH-001 Change 002`의 Azure 운영 설정 보존 정의를 최신 main 기준 단일 release로 게시한다. 기존 Change 024 exact-main·readiness·public security 계약을 그대로 사용한다. | TASK-AZURE-PILOT-001, Change 018 승인형 workflow, TASK-CI-COST-001, TASK-ADMIN-003 Change 002, TASK-PWA-PUSH-001 Change 002 | 기존 운영 인증·Teams·메일·업무 데이터와 VAPID 원문 보존. 미완성 공통 매뉴얼 작업 제외 | Yes | Ready PR `CI Gate` → exact main migration `0078` → Backend·Frontend → 공개·Web Push 보존 검증 |
