...
| 관리자 | 시스템 관리 중심 관리자 홈, 사용자 관리 재사용/확장, 부서 관리, 휴일 관리 재사용, 권한 매트릭스 read-only, 기준정보 변경 이력, 업무 시작/완료 이력, 알림/에스컬레이션 조회, 발송 실패/대기 상세 추적, active escalation L0~L3 breakdown, 삭제 예정 + 7일 후 완전 삭제 시도, 복구, 일괄 삭제/복구, 삭제 보류, 부서 field-level validation. `TASK-AUDIT-001` 전체 감사 이력은 local 구현·exact catalog·중앙 transaction 대표 검증을 v1 acceptance로 승인하고 모든 409를 Conflict로 보수 분류했으며 독립 재검증 PASS·Open P0/P1/P2 `0/0/0` | Item/포장방식/생산계획 단계/구매 필수 항목 관리자 통합 여부, role/permission 편집 UI, 삭제 예정 데이터 purge 운영 정책, 전체 감사 이력 운영 적용·보존량 관찰 |
...
| 3.3L | TASK-AUDIT-001 — 로그인·데이터 변경 감사 원장 | APPROVED_FEATURE_IMPLEMENTATION / POLICY_DECISION / UAT_RUNTIME | Publication Approved / Azure Deployment Approved / User Validation Pending | 대화형 로그인·명시적 로그아웃, 성공 데이터 변경, validation·충돌 실패 시도와 기존 권한 거부를 관리자 전체 감사 이력에서 통합 조회하고 선택 Excel로 내보내는 local 구현. Change 002에서 endpoint 분류 `185/185`(포함 156·제외 29), relation 분류 `145/145`(추적 94·제외 51), 중앙 PostgreSQL transaction 대표 검증과 local 동시 mutation `50/50`을 v1 acceptance로 승인하고 409/412를 `Conflict`로 확정했다. Audit append 실패의 업무 rollback과 잔존 `Duplicate` 제거를 보정한 뒤 Backend `567/567`, migration `59/59`, Frontend `235/235`, desktop·375px visual과 독립 재검증 PASS·Open P0/P1/P2 `0/0/0`을 확인했다. Change 003에서 원격 main 병합과 Azure 운영 migration·공개 release를 승인했다. | 기존 Entra/MSAL identity, `authorization_audit_events`, `data_export_events`, `Audit.Read.All`, migration `0082` | 156-route 1:1 matrix는 모든 route의 업무 규칙을 증명하지 않으므로 v1 acceptance에서 제외. 실제 Microsoft 로그인·Azure 50명 부하·local Persistent UAT handover·실제 외부 알림 시험 발송 제외 | Yes | Ready PR 필수 CI → exact main SHA Azure migration·Backend·Frontend release → 사용자 운영 검수 |
...
| 48 | 전체 field-level audit 확장 | Local Release Candidate Ready / 사용자 검수 대기 / 원격 미게시 | 사용자/운영 | TASK-AUDIT-001 | migration `0083` 후보가 대화형 login/logout, 포함 mutation `156`, 업무 relation `94`, 실패 저장 시도, 기존 권한 거부 통합 조회와 선택 Excel을 구현했다. exact catalog+중앙 transaction 대표 검증을 v1 acceptance로 승인했고 모든 409를 `Conflict`로 기록한다. 독립 재검증 PASS·Open P0/P1/P2 `0/0/0`; 과거 소급, Entra 단계 실패, worker/provider, archive/purge와 운영 적용은 제외 |
