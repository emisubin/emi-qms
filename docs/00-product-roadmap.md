...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 013 Domain·TLS Complete / Static Public / API Fix Local Validated | Change 011 migration `0069` `69/69 Exact`와 Backend/Frontend Healthy revision을 완료했다. Front Door domain·managed TLS·공개 root/PWA·origin 403과 Entra 공개 redirect를 확인했다. 공개 API가 Nginx 내부 routing host 오류로 404인 P1을 보정해 artifact·test·Frontend image local build를 통과했다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~012 기준선, 사용자 공개 Azure 배포 승인 | Change 013 원격 main 병합·immutable Frontend image 교체·API/로그인 smoke, Teams 관리자 승인·조직 catalog·Teams/Gmail actual provider 미완료. 에스컬레이션 세부 정책은 후속 기획 | Yes | Change 013 main 반영·Frontend 교체 → API·로그인 → Teams 승인·설치·event별 provider 검수 |
...
- 상태/다음 순서: Change 011 migration `0069`·최신 revision 교체 완료 / 세 Container Apps `3/3 Running` / Change 013에서 Front Door domain·managed TLS·공개 root/PWA·origin 403과 Entra 공개 redirect 확인 / 공개 API 404의 Nginx internal routing host P1 보정과 local image build 완료 / Teams `1.0.4` 관리자 승인 요청 제출 / Change 013 main 반영·Frontend image 교체 뒤 API·로그인·provider 검수가 다음 Gate
