...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | UAT_RUNTIME | Change 009 Main Merge Approved / Teams 1.0.4 Approval Submitted / Public Traffic Off | Provider-specific 코드·Teams manifest·P1 최소 권한, Portal ARM JSON 4개·OIDC image 게시, Foundation·RBAC·DB role bootstrap·migration `0068 Exact`·PITR restore와 최신 main 세 workload readiness 완료. Front Door CNAME은 유지하고 새 validation TXT의 DNS 반영을 대기한다. Change 009는 원래 계획의 수신자·event 시점으로 공개 Activity type 10개와 Backend 자동 delivery를 연결하고 migration `0069`, Teams `1.0.4` handoff package까지 검증했으며 사용자가 Teams 승인 요청과 원격 main 병합을 승인했다 | TASK-AZURE-PILOT-001, 비용·Budget 확인, Change 003~008 기준선, 최신 main image·migration `0068` handover, 사용자 공개 Teams 알림 구현·게시 승인 | Change 009 PR·merge·migration `0069`·Backend revision, 외부 DNS TXT·Front Door validation/TLS·Teams 관리자 승인·조직 catalog·actual provider 미완료. 에스컬레이션 세부 정책은 후속 기획 | Yes | Change 009 main 병합 → migration `0069`·Backend revision → DNS/TLS → Teams 승인·설치·event별 provider 검수 |
...
- 상태/다음 순서: Change 003~006 원격 main `496b88b` 병합·CI 완료, Foundation·secret-scope RBAC·최초 image 게시·DB role bootstrap·migration `67 Exact`·PITR restore rehearsal 완료 / 세 Container Apps provisioning `Succeeded`·latest revision healthy, Backend·ClamAV internal ingress·Frontend external ingress·origin health `200`·업무 route `403` / DNS TXT·CNAME과 validation token 일치, Front Door domain validation `Pending`·deployment `NotStarted`, public traffic·external notification 비활성 / Change 007 최신 main Backend·Frontend image 게시 승인, 게시 뒤 migration `0068`·revision handover가 다음 Gate
...
- 예상 migration: Azure는 기존 `67 Exact`, 최신 main은 additive data-backfill migration `0068` 포함. 최신 migration image의 one-shot job으로 `68 Exact`를 확인하기 전 새 application revision을 활성화하지 않음
...
- 다음 Gate: Change 006를 사용자 검수·게시 Gate로 원격 main에 반영한 뒤 그 새 main SHA로 Backend·Frontend·migration image와 revision을 갱신하고 migration `0068`을 승인된 UAT handover 절차로 적용한다. origin direct 차단을 확인한 뒤에만 Front Door domain·TLS·Entra·provider 검증으로 진행한다.
