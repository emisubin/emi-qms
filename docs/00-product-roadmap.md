...
| 6.3 | TASK-CI-COST-001 — GitHub Actions minute 최적화 | P2_REMEDIATION | Main Merged / PR·Main Validation Complete / Observation Pending | PR #89 최신 head `5/5`, 이전 run 취소 3건, squash merge `f50be6a`, main 성공 4·Full-Stack skip 1. rename 회귀 포함 local classifier `7/7`·Gate `6/6` | 일반 CI run 집계, TASK-AZURE-DEPLOY-001 Change 018 승인형 release 보존 | 문서 전용 closure PR과 최소 1주 Actions 사용량 관찰 | No | closure PR → 최소 1주 사용량 관찰 |
...
- 상태/다음 순서: PR #89 최신 head `5/5`·이전 run 취소 3건·squash merge `f50be6a`·main 성공 4/Full-Stack skip 1 완료 / 문서 전용 closure PR과 최소 1주 사용량 관찰
...
- 다음 Gate: 문서 전용 closure PR에서 무거운 3개 job skip과 `CI Gate` 성공을 확인한다. 최소 1주 사용량 감소는 게시 후 관찰한다.
...
| 96 | GitHub Actions 일반 CI minute 과소비 | PR #89·main 검수·merge 완료 / closure·1주 관찰 진행 | 사용자/개발/품질 | TASK-CI-COST-001 | 코드 PR 전체 품질 Gate와 Azure 수동 release를 보존했다. PR 이전 run 취소 3건과 main Full-Stack skip을 실제 확인했으며 문서 전용 skip은 closure PR에서 확인한다. 실제 절감률은 최소 1주 관찰한다. |
