...
6. Fable 5가 확인된 interview를 source of truth로 primary draft Markdown 전문을 한 번 작성한다. 기본 target은 `tasks/<task-id>-planning.md`이며, 사용자가 개인 검토용 또는 사용자-facing `docs/` 전문을 primary draft로 명시하면 최신 change에 사용자 요청·exact target을 기록하고 그 승인된 target 하나만 사용한다.
