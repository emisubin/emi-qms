...
- [x] 모든 주요 화면에 Review-safe banner가 보인다.
- [x] 조회·검색·필터·정렬·상세가 가능하다.
- [x] 저장/수정/삭제/복구/상태 변경 버튼이 잠겨 있다.
- [x] 읽음/발송/retry/확인/제외 버튼이 잠겨 있다.
- [x] 잠긴 버튼에 이유가 표시된다.
- [x] API/User 카드와 관리자 목록이 정상이다.
- [x] console 오류와 모바일 가로 overflow가 없다.
- [x] 5174 Development와 5185 Preview가 유지된다.
- [x] 저장이 필요하면 Development UAT로 이동해야 함을 이해한다.
- [x] SOP를 비개발 참여자와 함께 이해할 수 있다.

현재 상태: **Checklist 작성됨 / 자동 검증 완료 / 사용자 검수 완료 / PR #26 병합 승인**

검수 증빙: 검수 사용자 A / 2026-07-10 / HTTPS Review-safe UAT 5190와 본 User manual / 직접 화면·문서 검수 및 병합 승인.
