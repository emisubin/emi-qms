...
### 3.1 GitHub metadata 개인정보 안전 projection

- PR 조회에서 작성자, committer, reviewer, assignee, participant field를 요청하지 않는다.
- 기본 `gh` table, raw API response, commit metadata 전체 출력은 사용하지 않는다.
- PR 번호·상태·Draft·merge 상태·base/head branch·head SHA·changed file count와 CI success/failure/pending count만 요청한다.
- 결과는 boolean, integer, fixed enum과 repository file path로만 projection한다.
- 출력 전 email, 사람 식별 metadata key, GUID, HTML, long token, 허용되지 않은 JSON key를 차단하는 guard를 통과시킨다.
- Guard 실패나 실제 개인정보 출력이 발생하면 원문을 재출력하지 않고 merge를 즉시 중단한다.
...
- [x] 5190 접속과 Review-safe banner 정상
- [x] Migration 27/28/1 호환 상태 이해
- [x] 프로젝트·업무·알림·관리자 조회 정상
- [x] Dashboard/detail count 일치
- [x] 알림 recipient scope 정상
- [x] Mutation action disabled와 API 423
- [x] 주요 표 정렬과 mobile overflow 정상
- [x] 개인정보 원문이 보고에 없음
- [x] 데이터 정리 권장안 이해
- [x] SOP 반복 실행 가능
- [x] 신규 기능 No-Go 유지 확인
...
검수 증빙: 검수 사용자 A / 2026-07-11 / Current Review-safe 5190와 본 SOP / 화면·수치·접근 범위·개인정보 안전 GitHub projection 절차 검수 및 PR #29 병합 승인. Runtime·DB 방어는 자동 증빙을 함께 사용했다.
...
| 2026-07-11 | 개인정보 안전 GitHub projection과 사용자 검수 완료·PR #29 병합 승인 반영 |
