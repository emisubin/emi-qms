...
| 4.7 | TASK-QUALITY-OPERATING-MODEL-001 — 구매품 구분 기반 IQC / Change 004 Item별 LQC / Change 005 구분별 IQC 방식·양식 | APPROVED_FEATURE_IMPLEMENTATION | Change 006 Latest Main Integrated / Automated Validation Complete / User Validation Pending | Change 004·005를 CI 정책이 반영된 최신 `origin/main` `12fd5194`에 선택 이식했다. migration `0070`·`0071`, Item별 LQC 생성 snapshot, 구분별 IQC 검사 없음·스캔형·상세형을 보존하고, LQC 회귀 fixture를 명시적 운영 설정 경로로 보정해 전체 Backend `490/490`, Frontend `190/190`, Mock UI `8/8`, Full-Stack `57/57`을 통과했다 | TASK-009A·TASK-ADMIN-002·TASK-012A·WORKFLOW-CONTINUITY-001·PRODUCTION-CONTROL-001·TASK-CI-COST-001 기존 계약 | 사용자 화면 검수와 최신 PR head `CI Gate` 통과 전 merge·Azure 교체 금지. 기존 확정 IQC/LQC 증빙·Pending 삭제 금지 | Yes | Change 004·005 사용자 검수 → PR·CI Gate → main merge → migration·Backend·Frontend 운영 교체 |
