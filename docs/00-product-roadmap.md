...
| 6.2 | TASK-AZURE-DEPLOY-001 — 20일 Azure 시범 배포 | BUGFIX / UAT_RUNTIME | Change 032 Osan Phase-1 Released / Access Change 008 Local Validated · PR Pending | Change 007 hotfix 공개 뒤 확인한 부서 이동의 관리 역할 잔존, membership/local readiness 승인 상태 불일치와 System Administrator permission 누락을 Access Change 008에서 role provenance·공통 readiness·additive Business `0088`·멱등 backfill repair로 보정했다. 집중 compile/migration/3-DB/UI 검증이 통과했다. | TASK-AZURE-PILOT-001, TASK-CI-COST-001, TASK-SITE-ACCESS-001, TASK-G2-OPERATIONS-002 Change 004, TASK-OSAN-ISOLATION-001, TASK-OSAN-ACCESS-001, TASK-OSAN-PROJECT-001 | 일반 사용자 한 사업부, DB 분리, Cheongju 데이터·provider 보존, identity contract Directory `0001`/Business `0086`, ordinary 권한 상승 fail-closed | Yes | Change 008 commit·PR → exact-head CI 1회 → 승인된 squash merge → `0088`/repair/Backend/Frontend 공개 재배포 |
...
| 6.5 | TASK-G2-OPERATIONS-002 — 납품 목표·불량·홈 임시 시뮬레이션 | NEW_FEATURE / BUGFIX / UAT_RUNTIME | Change 004 Main Merged / Azure Released / Automated Public Formula Check Complete | 사용자 승인 Codex 직접 기획·구현. Additive migration `0084`로 납품 목표와 불량을 정식 데이터에 추가하고 홈 저장 없는 임시 시뮬레이션을 제공한다. Change 004는 2026-08-28부터 `오늘 재고 = 전일 재고 + 전일 생산 - 전일 불량 - 오늘 납품`으로 교정했다. 제품 PR #119, exact main `7a2c7f172a4a0e4b0e69a29c72ac205af1299c74`, Azure release `33589472932`이 통과했고 공개 8월 28일 기대·표시 재고 `6`이 일치한다. Migration·schema·원본 데이터는 변경하지 않았다. | TASK-G2-OPERATIONS-001 완료 계약, 최초 PR #115, Change 003 PR #117, Change 004 PR #119 | Persistent UAT 미적용. 운영 G2 원본 데이터·schema 무변경 | Yes | 사용자 운영 화면 검수 → 재고 흐름 관찰 |

### 6.6 오산 시범 운영 — 기획 승인·개발 Task 등록

2026-09-06 사용자는 오산 기획을 승인하고 문서화·Task 분해·개발 Roadmap 작성을 요청했다. 제품 구현은 각 하위 Task의 별도 승인과 품질 Gate를 따른다.

오산은 같은 PostgreSQL 서버 안의 별도 업무 DB를 사용한다. 8개 입력의 프로젝트 생성, 자유 제품명, 수량별 공통 7단계 진행, 전체 대상 포장 시 자동 완료와 전체 현황판을 제공한다. 청주의 18단계·제조·품질·물류·G2는 그대로 보존하고 오산의 중단·펜딩·외부 알림은 제외한다.

| 순서 | Task | 상태 | 선행조건 | Next Gate |
| ---: | --- | --- | --- | --- |
| 1 | [TASK-OSAN-ISOLATION-001](../tasks/osan-isolation-001.md) — 데이터 분리 기반 | USER_VALIDATION_COMPLETE_RUNTIME_VALIDATION_DEFERRED | 공통 소속 DB 포함 로컬 구현·격리 테스트 승인, Change 001~003, 사용자 검수 완료 | 완료 — 동적 검증 P2는 Task 6으로 이관 |
| 2 | [TASK-OSAN-ACCESS-001](../tasks/osan-access-001.md) — 사용자·사업부 전환 | CHANGE_008_LOCAL_VALIDATED_AWAITING_PR_CI | 1 | Change 008 exact-head CI → merge → Azure migration/repair/app handover |
| 3 | [TASK-OSAN-PROJECT-001](../tasks/osan-project-001.md) — 프로젝트 등록 | USER_VALIDATION_COMPLETE | 1, 2 | 완료 — Change 005 현재 화면 사용자 검수 완료 |
| 4 | [TASK-OSAN-PROGRESS-001](../tasks/osan-progress-001.md) — 진행·자동 완료 | PLANNED | 3 | 구현 승인 → 7단계 진행·자동 완료 구현 |
| 5 | [TASK-OSAN-DASHBOARD-001](../tasks/osan-dashboard-001.md) — 전체 현황판 | PLANNED | 4 | 구현 승인 → 집계·검색·desktop/mobile 검증 |
| 6 | [TASK-OSAN-VALIDATION-001](../tasks/osan-validation-001.md) — 격리 통합 검증 | PLANNED | 1~5 | 격리 실행 승인 → Full-Stack·청주 회귀·사용자 검수 |
| 7 | [TASK-AZURE-DEPLOY-001 Change 032](../tasks/osan-pilot-001-rollout-handoff.md) — 오산 1단계 개통 | PHASE1_RELEASED_CHANGE_007_HOTFIX_RELEASED | PR #121 phase-1와 PR #122 Change 007 hotfix가 exact main·required CI·Azure release를 통과했다. | 실제 계정 사용자 검수와 첫 실제 Osan 프로젝트 입력 |

Task 2 최초 구현은 local commit `2e29938f754f3d95444df2b341a921cfd1fca43f`로 고정했다. Task 3은 Backend 582/582, 누적 Frontend·browser·3-DB 검증과 Change 002~005 화면 보정을 통과했고 사용자가 2026-09-07 Change 005 현재 화면의 검수 완료를 명시했다. 이어 사용자가 Task 2의 단일 소속 selector 가시성 결함을 우선 보정하도록 명시해 Change 002를 재사용했다. Backend의 active membership collection이 두 곳 이상인 총괄에게만 desktop/mobile header와 access gate의 선택 UI가 보이며, 단일 청주·단일 오산은 control과 label을 보지 않는다. Frontend 297/297, mock browser 3/3, 실제 3-DB 격리 2/2와 1440×900·390×844 직접 확인을 통과했고 Change 002 사용자 검수는 대기한다. 사용자는 이어 Task 4~6보다 먼저 Task 1~3만 Azure에 배포해 실제 프로젝트 기준정보 입력을 시작하도록 명시했다. 이 부분 순서 변경은 `TASK-AZURE-DEPLOY-001 Change 031`에만 적용하며 progress mutation·auto-complete·dashboard, Pending/hold/cancel/deleted/Excel과 Osan external provider/worker는 제외한다. 일반 사용자의 dual-membership 전환 결함은 보류하고 phase 1에서는 membership 한 곳만 부여한다. 제품 개발의 다음 Gate는 계속 Task 4 구현 승인이고, 배포 Gate는 Change 031 Draft PR·CI 뒤 selector 사용자 검수와 exact main merge 승인이다. Task 1의 legacy 검사 자원 2개 정리는 사용자가 직접 수행할 P3이며 두 Gate를 막지 않는다.

이후 같은 날 사용자는 Azure 병합·배포를 다시 멈추고 `TASK-OSAN-ACCESS-001 Change 003`을 먼저 구현·검수하도록 명시 승인했다. 별도 `사업부 소속 관리` 진입점을 제거하고 `사용자 관리`에 Directory 승인 대기, 사업부별 부서·역할·부서장·활성을 통합했다. Directory 0003은 local profile을 먼저 commit한 뒤 membership을 공개하고 회수는 membership을 먼저 차단한다. 일반 사용자의 두 사업부 신규 활성은 DB에서 거부하며 지정 총괄만 다중 소속을 가질 수 있다. 검수 전 최소 집중 검증은 통과했고 전체 회귀는 사용자 검수 뒤 PR 게시 최종 head에서 한 번만 실행한다. 따라서 현재 배포 Gate는 Change 002 selector 세 경우와 Change 003 통합 화면 사용자 검수다. PR #121 remote head와 Azure는 변경하지 않았다.

후속 검수·CI·main 병합 뒤 오산 phase-1은 Change 032로 공개됐다. 운영 화면에서 확인된 이름·계정 ID 및 총괄 양 사업부 접근 누락은 `TASK-OSAN-ACCESS-001 Change 007`로 보정했고 PR #122 exact main `08c5366f...`, release run `34188740914`에서 Directory `0004`→기존 총괄 local-first backfill→Backend→Frontend 순서로 배포했다. 사용자 이름·계정 ID, 총괄 `3`명, 우측 selector와 청주↔오산 화면을 실제 확인했다. 일반 사용자 단일 사업부·기존 Cheongju 데이터·provider와 Osan worker off 계약은 유지한다.

같은 운영 검수에서 부서 변경 뒤 관리 역할·부서장 잔존, Osan 승인 표시와 실제 readiness 불일치, 총괄 System Administrator permission catalog 누락을 확인했다. 사용자는 세 결함 전체 보정과 운영 repair·main 병합·Azure 재배포를 명시 승인했다. `TASK-OSAN-ACCESS-001 Change 008`은 Business `0088` role provenance·전체 permission invariant, 공통 readiness, stale operation retry와 readiness-aware backfill repair를 구현했으며 local 전체 suite를 반복하지 않고 migration `1/1`, 격리 3-DB `1/1`, mock browser `3/3`을 통과했다. 다음 Gate는 exact PR head의 원격 전체 CI 한 번과 승인된 순차 재배포다.
...
| 97 | G2 납품 목표·불량·홈 임시 시뮬레이션 | Change 004 Main Merged / Azure Released / Automated Public Formula Check Complete | 사용자/제조/영업/물류/개발 | TASK-G2-OPERATIONS-002 | 최초 exact main `220d1201c9dbb881fb3e5c5061871fb943c7961b`에 납품 목표·불량·홈 임시 예상값과 migration `0084`가 반영됐다. Change 004는 제품 PR #119·exact main `7a2c7f172a4a0e4b0e69a29c72ac205af1299c74`·Azure release `33589472932`로 전일 생산·불량과 당일 납품을 반영하는 수식을 공개했다. 인증된 공개 2026-08-28 계산 `2 + 34 - 0 - 30 = 6`이 표시 재고 `6`과 일치한다. Schema·원본 데이터·Persistent UAT는 변경하지 않았으며 다음 Gate는 사용자 운영 관찰이다. |
...
| 99 | 오산 시범 운영 기획과 7단계 개발 로드맵 | Phase 1 Public Release Complete / Access Change 008 Repaired / Interactive User Smoke Pending | 사용자/개발/운영 | [TASK-OSAN-PILOT-001](../tasks/osan-pilot-001.md) | Task 1 DB 분리 기반, Task 2 통합 승인·사업부 selector와 Task 3 프로젝트 등록·목록·상세를 공개했다. Exact main `b41c932e2154a921cf8b0aab753fc6d0692b209f`, Azure release `34225420777`에서 Directory `0001..0004`, Cheongju·Osan business `0001..0088`, Backend·Frontend와 public security가 통과했다. Roleless membership `1`건과 stale managed System Administrator `1`건을 보정했고 final inspect `34229045510`은 전체 변경 marker와 양 DB permission gap이 `0`이다. 일반 사용자는 한 사업부만 유지한다. 다음 제품 Gate는 Task 4 구현 승인이고, 운영 Gate는 실제 계정의 부서 이동·Osan 승인·총괄 양 사업부 읽기/쓰기 smoke다. |
...
| 2026-09-02 | `TASK-G2-OPERATIONS-002 Change 004` PR #119 main 병합과 `TASK-AZURE-DEPLOY-001 Change 030` 공개배포 완료 | 제품 PR CI `33587777592`, exact main `7a2c7f172a4a0e4b0e69a29c72ac205af1299c74`의 main CI `33589432228`, Azure release `33589472932`이 통과했다. Backend·Frontend·public security는 `PASS`, migration은 `SKIPPED`다. 별도 공개 확인은 health `200`, 익명 root·API `401/401`, 인증된 G2 8월 28일 `2 + 34 - 0 - 30 = 6`과 표시 재고 `6` 일치다. 운영 G2 원본 데이터와 Persistent UAT는 변경하지 않았다. | 6.2, 6.5, 추적 97, TASK-G2-OPERATIONS-002 Change 004, TASK-AZURE-DEPLOY-001 Change 030 |
| 2026-09-06 | 오산 기획 승인·문서화·Task 분해·개발 Roadmap 등록 | 같은 PostgreSQL 서버 안의 별도 청주·오산 업무 DB, 승인된 8개 입력, 수량별 7단계, 전체 포장 시 자동 완료, 전체 현황판과 중단·펜딩 제외를 고정했다. 실제 제품 구현·운영 DB·provider·Git 게시는 하위 Task별 승인으로 분리한다. | 6.6, 추적 99, [기획](../tasks/osan-pilot-001-planning.md), [상위 Task](../tasks/osan-pilot-001.md) |
| 2026-09-07 | Task 2 마지막 일괄 검수 이관·local commit과 Task 3 구현 승인 | 사용자는 Task 2 화면 검수를 마지막에 한 번에 진행하도록 이관하고 Task 3 구현을 승인했다. Task 2 결과는 `2e29938f754f3d95444df2b341a921cfd1fca43f`로 고정했다. 앞으로 승인된 구현이 필수 테스트·parent review·적용되는 독립 검증을 통과하면 별도 질문 없이 exact allowlist를 local commit한다. 이 권한 확대는 local commit에만 적용하며 push·PR·merge·정리·Persistent UAT·provider·운영 적용은 별도 승인이다. | 6.6, 추적 99, [Task 2](../tasks/osan-access-001.md), [Task 3 Change 001](../tasks/osan-project-001-change-001.md) |
| 2026-09-07 | `TASK-OSAN-PROJECT-001 Change 001` 구현·자동 검증·parent review 완료 | 오산 전용 등록·목록·상세, 8개 입력, 수량별 대상과 7단계 snapshot, DB code unique, idempotency·rollback·감사·사업부 격리를 구현했다. Backend 582/582, Frontend 291/291, mock browser 1/1과 실제 3개 DB full-stack 1/1을 통과했고 parent review P2 네 건을 보정했다. Fresh GPT-6 High 독립 검증과 자동 local commit이 다음 Gate이며 사용자 화면 검수는 마지막 일괄 검수로 유지한다. | 6.6, 추적 99, [Task 3](../tasks/osan-project-001.md), [구현 보고](../tasks/osan-project-001-implementation-report.md) |
| 2026-09-07 | `TASK-OSAN-PROJECT-001 Change 001` 독립 재검증·품질 Gate 완료 | 첫 verifier가 반환한 replay access와 코드 내부 공백 표시 P2를 보정하고 Backend 582/582, Frontend 291/291, mock browser 1/1을 다시 통과했다. Fresh GPT-6 High 재검증에서 제품 신규 Finding은 없었고 exact allowlist 누락 P2도 문서 보정·재확인 `PASS / GO`로 닫아 open P0/P1/P2/P3 `0/0/0/0`을 확인했다. Task 3은 마지막 일괄 사용자 검수 대기 상태이며 다음 제품 Gate는 Task 4 구현 승인이다. | 6.6, 추적 99, [Task 3 Change 001](../tasks/osan-project-001-change-001.md), [구현 보고](../tasks/osan-project-001-implementation-report.md) |
| 2026-09-07 | `TASK-OSAN-PROJECT-001 Change 002` 청주형 목록·상세 정렬과 독립 재검증 완료 | 사용자의 명시적 순서 변경 지시로 오산 목록을 프로젝트별 한 행, 상세를 청주형 기본 정보와 단일 `진행 관리` tab 구조로 맞췄다. Frontend 292/292와 mock browser 1/1을 통과했고 첫 fresh GPT-6 High 검증의 mobile code 변형·생성 직후 상태 표기 P2 두 건을 보정한 뒤 새 fresh GPT-6 High `PASS / GO`, open P0/P1/P2/P3 `0/0/0/0`을 확인했다. Local synthetic 화면 검수 대기이며 Backend·DB·Push·PR·merge·Persistent UAT·provider는 변경하지 않았다. | 6.6, 추적 99, [Task 3 Change 002](../tasks/osan-project-001-change-002.md), [구현 보고](../tasks/osan-project-001-implementation-report.md) |
| 2026-09-07 | `TASK-OSAN-PROJECT-001 Change 003`에서 오산 전용 UI를 제거하고 청주 UI·UX를 직접 재사용 | 사용자는 Change 002가 청주와 비슷한 별도 오산 UI를 만든 결과라 과도하다고 판정했다. 목록·상세·제조 현황의 기존 청주 class·DOM·반응형 구조를 직접 사용하고 오산 차이는 데이터 매핑, 단일 `진행 관리` 탭, 현재 단계·완료 수/7과 금지 기능 제외로 제한했다. Frontend 292/292, mock browser 1/1과 fresh GPT-6 High `PASS / GO`, open 제품 P0/P1/P2/P3 `0/0/0/0`을 확인했다. Local 화면 검수 대기이며 Backend·DB·Push·PR·merge·Persistent UAT·provider는 변경하지 않았다. | 6.6, 추적 99, [Task 3 Change 003](../tasks/osan-project-001-change-003.md), [구현 보고](../tasks/osan-project-001-implementation-report.md) |
| 2026-09-07 | `TASK-OSAN-PROJECT-001 Change 004` 실제 공용 컴포넌트 통합·paired 시각 검증 완료 | Change 003은 청주 class와 유사 DOM을 별도 branch에 복제해 실제 행 높이·열 정렬·native element 차이를 남겼다. 목록·상세 요약·부서 현황을 세 공용 React 표시 컴포넌트로 통합하고, 청주·오산 1440×900·390×844 목록·상세 8개를 같은 run에서 캡처해 Parent와 fresh verifier가 눈으로 비교했다. Frontend 292/292, 집중 97/97, paired browser 1/1과 최종 `PASS / GO`, open P0/P1/P2/P3 `0/0/0/0`을 확인했다. `frontend/AGENTS.md`에 실제 공용 컴포넌트와 양쪽 시각 증빙 gate를 추가했으며 변경·게시 권한은 확대하지 않았다. Local 화면 검수 대기이고 Backend·DB·Push·PR·merge·Persistent UAT·provider는 변경하지 않았다. | 6.6, 추적 99, [Task 3 Change 004](../tasks/osan-project-001-change-004.md), [구현 보고](../tasks/osan-project-001-implementation-report.md) |
| 2026-09-07 | `TASK-OSAN-PROJECT-001 Change 005` 목록 전체 page composition 공용화·paired 시각 검증 완료 | 사용자 확인에서 Change 004가 행·카드와 상세 표시만 공용화하고 청주 목록의 검색·납기 filter, KPI와 상태 tab을 빠뜨린 사실을 확인했다. Page header부터 목록까지 `ProjectListPageComposition`으로 통합하고 오산에는 전체·시작 전·완료 조회만 연결했다. Frontend 293/293, 집중 98/98, paired browser 1/1, 열린 local·청주 화면 대조와 fresh GPT-6 High `PASS / GO`, open P0/P1/P2/P3 `0/0/0/0`을 확인했다. 앞으로 동일성 검증은 route 전체 inventory와 desktop/mobile 전체 screenshot을 기준으로 하며, 이는 완료 의무만 넓히고 mutation·Git·운영 권한을 확대하지 않는다. Backend·DB·Push·PR·merge·Persistent UAT·provider는 변경하지 않았다. | 6.6, 추적 99, [Task 3 Change 005](../tasks/osan-project-001-change-005.md), [구현 보고](../tasks/osan-project-001-implementation-report.md) |
| 2026-09-07 | Task 3 Change 005 사용자 검수 완료와 `TASK-OSAN-ACCESS-001 Change 002` selector 가시성 보정 | 사용자는 Change 005가 반영된 현재 Task 3 화면의 검수 완료를 명시한 뒤, 실제 이동 가능한 사용자에게만 사업부 선택 UI를 표시하고 단일 소속 사용자는 빈 label을 포함해 전혀 보지 않게 하라고 지시했다. Task 2 Change 002는 Backend의 active membership collection을 사용해 목적지가 두 곳 이상인 총괄에게만 desktop/mobile header와 access gate의 selection UI를 렌더링한다. Frontend 297/297, mock Chromium 3/3, 실제 3-DB 격리 2/2와 1440×900·390×844 직접 확인을 통과했다. 사용자가 지정한 `GPT_5_6_SOL_XHIGH_ONLY` 단독 경로를 사용했고 GPT-6 review는 요청하지 않았다. Local commit만 승인됐으며 Push·PR·merge·main·Persistent UAT·provider·운영 적용 권한은 확대되지 않았다. | 6.6, 추적 99, [Task 2 Change 002](../tasks/osan-access-001-change-002.md), [Task 3 Change 005](../tasks/osan-project-001-change-005.md) |
| 2026-09-07 | `TASK-AZURE-DEPLOY-001 Change 031` 오산 Task 1~3 부분 배포 순서 변경·Azure 작업 시작 승인 | 사용자는 일반 사용자 dual-membership 전환 결함을 뒤로 미루고 phase 1 계정에 membership 한 곳만 부여하는 운영 제약 아래, Task 4 진행·자동 완료와 Task 5 dashboard보다 먼저 로그인·사업부 gate·관리와 Osan 프로젝트 create/list/detail을 공개 배포하도록 명시했다. 기존 운영 DB를 Cheongju로 유지하고 같은 server에 Directory·Osan DB를 추가하며 role/identity/migration과 3-DB PITR rehearsal 뒤 public Backend를 연결한다. Read-only preflight, 배포 정의·기록·local 검증·commit, remote branch·Draft PR·CI와 승인된 Azure 조작은 포함되지만 exact main merge는 별도 1회 명시 승인을 유지한다. Change 002 selector는 자동·시각 검증 완료, 사용자 검수 대기다. | 6.2, 6.6, 추적 99, [Change 031](../tasks/azure-deploy-001-change-031.md), [rollout handoff](../tasks/osan-pilot-001-rollout-handoff.md) |
| 2026-09-08 | `TASK-OSAN-ACCESS-001 Change 008` 운영 접근 정합성 보정과 `TASK-AZURE-DEPLOY-001 Change 032` 재배포 완료 | PR #127 exact head CI 뒤 main `b41c932e2154a921cf8b0aab753fc6d0692b209f`를 release `34225420777`로 공개했다. Business `0088`, roleless membership `1`건 회수와 stale managed System Administrator `1`건 제거를 완료했고 final inspect `34229045510`에서 모든 변경 marker와 양 business permission gap이 `0`이다. Cheongju 데이터·provider, DB 분리/no-fallback, 일반 사용자 단일 membership과 Osan provider/worker off를 보존했다. | 6.2, 6.6, 추적 99, TASK-OSAN-ACCESS-001 Change 008, TASK-AZURE-DEPLOY-001 Change 032 |
