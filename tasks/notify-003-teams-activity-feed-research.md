...
- 검수 사용자 A EntraId active 사용자 row를 수신자로 확인했다. email과 Entra object id는 masked 기준으로만 검수했다.
...
- 검수 사용자 A 계정에 EMI 프로젝트 통합관리시스템 Teams 앱을 설치한다.
...
검수 사용자 A Teams 계정에 앱이 설치되어 있다는 사용자 확인이 있었고 현재 권한은 `TeamsActivity.Send`이므로, `TeamsActivityAppNotInstalled`를 RSC authorization 누락보다 Graph payload / 사용자 id / Teams app id / topic 방식 정합성 문제 후보로 재분류했다.
...
- 검수 사용자 A qms user의 Entra object id가 실제 Teams 사용자와 다르다.
...
- Graph Explorer 또는 관리자 권한 도구에서 검수 사용자 A 사용자 기준 installedApps 목록을 조회한다.
...
| TeamsActivity | 검수 사용자 A EntraId 사용자 Activity Feed | Sent | `07da3020-276a-41ba-8a90-9178d1133e0c` | 없음 |
...
- localhost manifest는 검수 사용자 A PC에서만 의미가 있다.
...
다중 사용자 TeamsActivity actual 검수에서 사용자별 `InstalledAppId`를 수동 관리하는 방식은 운영에 맞지 않다는 결론을 냈다. 검수 사용자 A 계정의 installed app id로 성공하더라도 검수 사용자 B 등 다른 사용자는 각자 다른 installed app id를 가지므로, per-user installed app id를 env에 넣는 방식은 확장할 수 없다.
...
- `.env.notify-local`에 `Notifications__TeamsActivity__TeamsCatalogAppId`가 configured 상태여야 검수 사용자 A/검수 사용자 B catalog topic actual 재검증을 수행할 수 있다.
...
- localhost manifest는 검수 사용자 A PC 로컬 테스트용이며, 다른 사용자 검수에는 Dev Tunnel/ngrok/운영 HTTPS 도메인이 필요하다.
...
- localhost manifest는 검수 사용자 A PC 로컬 전용이고, 다른 사용자/운영 검수는 Dev Tunnel/ngrok/운영 HTTPS 도메인이 필요하다.
...
- 검수 사용자 A EntraId 사용자: installedAppId 없이 text topic + Teams deep link webUrl로 `Sent`
- 검수 사용자 B EntraId 사용자: installedAppId 없이 text topic + Teams deep link webUrl로 `Sent`
