...
export function OsanStepper({ stages, individual = false }: { stages: OsanDashboardStage[]; individual?: boolean }) {
  return <span className="osan-status-track" role="group" aria-label="단계별 진행 상태">{stages.map(stage => {
    ...
    const hint = stage.sequenceNumber === 5 && !done ? '상시 가능' : next ? individual ? '다음 작업' : `가능 ${available}대` : stage.sequenceNumber === 7 && !done ? '포장 대기' : '';
